...
4. **Account for every declared path.** Each path §2.9 declared is now either staged by step 3, or reported by no record at all — because it is already committed and unchanged, or because this phase did not in the end write it. A declared path that exists under `$SPECS_PATH` and that step 3 staged nothing for — and any record step 2 could not read as a path — is named in §4.1's *declaration unaccounted for* clause. Nothing this step cannot place is dropped in silence — which is the half of the defect *What this closes* records that `-z` alone does not buy: the drop had no voice, so the run reported a handoff it had not made. This does not stop the entry point: the paths that did stage are still committed, pushed and opened as a pull request, and the clause is what tells the operator which declared artifact is not among them.

**`-z` is required, and `specs-repo-git.md` §2.1 is where the hazard it answers is stated.** That section names it twice for its own classifier — `git status --porcelain` wraps a path carrying a space, a `"`, a `\` or a non-ASCII byte in double quotes and octal-escapes the non-ASCII bytes, and a classifier anchored at `^` and reading the path as reported never matches it. §2.1's step 1 reads `-z` for that reason and so does this one, over a different path set. The difference worth keeping in view is that §2.1 can also **constrain the paths it owns** — it flattens `<docs-repo-slug>` to a single segment of safe bytes — while a deliverable path here need not be the plugin's to shape at all: `/brd-intake` copies a customer's linked files under the names the customer gave them, and `/idea` vendors a designer's exports under theirs. Under `-z` each record is terminated by a NUL and the path is emitted raw: strip the two status bytes and the space from the front of a record and the remainder is the path, byte for byte as declared. A **rename or copy** record carries a second NUL-terminated field, the original path, straight after it (`R  <new>\0<old>\0`) — consume it with the record it belongs to, never as a record of its own, which has no status bytes to strip. `-c core.quotepath=false` is **not** the fix and must not be substituted for `-z`: it suppresses only the octal escaping, and a path carrying a space is still quoted.

**What this closes.** Until `-z` was read here, a declared deliverable whose filename carried a space or a non-ASCII byte was reported quoted, matched no declared path, fell to **OTHER**, and was never staged — while the run reported the handoff as done. That is the silent-and-total failure the paragraph below names for an *undeclared* path, reached by a second route: the path was declared, and the classifier could not see that it was.
...
| `/prd-ground` **(a slice)** | the BRD's `coverage-ledger.md` | **stops**, and **splits row F into two stops on a test this gate cannot make** — no `coverage-ledger.md` in the folder at all is *never produced*, which is `PRD_GROUND_NEEDS_SPLIT` for a slice or, for the legacy unprefixed folder that reads as an interrupted intake instead, `PRD_GROUND_NEEDS_INTAKE`; a ledger in the folder but on no ref is `PRD_GROUND_NOT_HANDED_OFF` (produced, handoff declined), whose action is to land the files already on disk, and which names `/brd-split` as a way out only where re-running it would in fact stage them — never on a **bare** re-run against a fully-allocated parent. Never optional either way: grounding has no claim list without the inventory this file arrives with, and the route ships with no pre-gate behaviour to fall back to |
...
| `/prd-ground` **(a slice)** | the BRD's `brd/brd-inventory.md` | **stops** — `PRD_GROUND_NO_INVENTORY` where no inventory is in the folder (naming the producer by level), `PRD_GROUND_INVENTORY_NOT_HANDED_OFF` where one is in the folder and on no ref. Row F is split on that test rather than borrowing step 8's `PRD_GROUND_EMPTY_INVENTORY`, which reports a content fact and whose remedy would rewrite an inventory that is merely unmerged. Never optional: the run's whole claim list comes from this file |
| `/prd-ground` **(idea route)** | `prd.md` | **stops**, and **splits row F the same way**: no `prd.md` on any ref and none in the folder either is `PRD_GROUND_NEEDS_PRD` (never produced); one in the folder and on no ref is `PRD_GROUND_PRD_NOT_HANDED_OFF` (produced, handoff declined), whose action is to land the file already on disk and which never names `/create-prd`, since re-running it would author a second PRD rather than land the one written. Never optional: this route's whole claim list comes from `prd.md`, and it ships with no pre-gate behaviour to fall back to |
...
**The callers named in the two groups below refuse a resolved `BRD-` container outright, and the two groups refuse for different reasons — a difference that decides how much of the refusal their rows inherit. No count is kept here, and that is the fix rather than an omission**: successive attempts corrected the total and then asserted something new beside it — how many rows a group holds, what every row in it describes — each one quantified over a set whose members had not been opened one by one. And nothing gates a digit here: `scripts/check-docs.sh` check 11 derives each command's `require-on-main` target from the table above — column 2's backticked filenames — and never from the prose beneath it.

**The route commands** — `/prd-ground`, `/brd-interview`, `/brd-package` and `/brd-reconcile`, each with its own `*_ROOT_LEVEL` stop — refuse because grounding, deciding, packaging and reconciling all happen at the slice, and a root has none of that work to do, so a root never reaches those gates at all. That is what the `(a slice)` qualification on their rows above records. **`/prd-ground`'s idea-route row is the exception, and it shows the qualification is per row rather than per command**: it gates a PRD folder that is never a BRD slice at all, on the same never-optional principle as that command's BRD-route rows.

**The build-ladder commands** — `/create-prd`, `/create-ard`, `/specify`, `/epics` and `/prd-proposal`, each stopping with a `*_BRD_NOT_SLICED` id — refuse for an unrelated reason: a BRD is a container, and the PRD, the ARD, the specification, the Epic set and the effort proposal are all authored in the `PRD-` slices under it. Their test is the **directory prefix** `product-workflows:coverage-ledger-format` §5 fixes, falling to §5.1's positive-evidence test only where the folder resolved through `workflows-core:addressing` §5's legacy unprefixed fallback and carries no prefix to read. `/create-ard`, `/specify`, `/epics` and `/prd-proposal` each take that refusal before this gate runs at all; **`/create-prd` orders the two the other way**, so the ordering is a fact about each command rather than about the group — its `idea.md` gate is Phase 0 step 3 and its container refusal step 5a, taken *"the moment step 5 returns `status: found`"*, and step 3's ladder is skipped only on a **detected** BRD route, which a root BRD folder carrying no `brd-link.md` is not. **What the refusal does to their rows is nothing** beyond removing the container from the folders these commands run on: it qualifies no row to a route, and a row is read on whichever route its own Input exists on — `/create-ard`'s and `/specify`'s PRD rows say *"Gate the PRD — on every route"* in their own commands, while `/create-prd`'s `idea.md` row is idea-route only, its ladder *"Skipped entirely on the BRD route, where `prd-seed.md` is the seed and there is no `idea.md` to find"* (`commands/create-prd.md` Phase 0 step 3). Reading the route commands' slice-only qualification onto any of these rows would narrow it to a route it does not belong to.

`/prd-proposal`'s reason is narrower still, and it is the one whose refusal offers rather than only redirects: an effort proposal over a container is the *umbrella*, a different document priced from different inputs, which is why it names `/brd-proposal` instead of only telling the operator to descend. **`/brd-proposal` inverts all of this and is the only row here that does**: it refuses a *slice* and runs at the root, so its row describes a root run gating a file in each of several **child** folders rather than one in the folder it resolved. That is a gate target per included slice, not one per run, and its stop collects them.

For the consumers that predate this gate, rows D and E add the only new stop: an artifact that **exists** and was never handed off. That state was **not** unreachable before this feature — pre-J, `/specify` already created `spec/<EPIC>-<eslug>` (or `spec/<PRD>-<vslug>`) branches and offered branch + PR, and `/create-prd` did the same on `prd/<KEY>-<slug>`, with no downstream gate reading them; an artifact sitting on such a branch, unmerged, was a common, ordinary state. This is a real behaviour change: for that state, `/create-ard`, `/specify`, and `/epics` now hard-stop where they previously proceeded with a documented fallback (the deliberate, well-argued stop in `epics.md`'s `/create-prd`-refusal table). It qualifies caller-contract rule 3 (§5 — no consumer turns an optional input into a prerequisite) precisely: for those consumers, row F's `absent` case is still fully delegated to the caller's own pre-existing behaviour, but rows D/E are a new stop for a state that was previously reachable and previously non-blocking. All six `/brd-*` consumers named in that qualification — `/prd-ground`, `/brd-split`, `/brd-interview`, `/brd-package`, `/brd-reconcile` and `/brd-proposal` (`/brd-intake` consumes nothing and runs no gate) — sit outside that qualification entirely: they have no pre-gate behaviour, because their inputs and the commands that read them shipped together, so rule 3 has no optional input to protect there.
...
| `brd/brd-inventory.md` **(a slice)** | gated | named in §3.4's `/prd-ground` row, itself level-qualified there (§3.4: "The route commands ... refuse because grounding, deciding, packaging and reconciling all happen at the slice ... That is what the `(a slice)` qualification on their rows above records") — the route half of that paragraph's two groups, never the build-ladder rows beside it, whose own refusal qualifies no row to a route — a root never reaches that gate, so it only ever reaches a slice's own copy. It was classed advisory for one commit, in the same change that gated it |
| `brd/brd-inventory.md` **(a root)** | advisory | `/brd-intake` writes it, and `/brd-split`'s own step 8 reads it directly in `split_mode: full` — a plain worktree read, never a `require-on-main` gate — and stops on what it finds there (`BRD_SPLIT_EMPTY_INVENTORY`). That stop is about the file's *contents*, the same way `/prd-ground` stopping on a moved pin in `grounding/baselines.md` is (§4.0's own advisory-class note); no gate on the handoff exists for it |
| `coverage-ledger.md` **(a slice)** | gated | named in §3.4's `/prd-ground` row, itself level-qualified there for the same reason as `brd/brd-inventory.md`'s row above — a root never reaches that gate |
| `coverage-ledger.md` **(a root)** | advisory | `/brd-intake` writes it, and `/brd-split`'s own step 8 reads it directly in `split_mode: full` — a plain worktree read, never a `require-on-main` gate — computing the no-op test from its disposition counts (`coverage-ledger-format.md` §3). No gate on the handoff exists for it either, and `/brd-intake`'s whole `deliverable_paths` set is therefore **advisory**, not gated: nothing else in it (`brd/source/*`, `brd/brd-defect-log.md`, `brd/brd-link-log.md`, the optional seeds) carries a §3.4 row, so no path lifts the set to `gated` |
| `brd-link.md`, `grounding/baselines.md`, `brd/source/*`, `brd/brd-defect-log.md`, `brd/brd-link-log.md`, `interview/customer-questions.md`, `self-review-<YYYYMMDD>.md`, `customer-delivery-note-<YYYYMMDD>.md`, `bundle-<YYYYMMDD>/`'s files (one literal path each, never the directory — §2.3, the same rule the `customer-sent-<YYYYMMDD>/` row states), `reconciliation-<YYYYMMDD>.md`, `prd-seed.md`, `ard-seed.md`, `spec-seed.md`, `/idea`'s vendored `attachments/*` and `design/idea-sources/*` | advisory | each is read by a named consumer and gated by none: `brd-link.md` by BRD-route detection in `/create-prd`, `/create-ard` and `/specify`, by `/epics` step 1a and by `/brd-split` step 5; `baselines.md` by `/prd-ground` Phase 3 on a re-run; the intake and package artifacts by `/brd-package` and `/brd-reconcile`; `brd/brd-link-log.md` by `/brd-split` Phase 3 step 4, whose slice-inventory header names the parent's source document and reads that basename off the log's opening line (`product-workflows:brd-format` §1.1, §2.1); the three seeds by their one altitude consumer each; and `/idea`'s vendored `attachments/*` by `/create-prd` and the grill commands that read the folder's `idea.md` and follow its links, its `design/idea-sources/*` by `product-workflows:design-grounder` through the frame-set index row below |
| `code-defect-log.md` | advisory | `/brd-package` Phase 0 step 10 reads every `[CDF#n]` in it with its `disposition`, `statement`, `intent` and `blocked_on`, and `product-workflows:brd-package-reviewer`'s sixth hunt class reads it to settle whether an `argumentation` claiming a recorded defect has one. Both are plain worktree reads; §3.4 names no gate on it. **Its absence is an ordinary state, not a decline** — `/brd-interview` writes the file only where a round raised an entry or re-dispositioned one already on file, and every reader above says so where it reads it |
...
| `proposal.md` **(a `PRD-` folder)** | gated | named in §3.4's `/brd-proposal` row, which runs `require-on-main` on each **included** slice's proposal before rolling it into the umbrella. **Conditionally gated is still gated for this table's purpose**, the same call `grounding/design-grounding.md`'s row above makes and for the same reason: the gate fires on a slice some later umbrella run includes, and the producer cannot know at authoring time whether one will — a class that varied per run would have it guessing at the consumer's future state. An idea-route `PRD-` folder, which no umbrella can ever include, is where that gate will in fact never fire, and it takes the same class anyway |
| `proposal.md` **(a `BRD-` umbrella)**, `proposal-brief.md` at either level, and the archived `revisions/<KEY>_proposal_<YYYYMMDD>.md` / `revisions/<KEY>_proposal-brief_<YYYYMMDD>.md` | unread | **looked for, and none found**, which is what §4.3 asks of a producer reaching for that array. The umbrella is read by nothing: §3.4's `/brd-proposal` row targets a **slice's** proposal, and no command reads an umbrella's. The brief is derived from the proposal and consumed by the customer, never by a command. The archived revisions are read by `proposal-reviewer` inside the run that archived them, which is a same-run read before the handoff rather than a downstream one — the anchor a later re-run diffs against is the canonical `proposal.md` it reads *before* archiving it, exactly as `/update-prd`'s archived revision is unread for the same reason |
...
| a frame-set `index.md` | advisory | `product-workflows:design-grounder` **refuses to run** on a frame set holding no index, and `product-workflows:grounding-verifier` returns `NO_INDEX`/`STALE_INDEX` on one — both dispatched by `/prd-ground` Phase 5 and Phase 7, both reading the working tree. §3.4 names no gate on it |

**Never infer an artifact's merged-ness from a sibling's gate.** A consumer may read the worktree and stop on what it finds there — most do, legitimately, and that is not what this rule is about. What is forbidden is the sentence *"every deliverable a `handoff-to-main` run stages lands in one commit, **so** this gated file's presence on `origin/<default>` implies its sibling merged with it."* That inference is true of the run that produced them and is **not a property of the tree**: `/prd-ground --no-code` hands off `design-grounding.md` alone, a hand-committed set can land partially (`/brd-reconcile`'s own §3.4 row says so), and either leaves the sibling on no ref while the gated file is merged.
...
| Declaration unaccounted for | append `; <path> was declared but staged by nothing — it is NOT on <branch>` (§2.3 step 4), one clause per path |
...
**Every staged path is named, beneath the line.** On each row that committed something — *Committed, pushed, PR opened*, *PR already existed*, *PR not opened*, *Push failed*, *No remote* — follow the outcome line with one indented line per path §2.3 step 3 staged, in the order it staged them, each written exactly as git reported it under `-z` (raw, never re-quoted). Above eight, print the first eight and `…and <n> more`; §2.7's body file carries the full set either way. The line itself stays one line, so the rule above is intact. This is the positive half of §2.3 step 4 and it is what made the quoted-path defect survive releases: nothing printed which paths landed, so nothing revealed that one had not. An operator who reads the list against the artifact they just wrote sees a missing name in the place they are already looking.
...
**unread** — nothing gates it and nothing reads it. §4.0's register now names its members: `/specify`'s `_session.md`, `_glossary.md` and rendered `.html`, `/design`'s `_design-session.md` and `_design-glossary.md`, `/update-prd`'s archived revision, and the whole of `/brd-proposal`'s deliverable set — the umbrella's own `proposal.md`, either command's `proposal-brief.md`, and either command's archived proposal revisions. **Two artifacts have already left this class after someone actually looked** — the frame-set index, which `design-grounder` refuses to run without, and `slices.md`, which `/brd-reconcile` corrects in place. A producer reaching for this array is making the claim both of those failed: that it looked for a reader and there is none. Look, before selecting it:
