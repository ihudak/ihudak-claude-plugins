...
| `preload-context` | `UserPromptSubmit` | — | On a matched `/implement`/`/vuln`/`/upgrade` prompt, injects git/model-routing/specs context; a near-instant no-op otherwise. |
...
Every script above ends `exit 0` unconditionally — the exit code is never derived from what the hook found, so a missing notifier binary or an unparsed test-output format never stops a tool call, a turn, or the run itself. Two of the three also guard on `command -v python3` up front and exit 0 immediately when it is absent, degrading to silence rather than failing — `preload-context.sh` and `test-notify.sh`; `notify-done` carries no such guard because it never calls `python3` at all.
