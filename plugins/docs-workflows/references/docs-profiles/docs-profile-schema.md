...
- `dev_servers.readiness_timeout_seconds` is optional (default 120) — how many seconds a consumer polls a server it started for readiness: `/document`'s Phase 6.5 before falling back to the manual table, and `/docs-serve`'s Phase 5 before reporting a timeout, which leaves the server running and records its entry without a pid but with the process group it started it in and that group's leader's start time, so `--stop` can still end it, and `--status` recognise it once it binds, whatever its command line carries. Both stop polling sooner where the process group they started the server in is gone, and report the command's exit instead of a timeout — save, in `/docs-serve`, an exit at a port something is listening on or whose log names the port as already in use, which it disposes of as a collision at that port (its Phase 5) rather than as the command's exit.
...
- Where the operator can, record a `dev_servers.servers[].command` whose tool keeps the command line it was given — `node_modules/.bin/vitepress dev docs --host 0.0.0.0 --port {port} --strictPort` rather than a launcher that rewrites its own as it runs, as npm does, dropping its own options and the `--` before the script's — since, for a server it holds no record of, one started by hand or one whose record is gone, `/docs-serve` identifies it by the command line its process carries and allows for one launcher rewrite only, `npx <rest>` becoming `npm exec <rest>`. A server it started itself it identifies by the process group it started it in, whatever that command line says. That comparison removes every quote from the recorded command and the command line and collapses each run of whitespace, which undoes the shell's own quoting and nothing more. The VitePress command carries `--strictPort` because a command binds the port it is given, or fails (below).
- `dev_servers.servers[].command` keeps its server **in the foreground**: it runs the server as one of its own processes and never detaches it — no `setsid`, no `nohup … &`, no trailing `&`, and no detach flag such as `docker run -d` or `docker compose up -d`. **A detaching command is a profile defect, like a loopback-only bind**: fix the command, not the consumer. Both consumers that start a server depend on this, because each tracks what it started. `/document`'s render check stops a server by the process group its start created, and a server moved out of that group — by `setsid` into a session of its own, or by `docker run -d` into a container — is one the check can neither stop nor see bind after it has moved on (`references/docs-profiles/render-verification.md` §2). `/docs-serve` starts a server in a process group of its own too, follows it through that group, and records a pid for it only where the port's listener is a member of that group or stands in its own checkout, so a server detached out of both — into a container above all, whose published port is held by a process elsewhere or by none the socket table shows — leaves `--stop` nothing to stop it by. **A foreground `docker run -p …` or `docker compose up` keeps to this rule, yet `/docs-serve` cannot identify the server it starts**: the port it publishes is held by Docker's forwarding — a `docker-proxy`, the kernel's NAT, or rootless Docker's own port driver — never by a process of the command's, so no process on that port carries the recorded command. Where the socket table shows no process there — a root `docker-proxy`, or NAT — its Phase 5 records the entry without a pid, as it does for any port whose process the table does not show, and its `--stop` can signal only the process group it started, which holds the `docker` client and never the container. Where the table shows a process Docker started outside the checkout, the run names it as another checkout's and records nothing. Either way, stop the container with Docker. **A tool can detach itself, too, and Astro 7 does wherever it detects an AI coding agent** — Claude Code, whose shell sets `CLAUDECODE`, among them: `astro dev` then re-spawns the server as a detached process of its own, its arguments rebuilt from the flags Astro knows, and returns, so both consumers meet a boot whose group is gone while something else serves the port. Astro documents the opt-out, the `ASTRO_DEV_BACKGROUND` environment variable, and an Astro command sets it ahead of the tool — `ASTRO_DEV_BACKGROUND=0 node_modules/.bin/astro dev --host 0.0.0.0 --port {port}` — which keeps the server in the foreground, in the group its start created (Astro 7.3.2, run under Claude Code); an `astro dev` command without it detaches, a profile defect like any other. `astro preview` detaches the same way since Astro 7.2.0, with an opt-out of its own, `ASTRO_PREVIEW_BACKGROUND` — `ASTRO_PREVIEW_BACKGROUND=0 node_modules/.bin/astro preview --host 0.0.0.0 --port {port}` — and a preview command without it is the same defect (Astro's documentation, and its `cli/preview` source at 7.3.2, where any non-empty value opts out, as for `astro dev`). **Astro also runs one dev server per project root, and one preview server**: `astro dev` records the server it runs in a lock file, `.astro/dev.json` under the root, and while that server runs a second `astro dev` for the same root exits at once with *"Another astro dev server is already running"*, whatever `--config` it is given. So a profile that serves one Astro root twice — a public and an internal server, say — runs only the first of them unless one of the two commands passes `--ignore-lock`, the flag Astro documents for running another dev server of the same project beside the first — `ASTRO_DEV_BACKGROUND=0 node_modules/.bin/astro dev --host 0.0.0.0 --port {port} --ignore-lock` — and never `--force`, which stops the server already running and takes its place (Astro 7.3.2, run under Claude Code, and its `cli/dev` source). `astro preview` keeps a lock of its own the same way, `.astro/preview.json`: while one runs, a second `astro preview` for the same root exits at once with *"Another astro preview server is already running"*, and the same `--ignore-lock`, on either of the two commands, runs both — while its `--force` replaces nothing in the foreground, the second exiting with that same message (Astro 7.3.2, run under Claude Code, and its `cli/preview` source). Without the flag on either, `/docs-serve` meets the second as a command that exited before its port answered, and quotes Astro's message from its log; `/document`'s render check, which boots one server at a time, never runs the two at once.
