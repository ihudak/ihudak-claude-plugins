...
```
choices: ["Ground the inventory against code and design — /product-workflows:brd-ground <BRD-KEY> (Recommended) <merge-clause>", "Stop here"]
```

**Zero `[BR#n]` rows (a Phase 3 `EMPTY` read) — `/product-workflows:brd-ground` is left out rather than
offered and refused:**
...
route yourself into `/product-workflows:brd-ground <BRD-KEY>`, even as the same person? → run
