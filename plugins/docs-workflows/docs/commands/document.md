...
- **A docs-profile** — loaded in-repo, from the built-in default (which applies only to a repo shaped like the bundled worked example), or generated on demand by an inline `/docs-profile` run for any other repo that has none; a custom repo whose profiling run is cancelled stops with `PROFILE_REQUIRED`. Where that run finds a profile already at the root of the work tree the docs repository sits in, and you keep it, the run proceeds with that profile as an in-repo one.
