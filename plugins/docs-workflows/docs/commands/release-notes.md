...
`/release-notes` is **address-required** — there is no free-text or `@file` input; a prompt with no positional address, or with a token that fails the key grammar, stops with `RELEASE_NOTES_NEEDS_KEY`. The address is a `<KEY>`, or an `@<path>` naming a folder in the specs tree; `workflows-core:addressing` §3 resolves either. A `<KEY>` is found only by searching `$SPECS_PATH`, so with that variable unset Phase 0 stops before resolving it, naming the variable (`choices: ["Set SPECS_PATH (enter the path)", "Cancel"]`); an `@<path>` needs no specs tree to resolve and runs on.
...
- **A resolved address** — a key or an `@<path>` naming a PRD or Epic folder in the specs tree; `mode: direct` is rejected outright. A key needs `$SPECS_PATH` set, since it is found by searching that tree: with the variable unset the run stops in Phase 0, naming it and offering to set it or cancel, before resolving anything. An Epic folder's PRD is read from the PRD folder above it, and the draft then covers that Epic's changes; `release-notes.md` is the PRD folder's either way.
...
**Where it lands.** `release-notes.md` in the resolved PRD folder, appended as a section: the Change Type selects `## Breaking changes`, `## Feature updates` or `## Fixes`, and those sit under a heading for the release version one level above them (`--version <v>`, else asked; `# Unreleased` when declined), each titled draft's `### title` sitting below its section. The three former destination *files* are those three sections — the taxonomy is unchanged, only where a draft lands. Each draft sits under an HTML comment naming the PRD's key, or the Epic's on an Epic address, the date, and every commit that run read, by repository; it renders as nothing and is not part of what you paste. It is how a later run with diff grounding on takes its boundary per scope, **by what an earlier note read rather than by any date**: from each implementation record it reads every block recording a commit no earlier note covering that record read — a note for the PRD covers every Epic's, a note for an Epic that Epic's alone — so a note drafted for one Epic does not hide another Epic's earlier work, and a branch committed before a note but merged after it is read rather than silently skipped, wherever that note records what it read. The commit scan beside the record keeps to the same boundary: it drops every commit a block names and every commit an earlier covering note read, and nothing else, save what the date fallback below drops. A note from before this release records no read set, so its record falls back to its date, and the run then lists what that fallback dropped beside the ones it used: every block, and every commit it dropped by the commit's own date. A commit dropped *with* a block recording it gets no line of its own — that block's listing accounts for it, naming the record and the heading date, the block itself naming its commits — so a commit you cannot find a line for is one of those rather than one the run lost.

**The comment records what the run read, never what it merely found.** Phase 3 finds the commits — off the record, and off commit *messages* in the scan — and opens no diff; the diffs are read in Phase 5, and the read set is settled there. So a repository you answer *Skip and continue without its refs* for, one no clone under `$REPOS_PATH` matched, and a ref `diff-summarizer` could not open all contribute nothing to the comment — including a ref it answered with its key-commit fallback, which runs precisely because that ref is not in the clone, as a squash-merged and deleted branch leaves it. The run reports every one under **Not read**, and the next grounded run reads them again rather than skipping them forever on the strength of a note that never opened them; on the key-commit route that means the same block comes back next release and is described again, which is work re-described rather than work lost.
...
**Light gate only.** There is no Opus review, no tests, and no branch created by this command — `specs-preflight`, and the switch back to a branch the first preflight left when you enter a slice key at the BRD-container stop, may move `$SPECS_PATH` between branches that already exist and were created by the plugin, but neither creates one. The one optional gate is a **style check** (Phase 7): when the user chose it, `prose-style-checker` runs against the rendered draft and, on the auto-fix choice, `prose-fixer` applies safe fixes. Both work on a scratch copy of the draft, so neither touches a section an earlier run appended, and the command names the specs repository to the checker as the repository whose rules apply: the draft is checked under the house-style overlay the specs repository keeps in `.prose-style/rules/`, as `release-notes.md` itself would be, rather than under the rules of the repository the session happens to stand in. The report's style line names the rule set applied. That input arrived in `prose-style` 0.4.0, which echoes it back; this plugin declares `prose-style` with no version range, and where an older checker runs its output carries no echo, and the report records the style check `DEGRADED`, saying the specs repository's rules may not have applied and that `prose-style` needs updating. Optional here means the user's own answer in Phase 1 — `prose-style` is a declared dependency, so the phase never skips itself for want of a plugin.

**The run has no worthiness gate, and there is no content state in which it refuses to draft.** One existed and was retired: it read a `relevant_for_release_notes` flag off the PRD and stopped on an explicit `false` or `no`. Every PRD is relevant for release notes, so the flag asked a question with one answer and the only value that changed anything was one nobody should write; the field is retired in `workflows-core:prd-format` and a value left in an existing PRD is read by nothing. The run refuses only on its address and on what resolving it needs: Phase 0 stops naming `$SPECS_PATH` where a `<KEY>` arrives with that variable unset, stops with `RELEASE_NOTES_NEEDS_KEY` where the prompt carries no positional address at all or one that fails the key grammar, stops naming every match where one resolves ambiguously, and raises the shared re-enter/cancel escalation where one resolves to nothing. An address that resolves but leads to no `prd.md` the run can read takes a named stop in Phase 0 step 1, after the specs-repo preflight has settled the specs checkout's branch and before the run asks anything or resolves documentation grounding, never the re-enter/cancel escalation an unresolved key gets, since re-entering a key that resolved cannot help. `RELEASE_NOTES_BRD_NOT_SLICED` stops on a BRD container, whose PRDs sit in its slices: it lists each slice, and offers `Enter a slice key` or `Cancel`, taking a typed key only where it is one of the keyed slices listed and re-asking otherwise, and running the specs-repo preflight again for the slice entered, so the slice is read on the branch a direct run on it would read; a container with no slice yet is a plain stop naming `/product-workflows:brd-split`. `RELEASE_NOTES_FOLDER_NOT_PLACED` stops on a folder the run cannot place at any level, naming what the folder carries and the remedy: give it a carrier, or, where it holds an `idea.md` and no `prd.md`, run `/product-workflows:create-prd`. `RELEASE_NOTES_NO_PRD` stops on a PRD folder with no `prd.md`, or with one the run cannot read, which it reports with the read error and names no command for. Its remedy is `/product-workflows:create-prd` on the idea route; on a BRD-route slice it follows the slice's ledger — `/product-workflows:brd-split` while a claimed row is still unallocated, a missing or unreadable `coverage-ledger.md` reported by path, the parent's keep-or-remove `/brd-split` for a slice claiming nothing, and no command for a slice that claims rows but covers none of them here, which holds no PRD of its own and so nothing to draft a release note from; for an Epic with no PRD folder above it, it names moving the Epic folder into one. A PRD folder that carries no key stops too (`RELEASE_NOTES_PRD_NO_KEY`), naming the two ways to give it one. Each of these stops is a user halt, so none is recorded as a plugin gap. A `prd.md` whose frontmatter asserts no `kind: prd` is still read as the PRD. Whether a note is worth drafting is the decision of whoever runs the command.
