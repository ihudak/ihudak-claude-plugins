...
- **Runs:** `/idea`, `/create-prd`, `/update-prd`; also `/prd-proposal` and `/brd-proposal`, the two optional effort proposals — one for a slice, one for the programme over it — which gate nothing on the build ladder and which nothing on it waits on; each does gate its own input — `/prd-proposal` the folder's `prd.md`, `/brd-proposal` each included slice's `proposal.md` — and that is the whole of what either **gates** on, and each command's only hard refusal on readiness (see [proposal](#proposal) below); both refuse on other things, an unresolvable address and an uncapturable profile among them; also the early run of the companion plugin's `/docs-workflows:release-notes`, before any specification or design exists yet.
...
- **Produces:** `idea.md` in the PRD folder the address names, then **prd.md** written to that same folder under `$SPECS_PATH/specifications/`; an early release-notes draft; and, optionally, **proposal.md** and its rationale brief in that same folder, or in the `BRD-` container above a set of priced slices.
...
- **Cost phase(s):** `prd-creation` (`/idea`, `/create-prd`), `prd-update` (`/update-prd`), `brd-to-prd` (`/brd-intake`, `/brd-split`, `/brd-interview`, `/brd-package`, `/brd-reconcile`), `proposal` (`/prd-proposal`, `/brd-proposal`) — all role `pm`.
...
This plugin's spine ends at `specification.md`, landed on the specs repo's default branch. The companion `dev-workflows` plugin's Dev role — `/dev-workflows:design`, `/dev-workflows:implement`, and `/dev-workflows:ready` — reads it from there, along with any ARD this plugin's PA role produced, and takes the pipeline the rest of the way to shipped code and product documentation. That role, its own cost phases (`planning`, `implementation`, `readiness`, plus `documenting` and `docs-scaffold` for the companion `docs-workflows` plugin's `/docs-workflows:document`, `/docs-workflows:docs-init` and standalone `/docs-workflows:docs-brand`), and why there is no separate verification role are documented on `dev-workflows`'s own Roles and phases page — this one covers only the three roles whose commands ship here.

## Cost-attribution phases

Every cost-emitting command tags its cost line with a `phase` and a `role`. Seven phases are reached by this plugin's fourteen commands; each entry below names the command that emits it and what being in that phase means. Five more lifecycle phases exist for the companion `dev-workflows` and `docs-workflows` plugins' own commands — three and two respectively — and are documented on their own pages, not restated here. Each of the seven below can also be reached **by inheritance**: the companion `workflows-core` plugin's `/workflows-core:prompt`, `/workflows-core:feedback`, `/workflows-core:prompt-brainstorm` and `/workflows-core:prompt-grill-me` adopt the phase and role of whatever they are correcting, so a correction to a `/specify` output is a second entry in `specification`.

### prd-creation

Emitted by `/idea` and `/create-prd`, role `pm`. From either, being in this phase means the PRD does not yet have a merged specification or design — the work underway is idea refinement or PRD authoring, and Epics may or may not exist yet. The companion `docs-workflows` plugin's `/docs-workflows:release-notes` also lands here, by inference, on a run where neither `specification.md` nor `design.md` exists under the PRD's specs directory. So does the companion `workflows-core` plugin's `/workflows-core:frames`, on a run whose resolved folder asserts `kind: prd` or `kind: epic` — an idea-route PRD folder, or any Epic folder, one under a BRD-route slice included — and there the phase marks product-definition spend and nothing about the folder's progress: a folder it indexes may already hold a merged specification or design.
...
Emitted by `/brd-intake`, `/brd-split`, `/brd-interview`, `/brd-package` and `/brd-reconcile`, role `pm`, and by `/prd-ground`, role `pa`, on either route it runs. On the BRD route, being in this phase means a customer-supplied BRD is somewhere on the BRD-to-PRD route — its requirement inventory is being extracted, split into slices, grounded against code and design, allocated, decided, packaged for customer review, or reconciled against the review that came back — whether or not a slice of it already holds its PRD, since a later round, package or reconciliation runs on such a slice all the same; every command on that route runs as PM except `/prd-ground`, which is PM-initiated but PA/Dev-executed. `/prd-ground` tags this same phase on the idea route too, where there is no BRD in the folder's ancestry at all: an existing PRD's own `[AC#n]`/`[FR#n]` rows are being ground against code and design instead, optionally and ungated, after `/create-prd`. `/workflows-core:frames` also lands here, by inference, on a run whose resolved folder asserts `kind: brd` — a BRD container, or a BRD-route slice, though the slice is a `PRD-` folder — and there too the phase marks spend on that route and nothing about the folder's progress: a slice it indexes may already hold its PRD.
...
Emitted by `/prd-proposal` and `/brd-proposal`, role `pm`. Being in this phase means a requirement set is being **priced** rather than advanced — work packages, hours by role, and a range with its evidence — for a document a vendor sends a customer. It is the one phase here that no other phase waits on, and nothing it gates lies downstream of it: both commands gate their own input and nothing else — `/prd-proposal` waits on the folder's `prd.md`, which is the `prd-creation` phase behind it, and `/brd-proposal` waits on each included slice's `proposal.md`, which is this same phase gating its own earlier run. A proposal is optional at every readiness tier, and no command of the build ladder reads one. The one command that reads **another folder's** proposal is `/brd-proposal`, which gates on each included slice's `proposal.md` in order to roll it into the programme umbrella — a second proposal rather than a phase of the build. Every other read is an own-folder one: a later run of `/brd-proposal` anchors its re-estimate on the umbrella it wrote last time, a later run of `/prd-proposal` does the same with the slice proposal it wrote, and `proposal-reviewer` opens the proposal it is handed inside the run that wrote it — reads, never gates. It is its own phase rather than part of `brd-to-prd` because `/prd-proposal` runs on the idea route as readily as on the BRD route.
