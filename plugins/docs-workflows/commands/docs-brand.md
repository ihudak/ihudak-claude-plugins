...
2. **Validate the resolved repo is a writeable git work tree.** `git -C <repo> rev-parse --is-inside-work-tree` must print `true`; if it errors or prints anything else, stop: `DOCS_BRAND_NOT_A_GIT_WORKTREE: <repo> is not inside a git work tree.` `test -w <repo>` must succeed; if not, stop: `DOCS_BRAND_REPO_NOT_WRITEABLE: <repo> is not writeable.` Resolve and record the git root as `<repo-root>` (`git -C <repo> rev-parse --show-toplevel`). Every git call in this run runs as `git -C <repo-root>`, and the profile is read there, since `<repo-root>/.dev-workflows/docs-profile.yml` is its one home (`${CLAUDE_PLUGIN_ROOT}/references/docs-profiles/docs-profile-schema.md`, **Where the profile lives**). **The site's own files are not placed relative to `<repo-root>`**: a site can sit below it — a monorepo's `site/` — so step 3 finds the MkDocs configs, and every path this run reads or writes in the site derives from where each config sits and from its `docs_dir`.
...
- ALWAYS find the MkDocs configs through the profile's `builds[]` where it records one, else in the resolved directory, and derive every path in the site — the configs followed through `INHERIT`, the stylesheet, the assets — from where each config sits and its `docs_dir`, never from the git top level, where the profile is read and every git call runs (Phase 0 steps 2–3)
