...
- **The merged `specification.md` itself**, gated on the specs repo's main branch. **This is the one gated input in the pipeline that genuinely stops on absence** (`workflows-core:phase-handoff` §3): every other producer's optional input falls back to prior behavior when absent and reports it, but here no specification anywhere is a hard stop — `no specification.md exists yet for this item — run /pm-workflows:specify for it and merge it to the specs repo main first.` An *unmerged* spec (an open pull request, not yet on main) also stops, naming the branch and any PR — the same stop every other gated input applies.
...
- `/pm-workflows:specify` — the upstream command, shipped in the companion `pm-workflows` plugin, whose merged `specification.md` `/design` requires before it will start.
