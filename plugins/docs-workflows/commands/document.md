...
- Each of the endings `render-verification.md` §2's **The endings** defines ends the whole smoke-check rather than one space's part of it → never blocks; every page not yet checked falls back to the manual table, and `render_smoke_check` records `DEGRADED` (Ledger (final)). **What each ending records, and what that record names, is that list's to say** — including which of them name a command and a log and which names neither. Write the record it gives, and nothing it does not.
