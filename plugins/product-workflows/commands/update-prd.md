---
name: update-prd
description: PRD-update workflow (PM phase) — refresh/re-do an existing Product Requirements Document. Resolves the PRD from the specs tree (the only current copy — earlier ones are archived under revisions/), grounds on the PRD + any ARD/spec/transcript/grounding findings, updates it via a relentless grill against workflows-core:prd-format, gated by the Opus prd-reviewer, and writes canonical + archived revisions into $SPECS_PATH/specifications/PRD-<KEY>-<slug>/. Reads grounding/code-grounding.md and grounding/design-grounding.md when present and stamps consumed_by: PRD on each finding it draws on — the first writer of that value onto a grounding finding (create-prd already writes it onto a decisions.md record, never inside a grounding file). Product-level (no code scan).
...
4. **Resolve the base PRD.** It is the `prd.md` in the folder step 3 resolved — the only current copy there is (earlier revisions are archived under `revisions/` and never read as the base), and therefore authoritative without a test.
   ...
   **`/create-prd` is named only where that command can itself run.** It refuses **three** BRD-route
   shapes, not one (`${CLAUDE_PLUGIN_ROOT}/references/coverage-ledger-format.md` §5.2, the authority,
   not restated here), and this step has cleared **none** of them. (It refuses an Epic folder too,
   with `CREATE_PRD_EPIC_FOLDER`; step 3a has already stopped on that one, so no folder this table
   reads is an Epic.) `/update-prd` takes no container
   ...
   |---|---|
   | A `BRD-` container — the `BRD-` prefix, or on an unprefixed legacy folder §5.1's positive test (`coverage-ledger.md` or `brd/brd-inventory.md` present, and no `brd-link.md` naming a `parent:`) | **Not** `/create-prd`, which refuses a container in turn. Name the `PRD-` slices under it, one PRD each — enumerated by `/brd-split` Phase 0 step 9's positive test (an immediate subdirectory whose `brd-link.md` `parent:` names this BRD), never by a name match — and offer `/product-workflows:create-prd <SLICE-KEY>` once per slice, as the statement that a PRD belongs there rather than a promise that the slice is already eligible. Where the container holds no slice, `/product-workflows:brd-split <BRD-KEY> "<how to cut it>"` is the run that carves one, and it is a **no-op** on a ledger with no `unallocated` row (`commands/brd-split.md` Phase 0 step 10), so say what the operator does then rather than leaving the offer to fail silently |
   | No `brd-link.md` — an idea-route `PRD-` folder, or its legacy unprefixed form, which carries neither §5.1 file | `/product-workflows:create-prd <ADDRESS>`. There is no gate set, so neither data refusal exists for it and the container test is the only one there was — which this row has passed |
   ...
   | A `brd-link.md`; the gate set leaves **no** row `unallocated` **and** at least one `covered-here` | `/product-workflows:create-prd <ADDRESS>` — all three refusals cleared |
   | A `brd-link.md`; a gate-set row is still `unallocated` | **Not** `/create-prd`, which raises `CREATE_PRD_BRD_UNALLOCATED`. Name `/product-workflows:brd-split <SLICE-KEY>`, whose walk moves exactly those rows and which on a slice runs allocate-only — and say beside it that its own Phase 0 gates on this slice's grounding findings each carrying a verifier verdict and stops naming `/product-workflows:prd-ground <SLICE-KEY>` when they do not |
   | A `brd-link.md`; no gate-set row `covered-here`, and the gate set is **empty** | **Not** `/create-prd`, which raises `CREATE_PRD_BRD_NOT_ELIGIBLE`. This is a standing empty child: name the keep-or-remove `/product-workflows:brd-split <PARENT-KEY>`, the one run that resolves one and not a no-op there. `<PARENT-KEY>` is **read** off the same `brd-link.md` the `claims:` list came from (§5.2), never parsed out of the slice's own key or folder name. **Which form to name is the parent's own ledger's to decide**, exactly as `/product-workflows:create-prd` Phase 0 step 7's empty-gate-set row decides it: where that ledger still holds an `unallocated` row, name `/product-workflows:brd-split <PARENT-KEY> "<how to cut it>"`, since that run walks the row too and stops with `BRD_SPLIT_NEEDS_INSTRUCTION` without an instruction; where it holds none, the bare form; where it cannot be read, report it by path and name neither form |
...
1. **Confirm** the feature folder; the resolved PRD base; and the secondary artifacts discovered (specs draft / ARD / spec / transcript).
   - **Resolve documentation grounding here, then show its line.** Run `resolve-docs-grounding update-prd` per `Skill(skill: "workflows-core:reference", args: "docs-grounding resolve-docs-grounding")` — its step 3.5 index prompt included — and show the `docs grounding:` line from what it returns, in the form that reference fixes — `ON <root> (retrieval: …)` or `OFF (<reason>)` — verbatim, including any index-build, staleness, or shadowing clause it carries (off switch: --no-docs). It runs here, before any agent is dispatched, because step 3.5 asks its one-time index question before the run's real work; this is the run's one resolution (`workflows-core:docs-grounding`, *Invariants*), and Phase 2 dispatches on the state it returns without resolving again.
   - Report any discovered `ard.md`, `specification.md`, `grounding/code-grounding.md` or `grounding/design-grounding.md` that is not on the specs repo's default branch — per Phase 0 step 5, this grounding is unapproved but advisory-only; it is never a reason to stop the run.
...
Then dispatch on the documentation grounding Phase 1 resolved, resolving nothing again. Where it resolved `docs_grounding: ON`, `dispatch-docs-grounder` (`workflows-core:docs-grounding`) with `feature_summary` = the PRD goal + the change signal from comments, `key` = `<KEY>`. Carry the digest into the Phase 3 grill with **grill-rank** consumption. Where it resolved OFF, dispatch nothing.

Where Phase 0 step 5 discovered `grounding/code-grounding.md` and/or `grounding/design-grounding.md`, read every finding on file. A finding with no verifier `outcome` is not evidence and grounds nothing (`workflows-core:grounding-format` §8) — carry only the findings that hold one. Carry those into the Phase 3 grill with the same **grill-rank** consumption the docs digest above uses, ranking them into the Impact × Uncertainty gap list rather than appending. The highest-value case: a `[CG#n]` whose verdict says an `[AC#n]` the PRD asks for is already satisfied at the pinned commit is scope this update can drop or narrow, not a defect to raise. Where neither file was discovered, skip silently.

---

## Phase 3 — Update via grill

**Interview technique (grilling — embedded).** Conduct a **relentless** interview per `Skill(skill: "workflows-core:reference", args: "grilling-technique")` — one question at a time, recommend each answer, fact-vs-decision split, dependency order.

**Archive the base before the first write.** Immediately before this run's first write to `<feature-folder>/prd.md` — ordinarily the first live edit below, and wherever it falls — copy the current `prd.md`, the base Phase 2 read, to `<feature-folder>/revisions/<KEY>_<slug>_<YYYYMMDD>.md`. **`<slug>`** is the resolved folder's own name less its `<KIND>-<KEY>-` prefix (`workflows-core:addressing` §2), or less `<KEY>-` on a legacy unprefixed folder (§5) — the key is one this run already holds, so this strips a known string rather than parsing one — and where nothing remains the name is `<KEY>_<YYYYMMDD>.md`. **Where that name is already taken by any file in `revisions/`**, whoever wrote it, use the first name not taken of that name with `-2`, `-3`, and so on inserted before `.md` — which holds for both name forms, so an empty slug gives `<KEY>_<YYYYMMDD>-2.md`. The archive is taken **once per run**: every later write in this run — Phase 3.5's and 3.6's inline fixes, Phase 4's BLOCKER fixes, Phase 5's frontmatter — is a write to this run's own draft and archives nothing. **Where the copy fails**, stop before writing anything, naming the path and the error:
```
UPDATE_PRD_ARCHIVE_FAILED: could not archive <feature-folder>/prd.md to <archive path> (<error>). Nothing was written — /update-prd never overwrites a PRD it has not archived. Make the file readable and revisions/ writable, then re-run '/product-workflows:update-prd <KEY>'.
```
`/product-workflows:create-prd` archives an existing `prd.md` it is about to overwrite by this same rule, and cites it here.

Update the PRD live against `Skill(skill: "workflows-core:reference", args: "prd-format")`, applying the no-hard-wrap prose convention in `Skill(skill: "workflows-core:reference", args: "prose-formatting")`, **diffing against the base** rather than authoring from blank: surface what changed and why (drawing on comments / ARD / spec / transcript), resolve open questions, keep the PRD product-level. Apply the **self-consistency check** — no `[AC#N]` delivering an Out-of-scope behaviour, no `## Goal` vs `## Scope` contradiction, no conflicting `[US#N]`; record a deliberately-kept tension under `## Assumptions & open questions`. Preserve the frontmatter provenance fields (`sources`, `derived_from`, `seeded_from_prd` if present) — **and, on a PRD that carries them, `brd_key`, `brd_parent` and `depends_on`**, copied through the refresh unchanged. Those three are BRD provenance `/create-prd` on the BRD route wrote from the slice's own `brd-link.md` (`workflows-core:prd-format`), and this command reads no BRD tree for content — its only reads of a slice's `brd-link.md` and `coverage-ledger.md` word Phase 0's stops (step 4's table, which step 3a also applies to the folder above an Epic) and carry nothing into the PRD — so a value it drops is one nothing later can restore. **`brd_parent` travels with `brd_key`**: that route resolves a `PRD-` slice folder and refuses a `BRD-` container, and a slice always has a `parent:`, so a PRD arriving with `brd_key` and no `brd_parent` is one `prd-reviewer` raises as a finding rather than one this run should complete — preserve what is there and author neither. **No command of the build ladder reads any of the three** — neither `/product-workflows:epics` nor `/dev-workflows:ready` does. `depends_on` has one reader: `/product-workflows:brd-proposal`, which takes the programme umbrella's slice order from it (`${CLAUDE_PLUGIN_ROOT}/references/proposal-format.md` §14, *Sequencing*), so a value this run drops removes an edge from that ordering. `brd_parent` and `brd_key` are acted on by no command; `prd-reviewer` raises a finding when one appears without the other — an integrity check on the pair, never a consumer of what they record. Preserving all three is right regardless: provenance that survives an update is the precondition for any later reader, and re-deriving it later would mean re-reading a BRD tree that may have moved on. Dropping them would silently make a slice's PRD look like an ordinary one, with nothing left to restore it from. Carrying an existing value forward is **preservation, not authoring** — this run mints none of the three, asks the PM for none of them, and writes none onto a PRD that arrived without them, so `workflows-core:prd-format`'s rule that they are written only by `/create-prd` on the BRD route holds unchanged. `key` is likewise carried, not re-derived. `/create-prd` writes it on both routes, set to the resolved folder's own key (`workflows-core:prd-format`), so a PRD reaching this command already carries one and this run has nothing to mint: on the BRD route that value is the `PRD-` slice's key, which is the folder this PRD lives in, and a PRD arriving with `brd_key` and no `key` at all is a finding `prd-reviewer` raises rather than a gap for this run to fill.
...
Run the deterministic checks in `Skill(skill: "workflows-core:reference", args: "pre-lint")` (the Universal checks + the **key-collision** check on the body below the frontmatter + the **PRD** block) against the updated file; inline-fix mechanical findings; leave content gaps for the grill. Advisory — never blocks; `prd-reviewer` remains the gate.
...
1. **The archived snapshot is already on disk**: Phase 3 took it immediately before this run's first write to `prd.md` (*Archive the base before the first write*). Where no earlier phase wrote `prd.md` at all, take it now, by the same rule, before step 2 writes — never a second time in one run.
2. **Write the refreshed PRD** to the **canonical** path `<feature-folder>/prd.md`. Record `revision_of: <the archived snapshot path actually written, suffix included>` and `built_from_date: <YYYY-MM-DD>` (the date the update was built from) in the frontmatter.
3. **Close the consumption loop.** Where Phase 0 step 5 discovered `grounding/code-grounding.md` and/or `grounding/design-grounding.md`, set `consumed_by: PRD` on each `[CG#n]`/`[DG#n]` finding **this update actually drew on** — the same write `/create-ard` and `/specify` already make at their own altitudes, and the first time `PRD` has been written into that field on a **grounding finding**: `/create-prd` already writes `consumed_by: PRD` — the identical field, values and starting-at-`none` rule (`${CLAUDE_PLUGIN_ROOT}/references/decision-register-format.md` §1) — onto a `decisions.md` decision record on the BRD route, but never inside `grounding/code-grounding.md` or `grounding/design-grounding.md`, since it reads no `grounding/` file on either route. A finding read for context and not used to change the PRD is still `none`; marking it consumed would report a routing that never happened. A finding with no verifier `outcome` is never marked, whatever the grill did with the claim, because it was never evidence (`workflows-core:grounding-format` §8). Nothing else in a finding record is touched — no `verdict`, no `evidence`, no `id`, no renumbering — and this is the **only** write this command makes into either grounding file.
4. **Hand off** (commit-when-asked — never automatic). Invoke `Skill(skill: "workflows-core:reference", args: "phase-handoff")` and present its §4.3 consent choice verbatim: `choices: ["Branch + commit + push + open PR to main (Recommended)", "Just write the files — I'll handle git (the next phase will stop until this is on main)", "Cancel"]`. On the first choice, execute `handoff-to-main` (`Skill(skill: "workflows-core:reference", args: "phase-handoff handoff-to-main")`, §2) with `prefix: prd`; `feature_folder` as resolved in Phase 0; `deliverable_paths` = the canonical PRD file (step 2), the archived snapshot file Phase 3 wrote (step 1), and, where Phase 0 step 5 discovered them, `grounding/code-grounding.md` and `grounding/design-grounding.md` — because the `consumed_by` write above (step 3) lands in those two and an uncommitted consumption record is one no later run can read; `title: <KEY> Update Product Requirements Document`; and `body_facts` = which sections changed, the date the update was built from, the open-question count, the `prd-reviewer` verdict, and how many grounding findings were marked `consumed_by: PRD`. Emit its §4.1 outcome line in the Final report.
...
An updated PRD can invalidate what was derived from it. Where this run can go next:
  • Re-run the spec         — /product-workflows:specify <KEY> <merge-clause>    (PE, if one exists)
  • Re-run architecture     — /product-workflows:create-ard <KEY> <merge-clause> (PA, if one exists)
  • Re-run epics            — /product-workflows:epics <KEY>                     (PE)
...
The resume pointer is written in the terminal cost phase (Phase 7), per `workflows-core:session-hygiene` §1. Then: continuing as PM → `/compact`; handing to PA/PE → `/clear`. Guidance only.
...
1. **Invoke `impl-maintenance`** (subagent_type: "workflows-core:impl-maintenance", model: `<detection_model — §2.1 Sonnet chain>`) with a compact handoff: command `/update-prd`; what was updated (which sections changed + why); key events (BLOCK reviews, unresolved clarifications — or 'none'); workarounds; the `prd-reviewer` verdict; test result N/A; project root = the feature folder.
...
3. **Session cost (ALWAYS runs).** Invoke `Skill(skill: "workflows-core:reference", args: "cost-emission emit-cost")` and call `emit-cost` with `command: /update-prd`, `phase: prd-update`, `role: pm`, the run's `key`, `source`, and `plugin_version`. Surface the persisted path (or the report-only notice).
...
Report: the canonical PRD path + the archived snapshot path; which sections changed; the base `prd.md` the update was built from; open-question count; the `prd-reviewer` verdict; the prose style-check outcome; the `Phase handoff:` outcome line from `handoff-to-main` (`workflows-core:phase-handoff` §4.1); the handoff reminder; resolved model routing (+ any Opus degradation); the feedback + cost paths; the `Specs repo:` outcome line from `commit-artifacts` (`workflows-core:specs-repo-git` §6), with any guard notice repeated in full; and the next-step recommendations.

**Grounding consumption.** Where Phase 0 step 5 discovered `grounding/code-grounding.md` and/or `grounding/design-grounding.md`, report every product-altitude `[CG#n]`/`[DG#n]` finding still `consumed_by: none` after Phase 5 step 3, by id — an explicit "none" where there are none, so an empty list reads as an empty set rather than as an unrun check — **excluding the baseline `[CG#n]` findings**, which are never `consumed_by` anything and whose `none` therefore reports no gap (`workflows-core:grounding-format` §4.1); say that they are excluded, so a reader can tell an empty list from an unrun check. **Where the resolved folder carries `brd-link.md`, additionally exclude every `[CG#n]`/`[DG#n]` whose `claim` cites a `[BR#n]` that folder's own `coverage-ledger.md` now shows as `covered-by`**, read off that file's `disposition` column and never off `claims:` or the inventory (`${CLAUDE_PLUGIN_ROOT}/references/coverage-ledger-format.md` §3.1's trap, named there from the other side) — the BRD that row names *owns* the requirement, so nothing this update authors can consume a finding about it and its `none` reports no gap either; say how many were excluded on this count too, and note that this is a **report exclusion only** — no finding edited, renumbered, moved or marked, and no `consumed_by` value changed anywhere by this exclusion. On a folder with no `brd-link.md` — the ordinary idea-route case, and the one this task exists for — this second exclusion has no subject: its claims are `[AC#n]`/`[FR#n]`/`[US#n]`, never a `[BR#n]`, and there is no `coverage-ledger.md` in the folder to read, so report zero excluded under it rather than omitting the count. **Where Phase 0 step 5 found no `grounding/` file at all, say that instead of an unconsumed count** — a folder that was never ground is not a folder whose findings went unconsumed.
