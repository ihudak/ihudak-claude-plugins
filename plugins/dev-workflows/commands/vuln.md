...
- `RUN_FAILED` or `COMMAND_NOT_FOUND` → **nothing was captured, and what to do about it is the operator's decision rather than this command's.** Surface the block's `### Suites` rows and its `### Notes` lines first — a hint has an answer to give wherever a note names the runner's own command — then ask:
  ...
  - **Apply the fixes unverified** → record `baseline_unverified` and work every CVE. Each finishes `clean_finish: false` at Step 3.9, so each pull request is a draft the banner says not to merge.
  - **Cancel this run** → stop here. Nothing has been branched and nothing edited, so there is nothing to hand off and no CVE is reported worked.
...
- `clean_finish` — `false` when the CVE ended `BLOCKED` or `TESTS_NOT_RUN`, when its review is still `BLOCK`, when the user chose `keep-anyway` on a regression, or when the fixer's `notes` names a **New failure** its verify reported; `true` otherwise. **That last condition is not a status test and cannot be made one**: a New failure moves no `test-baseliner` `Status`, so the CVE reaches here as `SUCCESS` with a red suite behind it (`dev-workflows:test-baseliner` verify step 6, and `vuln-fixer` step 5, which records the list in `notes` for exactly this read). It is the mildest of the five — nothing says the fix caused it — and it still withholds an ordinary mergeable pull request, because what the flag asks is whether this diff is safe to merge unexamined and a failing suite answers no. Per §2.9 the flag never suppresses either half: the commit is prompt-free and the push is still **offered** under §2.4's choice, and what the flag itself changes is only the pull request (draft, DO-NOT-MERGE banner). What it can change is whether that choice is put again — the paragraph below, which is where this command differs from `/implement`.
