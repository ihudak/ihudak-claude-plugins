...
Round 3 of the exclusivity probe deferred eleven decision-cycle defects to a design pass of their own. They are listed in `docs/superpowers/verification/2026-09-23-exclusivity-probe-wider-vocabulary.md`, Round 3, under "Deferred by user decision". Tracing them turned up six more defects of the same kind (N1–N6 in §3), and implementation found twenty-five more (N7–N31). This spec fixes all forty-two.
...
| N30 | C | A `--rebaseline` run that stops in Phase 5 or 7 has already recorded the new pin, so the re-run never supersedes the moved repository's old-pin findings, which stay live. Found by the final fix wave. Ruling R74: the new pin is recorded only in Phase 8's write, with the findings it pins |
| N31 | A | A held record resting on a design divergence that an agreeing re-run retired (R66) waits for a successor that will never come. Found by the final fix wave. Ruling R75: as R19, a held record is put again where a superseded finding of its has no successor and its frame set was re-ground in that run, so none will come |
...
- **R52 (revises R50's placement).** The archive is taken **immediately before this run's first write to `<feature-folder>/prd.md`**, wherever that write falls. Phase 3 authors the draft live, so that is where it usually happens. The archive is taken once per run and never re-taken for the run's own draft.
- **R53 (N24).** `/update-prd` archives on the same rule: immediately before its first write to `prd.md`.
- **R54.** In `/create-prd`, an unreadable `prd.md` stops at Phase 1 step 2 with a named stop, before any grill or write. A failed archive copy stops with a named stop that Phase 7 classifies.
- **R51, extended.** The first-free rule states where the suffix goes. `revision_of:` records the name actually written. `proposal-format.md`'s same-day revision wording takes the same rule. Every writer into `revisions/` is named in addressing's `revisions/` register, and the archive gets its `phase-handoff` §4.0 register row.
...
### E2a. Rulings from Task 8's review

- **R55 (revises E2's site).** The `<CMD>_NO_PRD` test moves to Phase 0 step 1, where everything it tests is already known. It runs before any docs-repo commit, preflight, grounding consent or question. That includes the Epic variant.
- **R56.** On a BRD-route slice, `<CMD>_NO_PRD`'s remedy is qualified by the slice's state: `unallocated` rows lead to `/brd-split <KEY>`; no `covered-here` row gets its own remedy; a missing ledger gets its own remedy too. This mirrors `EPICS_NO_PRD`'s remedy table.
- **R57 (N25).** `/release-notes` and `/ready` stop on `status: invalid` with their own named stop, as their siblings do (`*_NEEDS_KEY`). The `key dir not found` rule's grammar-failure clause, which then has no caller, is deleted.
...
- The verification record `docs/superpowers/verification/2026-09-23-decision-cycle-lifecycle.md` is written last, after the final fix wave. It closes each §3 row as FIXED with the commit that fixed it. Only once all forty-two rows read FIXED is the known-bug count 0.
