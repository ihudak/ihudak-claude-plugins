...
- **In `product-workflows`.** The PRD → architecture → Epic breakdown → specification ladder this plugin's spine picks up from, and the six-command BRD-to-PRD route that feeds it — `/product-workflows:idea`, `/product-workflows:create-prd`, `/product-workflows:update-prd`, `/product-workflows:create-ard`, `/product-workflows:epics`, `/product-workflows:specify`, the route's own `/product-workflows:brd-intake`, `/product-workflows:brd-split`, `/product-workflows:prd-ground`, `/product-workflows:brd-interview`, `/product-workflows:brd-package` and `/product-workflows:brd-reconcile`, and its two effort-proposal commands `/product-workflows:prd-proposal` and `/product-workflows:brd-proposal`, which price a requirement set in hours and gate nothing.
