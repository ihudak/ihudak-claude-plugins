...
Convention for this list: **every handoff field this agent acts on carries a matching `NEVER …` rule here.** A bullet that reads wider than the writing brief — the `existing_image_decisions` pair below (never touch a declined or unlisted occurrence; never swap without the stale check) among them — is in-brief by that convention and stays; it is the rule that keeps its input field honest.
