...
| `readiness-reviewer` | opus | Read, Glob, Grep | Cross-artifact readiness verifier — checks the ARD/spec/design justify the declared status and the next transition; the only reviewer that does joint cross-artifact analysis. | `/ready` |
...
| `idea-reader` | per routing | Read, Glob, Grep | Ingests one idea source — an inline prompt, a markdown file, a community post, or a saved file — and returns a structured source digest for `/idea`. | `/idea` |
...
Every one of the 38 agents above is dispatched by at least one command. Two of them, `docs-grounder` and `vault-prior-art-finder`, are dispatched indirectly: their calling commands invoke a named procedure (`dispatch-docs-grounder`, `dispatch-prior-art-finder`) defined in the agent's own governing reference file rather than writing `subagent_type: "dev-workflows:<name>"` inline, but that procedure resolves to exactly the `subagent_type` named above, so the derivation counts it as a real dispatch.
