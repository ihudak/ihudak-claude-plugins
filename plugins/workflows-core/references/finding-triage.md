...
| `docs-scaffold-reviewer` → the orchestrator itself (`/docs-init`, and `/docs-brand` standalone) | yes |
...
**Where there is no fixer, the orchestrator is the fixer, and triage still runs.** `docs-scaffold-reviewer` and `proposal-reviewer` are Opus-pinned and their findings are reasoned claims, but neither has a dedicated fixer agent behind it: `/docs-init` and a standalone `/docs-brand` apply the survivors themselves, by direct edit, because the scaffold diff has none (docs-workflows design D25), and `/prd-proposal` and `/brd-proposal` fix surviving BLOCKERs inline, editing both proposal artifacts themselves, because a proposal has no delegated writer. That makes the station argument above *stronger*, not moot — the dismissal and the edit now sit in one place, so nothing downstream would catch a finding dropped without a reason. Everything below applies unchanged with "the fixer" read as "the orchestrator's own edit": survivors only, the patch gate binds that edit, and an emptied survivor set is settled by the user rather than silently promoted. An `--inline` `/docs-brand` run dispatches no review of its own; its diff is triaged inside `/docs-init`'s.
