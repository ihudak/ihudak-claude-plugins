...
- `toolchain-preflight.md` — the environment check `/document` runs at Phase 0, deriving the required tool set from the resolved profile, the repo's config signals and the repo's own documented prerequisites; the tool→gate map; and the prompt that fires only when a required tool is missing. `/docs-init` runs the same check at its Phase 2 but derives nothing — there is no profile yet — and hands it a fixed set of its own. `/docs-serve` runs no preflight, but tests its own tools with the file's test for a binary — not a bare `command -v`, which an alias or a shell function named after the tool passes; `/document`'s render check uses it too — and takes the file's definition of a command's tool, with that test, before it starts a server. The file also defines, once, the five names Vale reads its configuration from and how the plugin runs Vale over one — a lint and `vale sync` alike, on the repository's configuration with no global Vale configuration and no `VALE_CONFIG_PATH`, as a clean CI runner has neither, and with the limit it states for a configuration that sets no `StylesPath`, which keeps the machine's shared default styles directory — which `docs-style-checker`, `docs-scaffold-reviewer` and `/docs-init` cite.
