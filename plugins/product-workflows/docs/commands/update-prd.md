...
- **The PRD**, at the resolved folder's `prd.md` — the only current copy there is (earlier revisions are archived under `revisions/` and never read as the base), and therefore the base without a freshness test.
...
- An **archived snapshot** of the prior canonical PRD, copied immediately before the run's first write to **prd.md** — the grill edits that file live, so this is taken in Phase 3, once — to `<feature-folder>/revisions/<KEY>_<slug>_<YYYYMMDD>.md`, `<slug>` being the folder's own name less its `<KIND>-<KEY>-` prefix. Where that name is already taken by any file, the first free `…_<YYYYMMDD>-2.md`, `-3`, … is used, and `revision_of` records the name actually written. Where the copy fails, the run stops with `UPDATE_PRD_ARCHIVE_FAILED` before writing anything.
