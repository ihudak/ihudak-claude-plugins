...
**Diagram labels are derived from the command's own top-level phase headings, verbatim.** Two heading dialects exist on disk and both are honoured as written: `## Phase N — <title>` in eighteen commands, and `## Step N — <title>` in `/vuln`. The remaining two — the standalone reviewers — carry no top-level headings at all, which is consistent: neither qualifies for a diagram. Derived heading counts for the thirteen commands that get a diagram, which the plan will use as each page's expected node budget: `document` 37, `epics` 20, `implement` 17, `release-notes` 14, `specify` 13, `design` 13, `create-vi` 12, `create-ard` 11, `update-vi` 11, `ready` 11, `idea` 9, `vuln` 5 (steps), `upgrade` 3. A diagram may collapse consecutive phases into one node where they form a single user-visible step, and it may carry **decision nodes** in addition to phase nodes — but every label must come from the command, never from invention. A phase node's label comes from its heading; a decision node's label comes from the command's own branching construct (its classification step, a `choices:` gate, a documented if/else). The `/implement` diagram is the precedent and the reason this is stated explicitly: it carries `C{"Classify complexity (model-routing)"}` and `G{"SIGNIFICANT · HIGH-RISK?"}`, neither backed by a `## Phase N` heading, and the plan requires it moved **verbatim**. A rule forbidding un-headed nodes would contradict that instruction the moment Task 10 ran. A command whose real structure forks — `/document`'s two modes, `/design`'s three-take fan-out — is diagrammed with that fork visible; a linear chain of phase headings is a numbered list and gains nothing from being drawn.
