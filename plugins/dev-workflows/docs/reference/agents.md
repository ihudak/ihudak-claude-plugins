...
| Agent | Model | Tools | What it does | Used by |
|---|---|---|---|---|
| `brd-reader` | sonnet (pinned) | Read, Glob, Grep | Extracts a `[BR#n]` inventory from a customer-supplied BRD — `source_anchor` per row, unconfirmed `defect_candidates`; splits multi-obligation requirements. Never rewrites the source. | *(none yet — the `/brd-*` command family, starting with `/brd-intake`, is landing across later tasks in this increment)* |
...
Every one of the 34 agents above is dispatched by at least one command, with one forward-declared exception: `brd-reader` has no **Used by** entry yet because the `/brd-*` command family that will dispatch it — starting with `/brd-intake` — has not landed in this increment yet; it is added ahead of its callers the same way `references/brd-format.md` and its sibling BRD references were. Of the remaining 33, two, `docs-grounder` and `vault-prior-art-finder`, are dispatched indirectly: their calling commands invoke a named procedure (`dispatch-docs-grounder`, `dispatch-prior-art-finder`) defined in the agent's own governing reference file rather than writing `subagent_type: "dev-workflows:<name>"` inline, but that procedure resolves to exactly the `subagent_type` named above, so the derivation counts it as a real dispatch.
