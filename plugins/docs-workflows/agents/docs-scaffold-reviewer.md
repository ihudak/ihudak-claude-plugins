...
Invoked from `/docs-init` Phase 7.5 and `/docs-brand` §7.4, over the written scaffold diff. There is no dedicated fixer agent for this diff: the caller triages this reviewer's findings per `workflows-core:finding-triage` and applies the surviving findings itself in the orchestrator, and a survivor that fails the patch gate is surfaced for a human decision rather than patched. This agent returns findings only. A `BLOCK` verdict means "fix the blocking issue before the scaffold is considered finished".

**Why the checklist is a fixed list rather than the reviewer's own judgement.** Six of the seven dimensions below assert a relationship *between two files* — the public config against the internal one, a config against the CI workflow that runs it, a workflow step against the profile field that parameterises it. A reviewer reading one diff hunk at a time sees each file in isolation and has no occasion to notice that a relationship it depends on broke; the list exists so the cross-file check happens regardless of which hunk is in front of the model at the time. A scaffold also has no specification to conform to and no test baseline to compare against, which is why this reviewer is narrow and concrete rather than a general code reviewer running mostly-inert dimensions.

## Inputs

The caller passes a structured brief:

- **Task description** — one-paragraph summary of which command dispatched this review (`/docs-init` Phase 7.5, or `/docs-brand` §7.4 either standalone or `--inline` from `/docs-init`, in which case its diff is contributed to that command's own Phase 7.5 review rather than reviewed a second time).
- **Written file paths** — absolute paths of every scaffold file the run wrote or modified. On a full `/docs-init` run this is `mkdocs.yml`, `mkdocs.internal.yml`, `.vale.ini`, `styles/config/vocabularies/Project/accept.txt`, `requirements-docs.txt`, `.github/workflows/docs.yml`, and `stylesheets/extra.css`. On a `/docs-brand` diff this is the `theme:`/`extra_css` edits to the two mkdocs configs, `stylesheets/extra.css`, and the copied logo/favicon assets under `docs/assets/`.
- **`docs_tree`** — the full list of paths under `docs/` as actually written, from a `Glob` over the resolved repository. This is the ground truth for dimension 1 (navigation completeness) and dimension 7 (public-config leakage); a generated `nav:` is never checked against its own claim of being generated.
...
6. `Grep` the public config, `mkdocs.yml`, for the leakage class dimension 7 checks.
...
| 4 | Vale configuration | `.vale.ini` parses and every style it names resolves (`vale ls-config`); `accept.txt` exists beside it, even where it holds only the placeholder comment. |
...
| 7 | Public-config leakage | No `internal/` path, internal hostname, or the literal `docs-visibility: internal` marker string appears anywhere in `mkdocs.yml`. |
...
- NEVER flag `exclude_docs` differing between the two mkdocs configs, or `_snippets/` being excluded from both, as a mistake — `scaffold-tree.md` §5/§6 pairs both deliberately; the defect is a *missing* exclusion or a divergent content source, never the exclusion itself.
