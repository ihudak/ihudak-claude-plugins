...
9. **Enumerate existing children — `split_mode: full` only.** In `allocate-only` mode there are no
   ...
   **A positive test, not a name match.** Matching a subdirectory by name and then reading an
   ...
   list with the file it is missing, as Step 2 names a child it bars for an orphan row. A row moved
...
- **A child Phase 0 step 9 marked unreconcilable** — its `brd/brd-inventory.md` or `coverage-ledger.md` missing or unreadable. Write nothing into it and report it by name, with the file it is missing. A missing file was either never written or lost after it was written, and re-seeding a lost ledger would write `unallocated` over whatever that child's own walk had settled. No walk of this run gave it a row, since it is no target and no donor (Phase 0 step 9); a row it claims that this walk settled elsewhere keeps its `claims:` entry here, a state `/product-workflows:prd-ground` reads off this ledger rather than off that list. Name `/product-workflows:prd-ground <CHILD-KEY>` as the command whose stops tell those states apart and name the repair — `PRD_GROUND_NO_INVENTORY`, `PRD_GROUND_NEEDS_SPLIT` and `PRD_GROUND_RESTORE_FROM_DEFAULT` among them.
