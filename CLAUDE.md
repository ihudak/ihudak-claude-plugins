...
`/brd-split`) must load and follow this file at the
...
`plugins/dev-workflows/references/specs-repo-git.md` is the **single source of truth** for the two git entry points the plugin runs against `$SPECS_PATH` — `specs-preflight` (run start: flush leftover artifacts, retry an unpushed artifact commit, settle the branch) and `commit-artifacts` (terminal: stage the bounded artifact paths, commit, push). It owns the bounded write authority (three path shapes; `^(idea|prd|ard|spec|design|ready|brd)/` branches only — seven prefixes, one per phase-handoff producer role except `brd`, which is shared by all three commands of the BRD-to-PRD route), the three guards and their four-part notice contract, the branch-disposition table, and the `Specs repo:` outcome line. It bounds the plugin's **bookkeeping** paths only — a phase's **deliverable** (PRD, ARD, `specification.md`, `design.md`, `idea.md`, `_readiness.md`) is `references/phase-handoff.md`'s to commit, push, and hand off. Consumed by the twenty commands that write into `$SPECS_PATH` — every one of the twenty-four except `/api-guideline-reviewer`, `/guideline-reviewer`, `/statusline`, and `/docs-profile` (count it by `grep -l commit-artifacts plugins/dev-workflows/commands/*.md`, which returns the same twenty). Hard rules: every git call is `git -C "$SPECS_PATH"` and never a `cd`; `git add -A` is never issued at repository scope; never force-push, never `branch -D`, never merge/rebase/reset, never delete an `index.lock`; never fatal.

`plugins/dev-workflows/references/phase-handoff.md` is the **single source of truth** for the two phase-boundary git entry points — `handoff-to-main` (§2, producer) and `require-on-main` (§3, consumer: a ten-state gate, rows H/I/G/A/B/C′/C/D/E/F) — plus the seven-prefix branch authority (`^(idea|prd|ard|spec|design|ready|brd)/`, §1 rule 3), §3.4's row-F delegation table (an `absent` gated input falls back to the caller's own pre-existing behaviour, never a new prerequisite — the three consumers that stop instead, `/design`, `/brd-ground`, `/brd-split`, each gate an input that was never optional), the §4.1 `Phase handoff:` outcome line, and the §4.3 consent choice. It inherits four of `specs-repo-git.md`'s hard rules unchanged and deliberately diverges on three (§1 rules 5–7: `require-on-main` is fatal by design, the deliverable commit carries a `Co-Authored-By` trailer, and `handoff-to-main` runs only behind the caller's own consent choice, never prompt-free). Eleven producers call `handoff-to-main` — `/idea`, `/create-prd`, `/update-prd`, `/create-ard`, `/specify`, `/design`, `/implement`, `/ready`, `/brd-intake`, `/brd-ground`, `/brd-split`; nine consumers call `require-on-main` — `/create-prd`, `/create-ard`, `/specify`, `/design`, `/implement`, `/epics`, `/ready`, `/brd-ground`, `/brd-split` (`/update-prd` deliberately excluded — its base is the Jira import, never a gated specs artifact; `/brd-intake` also excluded — it is the route's first command and consumes no prior phase's deliverable).
...
- Every git invocation is `git -C "$SPECS_PATH"` — the working directory is NEVER changed; twelve of the twenty callers resolve or run inside an **implementation** repository — a code clone under `$REPOS_PATH`, or the cwd's own repo for `/implement`, `/upgrade` and `/vuln` — which is the criterion, not "any repository that is not the specs repo": `$DOCS_PATH` is a read-only search base nine of the twenty also resolve for docs grounding (`/brd-intake`, `/create-prd` and `/update-prd` among them), and counting it would make the number say nothing about where a stray `git` could land. Twelve is all twenty minus the eight that touch no implementation repository at all (`/brd-intake`, `/brd-split`, `/create-prd`, `/update-prd`, `/feedback`, `/prompt`, `/prompt-brainstorm`, `/prompt-grill-me`)
