...
- **Suggested session name:** <PRD-ID>-<slug>-<role>   (omit this line for a command the rename aid below excludes — never the case for this plugin's own five commands, each of which either resolves a key or skips `resume.md` entirely; the companion `product-workflows` plugin's `/idea` is the family's own example, and it is excluded for its phase length rather than for want of a key, which it takes as a mandatory argument)
...
Within this rename-aid set, a PRD key is first available at the companion `product-workflows` plugin's `/product-workflows:idea`, and every command that takes a `<PRD>` argument (`/design`, `/ready`, `/implement` here, the companion `product-workflows` plugin's `/product-workflows:create-ard`, `/product-workflows:epics`, `/product-workflows:specify`, and the companion `docs-workflows` plugin's `/docs-workflows:document` and `/docs-workflows:release-notes`) prints a suggested `/rename <PRD-ID>-<slug>-<role>` line, so you can find this session again later in `claude --resume` by name instead of by scrolling. `<role>` is the lane tag of the command that just finished — pm, pa, pe, or dev. `/product-workflows:idea` and `/product-workflows:create-prd` are excluded from this aid, and the reason is the phase rather than the key — as the opening clause above already implies, each of those two takes a mandatory key as its first argument and refuses without one, so a PM run always has a key it could name the session after. Idea refinement is simply short enough that naming the session isn't worth automatically suggesting.
