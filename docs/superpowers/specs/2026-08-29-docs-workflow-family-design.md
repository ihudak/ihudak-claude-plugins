...
5. Start the command with Bash `run_in_background`. As shipped, not with `run_in_background` but in a process group of its own, started as `/document`'s render check boots a server (`docs-workflows:docs-profiles/render-verification` §2 step 2), so the command tracks everything it started; with its `{port}` token replaced by the port it serves on, and nothing else in it rewritten, from the repository's git top level, where the profile lives (§8.5), and only once the command's tool is found, a package manager's installed dependencies with it — a missing one stops with `DOCS_SERVE_NO_SERVER_TOOL` before anything starts, rather than after a readiness wait for a server that could never start (`/docs-serve` Phase 4). A server command binds the port it is given or fails: one that moves itself off a busy port — as Vite does without `--strictPort` — is a profile defect, like a loopback-only bind (`docs-workflows:docs-profiles/docs-profile-schema`'s field rules).
6. **Poll for readiness** up to `dev_servers.readiness_timeout_seconds` (default 120), then print the URL. As shipped, what answers is judged by step 3's evidence and recorded with its listener's pid only where step 3 would re-adopt it as the selected server — its listener in this checkout, identified as that server or as a one-server profile's only one — so a race lost to another checkout, to another of the repo's servers or to a foreign process records nothing and names what won the port; a command whose process group is gone before its port answers ends the poll at once, reported with its log's last lines and recorded nowhere, rather than waiting out the timeout as a server that might still come up; a timeout records its entry without a pid, and so does a port whose process the socket table does not show at all, which may be the server just started — the two entries that ever lack one — each with the id of the process group the command started (`/docs-serve` Phase 5).
7. Record the pid and port — and the space and visibility of the server started — under `.dev-workflows/` so `--stop` and `--status` work across sessions. As shipped, that is the `.dev-workflows/` at the repository's git top level, beside the profile (§8.5); the pid is the one the socket table names for the port, never a wrapper's, and it counts as living only while it still holds that port — step 6's timeout and a port whose process the table does not show being the two entries recorded without one; beside it, for a server the command started, is the id of the process group it started it in, which `--stop` signals — before the server has bound its port, too — only while that group's leader still leads it in this checkout with the recorded command on its command line, since a group id is reused as a pid is; `--stop` reports a server stopped only once no process there is identified as one of its servers and, where nothing there is identified at all, its port no longer answers as the docs site (`/docs-serve` Phase 5 and `--stop`). Where no rule ignores the state file, the command appends its one path to the repository's local `.git/info/exclude`, never committed, so a clean-tree gate does not meet it as an untracked file (`/docs-serve` Phase 7). A bare `--port <n>` with `--status` or `--stop` names the server bound to that port, never one a collision moved off it. And where the profile sets `dev_servers.concurrent: false`, nothing starts while another of its servers is known to be running — a living recorded entry, or one this run's own port checks identified, which on such a profile take in every other server's configured port (`/docs-serve` Phase 2 step 3 and Phase 4).
