...
3. The target must be a writable git work tree, or an empty/absent directory that the command offers to `git init`. A non-empty directory that is not a git work tree stops with `NOT_A_GIT_WORKTREE`. As shipped, a work tree must be the target's own top level: a target below one — a `site/` in a monorepo — stops with `DOCS_INIT_TARGET_BELOW_TOPLEVEL`, because the profile lives at a work tree's top level (§8.5) and so do the CI workflow and every path the scaffold fixes.
