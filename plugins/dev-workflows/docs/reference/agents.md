...
| `api-guideline-reviewer` | per routing | Read, Glob, Grep | Reviews an OpenAPI spec against Dynatrace REST API and IAM permission naming guidelines — version consistency, naming, IAM scope format, status codes, schema composition. | `/api-guideline-reviewer` |
