...
**What you also need, and it is not a plugin.** Nothing beyond `gh`, when present, for pull-request metadata — the pipeline otherwise reads and writes one markdown tree plus the mounted code repositories. Two commands do reach the network: `/vuln` looks a CVE up in the NVD API, and `/upgrade` queries the package registries. If you keep your work in a tracker as well, syncing the two is yours to arrange; no command here learns whether one exists.
