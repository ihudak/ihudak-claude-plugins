...
**Nine of these callers refuse a resolved `BRD-` container outright, in two groups that refuse for different reasons — and the difference decides how much of the refusal their rows inherit, which is why naming a bare count here was wrong twice.** **Four are route commands** — `/prd-ground`, `/brd-interview`, `/brd-package` and `/brd-reconcile`, each with its own `*_ROOT_LEVEL` stop — because grounding, deciding, packaging and reconciling all happen at the slice and a root has none of that work to do. **Their four rows therefore describe a slice run**, and a root never reaches the gate at all — except `/prd-ground`'s idea-route row just above, which gates a PRD folder that is never a BRD slice at all, on the same never-optional principle as its two BRD-route rows. **The other five refuse for an unrelated reason and their rows carry none of that qualification**: `/create-prd`, `/create-ard`, `/specify`, `/epics` and `/prd-proposal` each stop with a `*_BRD_NOT_SLICED` id on the shared `product-workflows:coverage-ledger-format` §5.1 test, taken on the directory prefix before this gate runs at all, because a BRD is a container and the PRD, the ARD, the specification, the Epic set and the effort proposal are all authored in the `PRD-` slices under it. `/prd-proposal`'s reason is narrower still, and it is the one whose refusal offers rather than only redirects: an effort proposal over a container is the *umbrella*, a different document priced from different inputs, which is why it names `/brd-proposal` instead of only telling the operator to descend. **Their five rows describe a `PRD-` folder run of either route** — a BRD slice and an idea-route PRD folder alike — so reading the four route commands' slice-only qualification onto them would narrow five rows to a route they do not belong to. **`/brd-proposal` inverts that and is the only row here that does**: it refuses a *slice* and runs at the root, so its row describes a root run gating a file in each of several **child** folders rather than one in the folder it resolved. That is a gate target per included slice, not one per run, and its stop collects them.
...
| `brd/brd-inventory.md` **(a slice)** | gated | named in §3.4's `/prd-ground` row, itself level-qualified there (§3.4: "Four are route commands... Their four rows therefore describe a slice run") — the route half of that paragraph's two groups, never the five build-ladder rows beside it, which describe a `PRD-` folder run of either route — a root never reaches that gate, so it only ever reaches a slice's own copy. It was classed advisory for one commit, in the same change that gated it |
