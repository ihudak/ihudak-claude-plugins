...
- Branch switching and fast-forward pulls remain sanctioned prep **on a writable clone** — they change which committed revision is present, not the content of it. Prep operations are limited to the reads `read-only-repos.md` §3 names — its chain's `git symbolic-ref` and `git rev-parse --verify --quiet` existence probes, and the `git log`, `git rev-list` and `git rev-parse` behind its three recorded facts — plus `git status`, `git remote set-head`, `git switch` and `git pull --ff-only`. On a **read-only** mount, prep runs only §3's write-free reads and none of the rest — not the dirty-tree `git status` its §2 skips, and none of the writes; see `${CLAUDE_PLUGIN_ROOT}/references/read-only-repos.md`. (The caller's terminal `commit-artifacts` step touches only `$SPECS_PATH` and never dispatches this agent's git.)
