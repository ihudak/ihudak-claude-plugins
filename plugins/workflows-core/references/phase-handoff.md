...
**unread** — nothing gates it and nothing reads it. **No artifact in the register above is currently in this class**, and that is a finding rather than an oversight: every artifact this family hands off turned out to have a reader once one was actually looked for. The frame-set index was the class's only member until §4.0's verification found `design-grounder` reading it. The array stays, because the class is a real third answer and the next artifact added may sit in it — but a producer selecting it today should first satisfy itself it has not made the same mistake:
