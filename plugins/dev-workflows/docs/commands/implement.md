...
`test-writer` is **mandatory** for any code change; when no single test framework is detected — none found, or more than one and `test-baseliner` refused to guess — the run asks the user to specify a command or explicitly skip (logged in the Final Report) rather than silently proceeding without tests. The Phase 3.5 fix loop caps at two attempts; regressions still present after that are surfaced to the user rather than looped on.
