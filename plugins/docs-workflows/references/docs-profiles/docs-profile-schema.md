...
- Where the operator can, record a `dev_servers.servers[].command` whose tool keeps the command line it was given — `node_modules/.bin/vitepress dev docs --host 0.0.0.0 --port {port}` rather than a launcher that rewrites its own as it runs — since `/docs-serve` identifies a server by the command line its process carries and allows for one rewrite only, `npx <rest>` becoming `npm exec <rest>`.
