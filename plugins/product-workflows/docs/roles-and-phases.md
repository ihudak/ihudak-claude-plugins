...
- **Runs:** `/idea`, `/create-prd`, `/update-prd`; also `/prd-proposal` and `/brd-proposal`, the two optional effort proposals — one for a slice, one for the programme over it — which gate nothing on the build ladder and which nothing on it waits on; each does gate its own input — `/prd-proposal` the folder's `prd.md`, `/brd-proposal` each included slice's `proposal.md` — and that is the whole of what either refuses on (see [proposal](#proposal) below); also the early run of the companion plugin's `/docs-workflows:release-notes`, before any specification or design exists yet.
