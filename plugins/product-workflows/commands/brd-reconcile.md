...
below), the same way `/brd-package`'s disposition picker draws its four from its reviewer agent's
...
1. **A live working document.** Corrected in place — these are the documents the route works on,
   ...
   | `decisions.md` — a record's **decision fields**, the set `decision-register-format.md` §4 names, or its `argumentation` | **`refused-with-reason`** where the row asks for a direct edit — the set is cited rather than listed so a field §4 adds is refused on the day it is added, where an enumeration would let it through as prose. Those move only through this command's own freeze, §4's two reopening causes, or the propagation sweep's four dispositions. A customer who wants a decision changed has already changed it: their answer is a `[CD#n]`, frozen in the *Freeze the customer decisions* phase, which reopens what it contradicts |
   ...
   **A refusal here is not a refusal of the customer's point.** In every row above the substance
   ...
   `/brd-interview` round, the next intake, or, for an allocation no command can make, a person
   named in *what still needs a human* — and the refusal says which,
...
they were asked. An append is how an append-only record moves on, and its last `Status:` line
...
`open` resolves nothing** — its question stays held for the customer (*Freeze the customer
decisions*), and so does its defect.
...
register, the code-defect log, every round record, every dated snapshot, and the customer's own
captured files.
...
| a `decisions.md` record's **decision fields**, the set `decision-register-format.md` §4 names | `needs-a-human` unless it is this run's own propagation-sweep write. They move only through the four dispositions the previous phase fixes, or through §4's two reopening causes — never because a sentence nearby went stale. The set is cited rather than listed so a field §4 adds is covered on the day it is added; a record's `argumentation` is **not** in it and is the route's own prose, which is what `updated` is for (below) |
