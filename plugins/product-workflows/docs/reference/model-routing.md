...
**Two commands have a classification floor, and it is a property of their output rather than of their input.** `/prd-proposal` and `/brd-proposal` both floor at `SIGNIFICANT` — the run produces a number a customer will make a commercial decision on, and the format's own [residual-risk rule](proposal-format.md#the-risk-the-format-cannot-remove) states it plainly: a plausible number with a defensible-looking argument is more dangerous than an obviously rough one. An umbrella compounds it, because a reader checking one is checking a roll-up rather than a derivation. Neither floor is about how much there was to read; a one-package tier-1 proposal floors exactly as a programme umbrella does. Both escalate to `HIGH-RISK` where the folder's own state warrants it. Combined with the stricter gate above, the practical effect is that both commands need an Opus session or an explicit, logged override.

**No command here floors the way the companion `dev-workflows` plugin's `/implement` does**, on multi-source input. A PRD, an Epic, or a BRD is always a single addressed item; the fan-out that varies is how many repositories a grounding pass touches, which is a within-classification detail rather than a floor of its own. The two floors above are the exception that proves the distinction — neither is triggered by the size or the shape of what was addressed.
