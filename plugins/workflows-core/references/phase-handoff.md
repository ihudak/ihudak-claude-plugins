...
**Resolve `<default-ref>` before testing it — the same probe §2.1 already runs on the producer side, not a second design:** `git -C "$SPECS_PATH" remote get-url origin`. Exit 0 with a non-empty URL → `<default-ref>` is `origin/<default>`, and every primitive below, and row G, are unchanged from what shipped. Anything else → `<default-ref>` is the local `refs/heads/<default>`, and every primitive below tests against that instead. **The two cases are not one weakened into the other.** With a remote configured, its tracking ref is the only trustworthy record of what merged — the local branch can be stale, ahead, or diverged, which is exactly the uncertainty row G exists to refuse. With no remote configured, there is no remote state to be uncertain about: the local default branch **is** the default branch, so verifying against it is the correct application of this gate, not a relaxation of it. §2.1 has treated a remote-less specs repo as an ordinary, supported state since the fix that made its push probe "a capability probe, never a gate failure" (§2.1) — this section only brings the consumer side into agreement with a producer behaviour that already shipped.
