...
- `profile`, `docs_repo_path`. When `profile.frontmatter.changelog_guidelines` is absent, the two inline changelog rules (customer-readable one-liner, no Jira key) are the whole requirement.
