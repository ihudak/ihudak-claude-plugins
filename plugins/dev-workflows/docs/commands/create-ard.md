...
Five `dev-workflows` subagents are dispatched: `docs-grounder` (Phase 3, read-only grounding on the shipped product docs — default ON when `$DOCS_PATH` resolves, advisory, never a gate), the folder read (Phase 2 — for a PRD-level run, only when no authored PRD file is present on the specs repo's default branch; for an Epic-level run, always, scoped to the Epic), `code-scanner` (Phase 3, one instance per confirmed repo, up to 4 concurrent per batch), `ard-reviewer` (Phase 5, Opus-pinned), and `impl-maintenance` (Phase 8, session lessons-learned). The detection-tier agents run at `detection_model` (the §2.1 Sonnet chain); `ard-reviewer` runs at `review_model` (the §2 Opus chain, frontmatter-pinned, no override). The interview and the ARD authoring itself run inline on the session's own `current_model` rather than through a delegated subagent.

## What it needs

- **The PRD on the specs repo's default branch** — gated via `require-on-main` against `specifications/<PRD>-<vslug>/`. An unmerged PRD is a hard stop, naming the branch and any open pull request. An **absent** PRD is not a stop: the run falls back to reading the Jira export directly through the folder read, and reports that it did so — `/specify` reports an absent PRD the same way, though it has no fallback to make: it reads the item from Jira on every run regardless, and it means `/create-ard` never requires `/create-prd` to have run first.
...
- **No Jira input at all.** No command here reads outside `$SPECS_PATH`, and `$VAULT_PATH` is not consulted — a BRD key names a folder under `$SPECS_PATH`, and handing it to a Jira export lookup would fail on a key no tracker was ever asked for.
