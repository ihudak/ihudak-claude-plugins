...
Otherwise, for each recorded entry — one per port, the key Phase 7 writes: test the pid with `kill -0`, and re-probe the port and URL with the same reachability check Phase 2 uses. Report, per entry: port, space (and visibility, where recorded), pid, URL, and one of *running* (pid alive, port answering as recorded), *stale* (pid gone, or a live pid whose port no longer answers this docs site), or *unknown* (the probe itself failed). A selector narrows the report to one entry, **resolved to a port first and then looked up** by Phase 7's port lookup: `--port <n>` is that port, and `--internal` is the port Phase 1's own server selection resolves for `--internal`. With neither given, every recorded entry is reported.

## `--stop` mode

Read the same state file. Select the entry to stop — **by port, always**: a selector is resolved to a port first, and that port is then looked up by Phase 7's port lookup.

- A `--port <n>` argument is the port — no prompt.
- An `--internal` flag resolves to the port of the server Phase 1's own selection (steps 1–3, `--port` override included) picks for `--internal` — the same selection a serve run would make, so `--stop --internal` stops the server `--internal` started. On the two-server profile `/docs-workflows:docs-init` writes, that is the `internal`-tagged server, never its public twin of the same space.
...
1. Read `<repo-root>/.dev-workflows/docs-serve.state.json` (Phase 7 writes it) and look up the port this run resolved (Phase 1, as overridden by `--port`) by Phase 7's port lookup. An entry for that port whose pid answers to `kill -0` is the running server — including one an earlier run's Phase 3 moved to a later port, which the lookup finds by its `requested_port`. Report its recorded URL and stop. **Never start a second server.**
2. No living recorded entry (a different session started it, the state was lost, or nothing has ever run) — probe the port directly. An unreachable port means nothing is listening: proceed to Phase 4. A port that answers needs identification, not just presence — fetch the response and test it for something that actually names this docs site (the profile's `repo.name`, or the generator's own `site_name`, whichever the repo exposes). A confirmed match is the same already-running case as step 1: report the URL and stop, recording an entry at that port into the state file, in Phase 7's shape, since Phase 7 never ran for it this session. An answer carrying no such marker is Phase 3's case.
...
Write (create, or merge into) `<repo-root>/.dev-workflows/docs-serve.state.json` via Bash — an object **keyed by the port actually bound** (after any Phase 3 shift, written as a string), each entry carrying `{space, visibility, requested_port, pid, url, command, started_at}`: `space` and `visibility` copied from the selected `dev_servers.servers[]` entry (`visibility` only where it records one), `requested_port` the port Phase 1 resolved before any Phase 3 shift, and `url` exactly what Phase 6 reported. Writing at a port replaces whatever was recorded there, which is correct: Phase 2 has just established that nothing living of ours holds it. This is what lets `--status` and `--stop` (above) work in a later session without re-deriving anything.
...
**The port lookup** — the one rule Phase 2, `--status` and `--stop` all apply. The entries *for* a port *P* are the one keyed at *P* and any whose `requested_port` is *P* — a server Phase 3 moved off *P* because something else held it. Phase 2 wants the living one among them; `--status` reports every one; `--stop` stops the one there is, and where there are several lists them as prose and asks, with its own picker above. A selector (`--port`, `--internal`) is always resolved to *P* first — through Phase 1's own selection for `--internal` — and only then looked up; nothing here searches the file by space.
...
- ALWAYS record pid/port state under the resolved repo's own `.dev-workflows/`, never under `$SPECS_PATH` — keyed by the port actually bound, NEVER by space id, and looked up by port for `--status` and `--stop` (Phase 7's port lookup); a space id does not identify a server when one space is served twice
