...
An open requirement defect of those five classes is now `/brd-interview`'s seventh question source (Phase 3). It is asked **once across the slices**, by the slice holding the lowest-numbered of its rows still live — `covered-here` or `deferred-to` in that slice's own ledger — and never again once any slice's `interview/customer-questions.md` records it; a defect whose owner cannot yet be decided, because a row it lists is still `unallocated` or a sibling's ledger or question set cannot be read, is withheld and reported rather than guessed. The question is always `[C]` (`interview-tagging.md` §1): it states what the defect records — the two readings, the two requirements that cannot both hold, the missing observable outcome — and names the image where the row was drawn from one. It travels in the package as every held `[C]` does — the review schema gains no section, and the answer comes back in its decision log — with its held entry naming the `[DEF#n]`. `/brd-reconcile` copies that id into the answering `[CD#n]`'s new `settles` field (`decision-register-format.md` §1), from the held entry and never from the customer's prose, and its Phase 8, which wrote two of the log's resolutions, now writes a third — `resolved-by: <SLICE-KEY>/[CD#n]`, which `brd-format.md` §4 now admits beside `resolved-by: <SLICE-KEY>/[CG#n]`. Both carry the key of the slice whose decision or finding settled the defect, in the shape `conditional_on` already gives a record of another BRD: each slice numbers its own `[CD#n]` and `[CG#n]` from 1 while the defect log is the parent's, so a bare `[CD#2]` there would name no one record. A settled defect takes `withdrawn` where the answer drops the requirement, which the ledger then records `rejected: [DEF#n]`; `customer-amended <date>` where the review supplied corrected text; and otherwise `resolved-by: <SLICE-KEY>/[CD#n]`. Every round record `/brd-interview` writes now carries a `requirement defects:` line saying which defects that round asked and which it withheld, and why.
