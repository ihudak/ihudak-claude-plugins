...
Eight of the eleven agents above are dispatched by a command this plugin ships today. The three in this section are not yet: `docs-auditor`, `ia-planner` and `docs-audit-reviewer` are dispatched by `/docs-audit`, and while that command is absent from `commands/` none of them is reachable — they ship ahead of it because the contracts they execute were frozen first, and each had to be reviewed alongside the contracts it executes. `/docs-profile` dispatches none of them: it scans a documentation repository itself and writes the profile `/document` consumes.
