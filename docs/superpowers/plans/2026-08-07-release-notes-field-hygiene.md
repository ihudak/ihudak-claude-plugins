...
**Corrected 2026-08-13 (R40, fix round 2):** the directory enumeration above undercounts. Re-verified against the same `4720e28`/`f64a69b` worktree `diff -rq`: of the 42 CRLF-only files, 37 are under `references/guidelines/*` and `references/api-guidelines/**` as stated, but the remaining **5** are under `references/fix-vuln/` (`build-systems.md`, `nvd-api.md`) and `references/upgrade/` (`compatibility.md`, `ecosystems.md`, `lts-sources.md`) — confirmed CRLF-vs-LF-only (identical to canonical after `tr -d '\r'`). The counts (54/47/42) are correct and unaffected; only the directory list was incomplete. Matches the four-directory phrasing the sibling annotation in `2026-08-08-document-gate-enforcement.md` (R38, WRONG-TARGET) already uses.
...
**Corrected 2026-08-13 (R40, fix round 2):** the directory enumeration above undercounts. Re-verified against the same `4720e28`/`d404a75` worktree `diff -rq`: of the 42 CRLF-only files, 37 are under `references/guidelines/*` and `references/api-guidelines/**` as stated, but the remaining **5** are under `references/fix-vuln/` (`build-systems.md`, `nvd-api.md`) and `references/upgrade/` (`compatibility.md`, `ecosystems.md`, `lts-sources.md`) — confirmed CRLF-vs-LF-only (identical to canonical after `tr -d '\r'`). The counts (54/47/42) are correct and unaffected; only the directory list was incomplete. Matches the four-directory phrasing the sibling annotation in `2026-08-08-document-gate-enforcement.md` (R38, WRONG-TARGET) already uses.
