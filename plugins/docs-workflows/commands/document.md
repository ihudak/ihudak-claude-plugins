...
7. **Toolchain preflight.** Execute `${CLAUDE_PLUGIN_ROOT}/references/toolchain-preflight.md` against
   ...
   `curl` and `ps` for the smoke check's probes wherever the profile records a dev server; repo
...
- A check that **cannot run** — `curl` or `ps` is not installed, or a probe exits 127 — ends the smoke-check too → never blocks; every page not yet checked falls back to the manual table, the record names the tool, and `render_smoke_check` records `UNAVAILABLE` (Ledger (final)).
...
- `render_smoke_check` — `RAN` when the smoke-check completed for every space in scope — a 404 does
  ...
  its command);
  `UNAVAILABLE` when the check could not run because `curl` or `ps` could not (Step 2) — naming the
