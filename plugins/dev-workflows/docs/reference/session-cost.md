...
Seventeen commands emit a cost entry: `/idea`, `/create-prd`, `/update-prd`, `/create-ard`, `/specify`, `/epics`, `/design`, `/implement`, `/ready`, `/document` (both its keyed and direct modes), `/release-notes`, `/brd-intake`, `/brd-ground`, `/brd-split`, `/brd-interview`, `/brd-package`, and `/brd-reconcile`. Sixteen of those pass a fixed `phase`/`role` pair baked into the command itself; one infers it. `/release-notes` passes `phase: inferred, role: inferred` and lets the cost phase resolve the real values by checking whether a `specification.md` or a `design.md` already exists under the PRD's specs directory — neither present means the PRD itself was only just created, so the entry is attributed to the PM's early run (`prd-creation` / `pm`); either one present means engineering work is already underway, so the entry is attributed to the dev's later documenting run (`documenting` / `dev`). Epic presence is deliberately excluded from that check, since a PRD can have drafted Epics while still entirely in product hands.

**What does not emit one, and why.** `/vuln`, `/upgrade`, and `/docs-profile` emit no cost entry. The number measures AI investment in a product increment, and none of those advances a PRD- or BRD-scoped artifact — a CVE bump or a library upgrade is noise against a PRD, and a metric that averages the two answers a question nobody asked. **Session *feedback* is a different question**: `/vuln` and `/upgrade` do emit that, because it records a gap in the plugin itself rather than the cost of the work.
...
**Why the other three do not.** `/vuln` and `/upgrade` have no Product Requirements Document to attribute spend to and sit outside the pipeline entirely; `/docs-profile` is a one-shot utility with no PRD either. (`/statusline`, the fourth command that has never emitted one, sets a configuration value rather than running a task, and ships in `workflows-core`.)
...
Two details in that sentence are doing real work. The ceded run is found **by name**, not by counting invocations — a window often contains commands that emit no cost entry at all (`/vuln`, `/upgrade`, `/statusline`, `/docs-profile`), and an interrupted run leaves a mark too, so counting positions would shift every attribution by one and file a security run's spend under a product phase. And a command invocation is recognised by its whole envelope rather than by one tag, because Claude Code writes built-in and plugin commands in two different tag orders; matching only the built-in shape finds no plugin command at all. If a recorded intent matches no invocation, nothing is written for it — its spend stays with the replaying run, and the run says so rather than attaching it to a neighbour.
