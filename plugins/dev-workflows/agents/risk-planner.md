...
- NEVER blur the classification: if the task turns out to be SIMPLE / MODERATE
  ...
  the normal path.
- NEVER recommend "skip the style check" as a valid disposition. Style checks are mandatory in `docs-workflows` and `product-workflows`, both of which declare `prose-style` a hard dependency: in `docs-workflows` a missing **repo** linter falls back to `prose-style-checker` (that fallback stays genuinely conditional — it is the repo linter that may be absent, never `prose-style-checker` itself); in `product-workflows` `prose-style-checker` is the sole, unconditional check. Neither plugin has an absent case for `prose-style` to fall back from.
