...
| Docs | [`/docs-init`](docs/commands/docs-init.md) | Scaffold a documentation repository for a project that has none: a product-shaped page skeleton, two builds, Vale, CI gates, and the profile every later command reads. |
...
Eight agents and sixteen reference pages carry the docs-repo discovery, PR-diff summarising, doc planning, doc writing and doc review that `/document` and `/release-notes` draw from; `/docs-profile` and `/docs-serve` each work through a scan or a profile read of their own rather than this pool, and the `docs-scaffold-reviewer` that `/docs-init` and `/docs-brand` share is the eighth agent — reviewing a scaffold diff, never a page. Those sixteen markdown pages plus two bundled data files — `default-owners.txt` and `docs-profile.default.yml`, which the commands, the skill and a hook read as data rather than open as prose — are the **eighteen files** under `references/`; that is the whole of the difference between the page count and the file count, and [References](docs/reference/references.md) does the arithmetic file by file. Alongside them ship the `docs-frontmatter` skill `/docs-profile` points a repository at, and two advisory hooks — one injecting specs context on a `/document` or `/release-notes` prompt, one reminding about a docs page's changelog and owners frontmatter.
...
| [Workflow overview](docs/workflow.md) | The six commands as one diagram, and the two modes of `/document`. |
