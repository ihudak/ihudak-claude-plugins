...
**Second correction (2026-08-16, post-merge):** the table above has been reordered to `H I G A B C′ C D E F`, matching §3.3 as corrected. It previously read `H I A B C C′ D E F G` — leaving G last and C before C′ — so the correction note below described a fix the table itself had not received. A trace whose row order contradicts the contract it traces would lead a reader to re-derive the very shadowing this file exists to rule out. Only the row order changed; every reaching condition and outcome is unaltered.
