# Increment 4 — `pm-workflows` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extract the twelve product-management commands, their twelve agents and their nine references out of `dev-workflows` into a new `pm-workflows` plugin, leaving `dev-workflows` as the five code-facing commands and nothing else — the last structural increment of the five-plugin split.

**Architecture:** `pm-workflows` declares `workflows-core` and `prose-style` as dependencies, exactly as `docs-workflows` does. The move is a `git mv` of a partition that measurement shows is **exact**: no agent and no reference is shared between the pm and dev groups, so every `${CLAUDE_PLUGIN_ROOT}/references/<name>` citation among the moving files stays correct by construction, because the citing file and the cited file move together. The rewrite surface is therefore small and mechanical — 38 lines of `dev-workflows:<token>` that must become `pm-workflows:<token>` — plus one bare-basename cross-boundary citation the loader gate cannot see.
...
- **S1 — five plugins.** `workflows-core`, `pm-workflows`, `dev-workflows`, `docs-workflows`, `guideline-reviewers`. After this increment all five exist and `dev-workflows` is simply what remains — there is no fifth structural step.
- **S4 / S16 — a file or agent moves to core when its second consumer group *actually exists*, never in anticipation of one.** `design-grounder` has one consumer group today (`/brd-ground`, pm) and therefore moves **to `pm-workflows`**, not to core.
- **S8 — `dev-workflows` goes to 4.0.0; `pm-workflows` starts at 1.0.0.** A version describes a plugin's own interface, not the provenance of its files.
- **S12 — `prose-style` is a declared dependency of `pm-workflows`, and the graceful-skip branches are deleted.** Increment 3 retired `docs-workflows`'s share; increment 4 inherits the balance in `/epics`, `/create-prd`, `/update-prd` and their documentation pages. **Count statements, not forks** — the sentences describing the branch are the half that goes stale silently, because nothing gates them.
...
| → `pm-workflows` | stays in `dev-workflows` |
|---|---|
...
| → `pm-workflows` (9) | stays in `dev-workflows` (15) |
|---|---|
...
**Created:**
- `plugins/pm-workflows/.claude-plugin/plugin.json` — name, description, author, version `1.0.0`, `dependencies: ["workflows-core", "prose-style"]`
- `plugins/pm-workflows/README.md`, `plugins/pm-workflows/LICENSE`
- `plugins/pm-workflows/commands/` — 12 files, moved
- `plugins/pm-workflows/agents/` — 12 files, moved
- `plugins/pm-workflows/references/` — 9 files, moved
- `plugins/pm-workflows/docs/` — a new tree, derived from what the plugin ships
...
- `plugins/workflows-core/scripts/command-namespaces.json` — `pm-workflows` key added, `dev-workflows` reduced to five
...
### Task 1: Scaffold `pm-workflows` and move the 33 files

**Files:**
- Create: `plugins/pm-workflows/.claude-plugin/plugin.json`, `plugins/pm-workflows/README.md`, `plugins/pm-workflows/LICENSE`
- Move: 12 commands, 12 agents, 9 references (exact lists in "The measured allocation" above)
- Modify: `.claude-plugin/marketplace.json`

**Interfaces:**
- Produces: the `pm-workflows` plugin directory and its catalogue entry. Every later task assumes both exist.
- Consumes: nothing.
...
mkdir -p plugins/pm-workflows/{commands,agents,references}
for c in idea create-prd update-prd create-ard specify epics \
  ...
  git mv plugins/dev-workflows/commands/$c.md plugins/pm-workflows/commands/$c.md
done
for a in ard-reviewer brd-package-reviewer brd-reader code-grounder customer-review-reader \
  ...
  git mv plugins/dev-workflows/agents/$a.md plugins/pm-workflows/agents/$a.md
done
for r in ard-format brd-format bundle-packaging coverage-ledger-format customer-review-schema \
  ...
  git mv plugins/dev-workflows/references/$r.md plugins/pm-workflows/references/$r.md
done
```
...
```bash
ls plugins/pm-workflows/commands | wc -l    # expect 12
ls plugins/pm-workflows/agents   | wc -l    # expect 12
ls plugins/pm-workflows/references | wc -l  # expect 9
...
Add one entry with `"source": "./plugins/pm-workflows"`, matching the shape of the `docs-workflows` entry. **Do not reformat the file** — Claude Code parses it. The entry's `description` must be the same blurb as `plugin.json`'s, under the same 1024-character budget.
...
`check-docs.sh` will still be red at this point (no `pm-workflows` docs tree, and `PLUGIN_RELS` does not name it) — that is expected and is Tasks 5 and 6's work. Report it as expected-red, not as green.

- [ ] **Step 6: Commit**

```bash
git add plugins/pm-workflows .claude-plugin/marketplace.json plugins/dev-workflows
git commit -m "feat(split): extract pm-workflows — 12 commands, 12 agents, 9 references"
```
...
**Files:**
- Modify: 10 files under `plugins/pm-workflows/commands/`, 8 file-slots under `plugins/workflows-core/references/`, `plugins/dev-workflows/README.md`, `CLAUDE.md`
- Modify: `plugins/dev-workflows/commands/ready.md:237` (the ruling)

**Interfaces:**
- Consumes: Task 1's moved tree.
- Produces: a tree where no `dev-workflows:<token>` names a thing that now lives in `pm-workflows`.
...
Expected shape at measurement time: **38 lines across 20 files** — 10 files in `plugins/pm-workflows/commands/` (moved files citing their own agents), 8 in `plugins/workflows-core/references/`, `plugins/dev-workflows/README.md`, and `CLAUDE.md`.

- [ ] **Step 2: Rewrite each to `pm-workflows:<token>`**
...
`/ready` stays in `dev-workflows`; `ard-format.md` is now in `pm-workflows`. Read the surrounding phase to decide which of these it is, then apply the matching fix:

1. **The citation is explanatory** — it names where a frontmatter field is defined and `/ready` never reads the file. Then rewrite it to name the field without pointing at an unreachable file, e.g. drop the parenthetical or qualify it as `pm-workflows:ard-format`, whichever the surrounding sentence supports.
...
for r in ard-format brd-format bundle-packaging coverage-ledger-format customer-review-schema \
         decision-register-format idea-format interview-tagging specification-format; do
  hits=$(grep -rn "$r" plugins/dev-workflows --include=*.md | grep -v "pm-workflows:$r")
  [ -n "$hits" ] && echo "== $r ==" && echo "$hits"
done
```

Then the same scan in the other direction (a `pm-workflows` file naming a reference that stayed in dev). Every hit is either a real dangling pointer or correct prose — decide each one by reading it, and record what you decided.
...
```bash
git add plugins/pm-workflows plugins/workflows-core plugins/dev-workflows CLAUDE.md
git commit -m "fix(split): repoint pm-bound citations at pm-workflows"
```
...
**Files:**
- Modify: `plugins/pm-workflows/commands/epics.md`, `create-prd.md`, `update-prd.md`
...
```bash
grep -rn "prose-style" plugins/pm-workflows/commands/{epics,create-prd,update-prd}.md \
  plugins/dev-workflows/agents/risk-planner.md \
  plugins/pm-workflows/docs plugins/dev-workflows/docs 2>/dev/null
```
...
`prose-style` is a declared dependency of `pm-workflows`, so an unsatisfied dependency **disables the plugin** rather than letting it half-run. Every "skipped gracefully" path is therefore unreachable, and keeping unreachable branches is its own defect.
...
```bash
git add plugins/pm-workflows plugins/dev-workflows
...
- Possibly create: `plugins/pm-workflows/hooks/`, or `plugins/workflows-core/hooks/`

**Interfaces:**
- Consumes: Task 1's moved tree — `/epics` is now a `pm-workflows` command while the hook that preloads for it is still in `dev-workflows`.
...
`/epics` has moved to `pm-workflows`, but a `UserPromptSubmit` hook fires on **every** message from **every** installed plugin — it is not scoped to its plugin's own commands. So today, after Task 1: a user holding both plugins still gets the preload; a user holding `pm-workflows` alone gets none.
...
Model `pm-workflows`'s script on `docs-workflows`'s, the smaller of the two.
...
Neither is pipeline-specific: `notify-done.sh` is a `Stop` hook and `test-notify.sh` a `PostToolUse`/`Bash` hook, so both fire regardless of which command ran. They stayed with `dev-workflows` through increment 3, which means a user who installs `pm-workflows` alone gets no completion notification.
...
2. **Duplicate them into `pm-workflows`.** Everyone holding both plugins gets **double-notified** — the failure increment 3 named explicitly when it declined to duplicate.
...
- Produces: gates that see `pm-workflows`. Task 6 and Task 7 cannot be verified until this lands.

- [ ] **Step 1: Add `pm-workflows` to the three dispatch lists**
...
- `PLUGIN_RELS` gates every plugin's docs tree identically — `pm-workflows` joins it.
- `COST_PLUGIN_RELS` scopes check 8 (`emit-cost` phase/role pairs against `workflows-core:cost-emission` §7) — `pm-workflows` joins it, because the moved commands emit cost.
- `HANDOFF_PLUGIN_RELS` scopes check 11 (the `/brd-*` `choices:` placeholder rule) and today names `plugins/dev-workflows` **only**. Every `/brd-*` command has now moved, so this becomes `plugins/pm-workflows` — a **replacement, not an addition**.
...
- qualifier `/pm-workflows:` → **empty**

So `HANDOFF_PLUGIN_RELS` and that one glob must change together:

| Change | Result |
|---|---|
| `HANDOFF_PLUGIN_RELS` → `pm-workflows`, glob left as `dev-workflows` | glob empty → `check_merge_clause:927` **fails loudly**: *"no longer names the command family … the family is read ONLY from the scope-paragraph line"* |
| glob → `pm-workflows`, `HANDOFF_PLUGIN_RELS` left as `dev-workflows` | glob empty for dev → same loud failure |
...
Task 2 deliberately left that sentence untouched — rewriting its prefixes while it still says "every command in `dev-workflows`" would have made it self-contradictory, and its prefixes are already mixed across two plugins. **Re-measure it here**, after `HANDOFF_PLUGIN_RELS` becomes `plugins/pm-workflows`, because only then does the sentence have a tree to be true about. Then reconcile the two copies — and **prefer a citation to a second copy**, which is the rule that would have prevented this.
...
git commit -m "chore(gates): teach the scripts about pm-workflows"
```

---

### Task 6: Build `pm-workflows`'s documentation tree

**Files:**
- Create: `plugins/pm-workflows/docs/` — index, top-level pages, 12 command pages, reference pages
- Modify: `plugins/pm-workflows/README.md` (role-indexed pointer table)
...
Derive `pm-workflows`'s set the same way, and state the derivation. `brd-workflow.md` and `roles-and-phases.md` are the two candidates that need a decision: measured against the current `dev-workflows` tree, `brd-workflow.md` names pm commands 98 times and `roles-and-phases.md` 74 — both are overwhelmingly pm content.
...
- **Check 11** gates the `/brd-*` `choices:` placeholder rule and now runs against `pm-workflows`. It derives its family from the first such phrase in `workflows-core:next-phase-offer`'s scope paragraph, its targets from `phase-handoff.md`'s row-F table, and its writers from each command's own `deliverable_paths`. **Every one of those relations coming up empty fails rather than passes** — so if check 11 reports nothing, verify it is because the content is correct, not because `HANDOFF_PLUGIN_RELS` points at a plugin with no `/brd-*` commands in it.
...
```bash
git add plugins/pm-workflows plugins/dev-workflows
git commit -m "docs(pm): build the pm-workflows documentation tree"
```
...
The reinstall section must gain `pm-workflows`, and its point restated: **reinstall the plugin that holds the file you edited — not the one whose workflow you were thinking about.**
...
The spec's stated verification for this increment: **the full pipeline runs across three plugins — a PRD authored in `pm-workflows`, designed in `dev-workflows`, documented in `docs-workflows`.**
