...
**`docs-workflows` hard-requires two: `workflows-core` and `prose-style`.** The first is the same structural requirement for the same reason — its three commands and seven agents read the shared corpus through the loader skill and dispatch core's agents natively. The second is what makes it different from every other plugin here: `prose-style` is a *marketplace sibling*, and declaring it converts a runtime probe into an install-time guarantee, which is why `docs-workflows` carries no absent case for its style gate at all. A relationship's kind is a property of the declaring plugin, not of the plugin declared — the same `prose-style` is a hard dependency here and an optional companion in `dev-workflows`, and neither reading is wrong.
