...
9. **Enumerate existing children — `split_mode: full` only.** In `allocate-only` mode there are no
   ...
   **A positive test, not a name match.** Matching a subdirectory by name and then reading an
   ...
   passing for every child it leaves standing with both files. **Carry the marked sets — standing empty, out of step
...
| Mode | Fires when |
|---|---|
| `full`, `recut_mode` false | **exactly one** slice stands as a `covered-by` target — the union of the children Phase 3 keyed this run and the children Phase 0 step 9 enumerated is a single folder — **and** two or more rows are in the set this step would actually offer (below), which leaves out every row that slice may not be offered |
...
- **Assign to a named slice** (`split_mode: full` only) → prompt for the slice's key: any
  ...
  other child that still claims this `[BR#n]` is offered it**, its own row reading `unallocated`
...
whether it actually split and/or walked the ledger — **a fully-allocated ledger is not on its own the no-op**, since the re-cut path and the reconcile path both start from one, so name the branch step 10 took rather than the ledger state it read; **every child Step 3 reconciled although no walk of this run touched it**, with each `[BR#n]` added or withdrawn, every ledger row Step 3 left standing (its case list), and every child Phase 0 step 9 marked unreconcilable, by name; the classification and model routing (+ any
