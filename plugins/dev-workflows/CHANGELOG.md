...
**`/vuln` diverges from `/implement` on one point, deliberately:** `/implement` honours the operator's own *"Skip tests for this run"* without penalty and flips `clean_finish` only on the run's own record after two failed hints (`code-handoff` §2.9). Here both provenances finish `false`. What ships unverified is a security remediation, the flag's only effect is the draft and its DO-NOT-MERGE banner, and this state already finished `false` before the question existed. Clearing the flag on the operator's answer would be a loosening, not a symmetry.
