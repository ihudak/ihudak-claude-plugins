...
- `DOCS_INIT_TARGET_BELOW_TOPLEVEL` — the target is inside a git work tree but below its top level, such as a `site/` in a monorepo. A scaffold's profile, its CI workflow and every path it fixes sit at a work tree's top level, so the command scaffolds there or nowhere: point it at the top level, or at a directory outside that work tree.
