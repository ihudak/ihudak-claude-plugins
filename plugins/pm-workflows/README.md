...
| PM *(BRD route — inventory)* | [`/brd-intake`](docs/commands/brd-intake.md), [`/brd-split`](docs/commands/brd-split.md) | Intake a customer BRD verbatim and extract its requirement inventory (step 1), then split it once each row has a recorded fate (step 3). |
| PA *(BRD route — grounding)* | [`/brd-ground`](docs/commands/brd-ground.md) | Step 2, and mandatory: ground every requirement claim against the mounted code and design repos. PM-initiated, PA/Dev-executed. `/brd-split` gates on its findings. |
...
| PA *(optional)* | [`/create-ard`](docs/commands/create-ard.md) | Author an Architecture Decision Record, grounded in the mounted implementation code. Optional: a PRD can hand straight to `/specify`. |
