...
`package.json` and lint configuration beside itself, not at `repo_root`. So where the caller passes
`site_root`, each rung below looks for its configuration in `site_root` first and then in
`repo_root`, and runs from the directory where it found it; without `site_root` it looks in
`repo_root` alone. The commands the profile records — step 2's per-space `lint` — run from
...
- A rung whose configuration is absent is still a rung the ladder passed: record it with `outcome: not_detected` and a one-line `reason` (e.g. "no .vale.ini in the site or at the repo root"). `primary_attempts` describes the whole climb, not only the failures.
