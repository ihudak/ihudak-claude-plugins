...
**What this table is, and what it is not.** It classifies the artifacts whose class a producer has had to resolve, and it is **not** a census of everything the family hands off: roughly thirty distinct paths appear across the tree's `deliverable_paths` declarations, and twelve are listed here. Most of the rest ride in a set that already contains a gated path and are carried by the strongest-class rule below, which is why their absence has cost nothing so far — but §4.0's rule requires a class **per path**, so a producer whose whole set is unlisted has nothing to resolve against. `/brd-ground` under `--no-code` was exactly that case until `design-grounding.md` was classed. **Treat an unlisted path as unclassified rather than as unread**, name its reader, add its row, and only then pick the array.
