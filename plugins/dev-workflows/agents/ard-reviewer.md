...
- **Identifier integrity:** `[AD#N]` unique + contiguous; cross-references point at existing IDs. A dash-form ID (`[AD-1]`, …) is a **BLOCKER** — Jira auto-links it to an unrelated ticket on paste, and the vault importer rewrites it into `[[[AD-1]]]` on export.
