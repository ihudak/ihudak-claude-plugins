...
The boundary between the two runs is not guessed. A transcript already records every slash-command invocation, so the replaying run splits its own measurement window at the point the ceded run began, writes the earlier slice under the ceded run's labels, and keeps only the later slice for itself. The two slices are disjoint and sum to what the replaying run would otherwise have claimed whole.
