...
**Amended after the whole-branch review: the key in a document's filename is its own package's key, never this run's applied uniformly.** A prerequisite package copied in arrives already named from the packaging run that built it and keeps those names, so rule 1's `<BRD-KEY>` means *the key of the package the document belongs to*. Read the other way, every document takes this run's key, the bundle yields one partition, and a prerequisite's `[CG#7]` resolves against this package's `[CG#7]` — the wrong finding, and the check goes green: the flattened-bundle silent pass this partition was chosen to remove, one level down. The partition key is resolved against the key set the run already holds — this run's key plus every prerequisite key Phase 2 carried — never parsed out of a filename. **Vacuity guard:** a bundle into which a prerequisite package was copied and which nonetheless partitions to one is a collapsed bundle, not a clean one, so a prerequisite key answering to no partition is a `BRD_PACKAGE_DEAD_CITATION` — a relation that comes up empty fails rather than passes. **Amended after the fix-wave re-review: the guard is scoped to a prerequisite whose package was actually copied in**, which is what the opening clause already said and what the operative sentence, written unconditioned, then dropped. Phase 2 carries a key on two branches with nothing to copy in — *BRD not found* and *no package on file; nothing to copy in* — and both are ordinary and non-stopping, so the unconditioned reading stopped two correct runs; the discriminator is Phase 2's own carry, *whether a package of its own was found*. A prerequisite with no package copied in contributes no partition and is not a defect, which is the same state §4's structured-field rule discharges from the other side.
...
**Amended again after the fix-wave re-review: that was written as an instance, and the reasoning licenses a rule.** `conditional_on` is not the only structured field bearing an identifier that reaches the bundle, and relation 1 as first amended still stopped on every other one. The rule is that a reference carried by a **structured field whose format another authority fixes** is a qualified cross-package reference wherever that authority defines the field to name another BRD's record — §6 reads what the tree writes, and the fields' owners are the authorities, not §6. The fields that exist today are **derived from those authorities rather than kept as a closed list in §6** — `prerequisite` (`workflows-core:grounding-format` §2, §5), `resolved-by: [CG#n]` and a defect entry's `[BR#n]` references including a `conflict`/`duplicate` counterpart (`brd-format.md` §3, §4), and `superseded-by: [BR#n]` with an orphan row's own `id` (`coverage-ledger-format.md` §2, §3) — and a new such field is that authority's to declare. `prerequisite` needs one thing more: it fixes no spelling, so it can resolve **wrongly** rather than merely failing, and an unqualified value is therefore reported rather than silently resolved into this package's corpus — relation 2's disposition for a multi-id claim, applied for the same reason.
...
- **A corpus every one of whose records is malformed the same way**, added after the fix-wave re-review. The empty-versus-unreadable test is drawn on record-shaped content, so a file holding nothing recognisable as a record reads as empty and passes, reproducing the outcome `BRD_PACKAGE_CORPUS_UNREADABLE` exists to prevent. It is acceptable and not silent: the run still stops, because every reference into that corpus fails relation 1 as a dead citation — what is lost is the diagnosis, not the refusal. A test tighter than the parse it adjudicates would be undecidable by an agent whose parser has just failed.
