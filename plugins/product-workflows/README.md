...
| PA *(BRD route — grounding)* | [`/prd-ground`](docs/commands/prd-ground.md) | Ground each carved slice's claims against mounted code and design repos — mandatory, PM-initiated, PA/Dev-executed. A root is never ground; the slice's `/brd-split` re-run gates on these findings. |
