...
- **`gh` + `gh auth login`** *(recommended)* — lets the family open the pull request it drafts: `/document` offers a `gh pr create` for the docs branch it wrote, and every phase handoff opens one in the specs repo. It buys nothing for diff reading, which is pure local `git` on any host.
