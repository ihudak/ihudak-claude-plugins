...
5. Start the command with Bash `run_in_background`. As shipped, with its `{port}` token replaced by the port it serves on, and nothing else in it rewritten, from the repository's git top level, where the profile lives (§8.5) (`/docs-serve` Phase 4).
