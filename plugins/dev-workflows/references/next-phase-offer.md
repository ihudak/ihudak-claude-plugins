...
**Where this rule applies: every next-step offer this plugin prints.** The `<merge-clause>` placeholder is the convention the six `/dev-workflows:brd-*` commands write their offers to, and an offer added to that route carries it. It is equally the convention of the four older pipeline offers, converted after the route shipped: `/dev-workflows:create-prd`'s and `/dev-workflows:update-prd`'s *Next steps* phases, `/dev-workflows:create-ard`'s *Next-step offer (adaptive)* phase, and `/dev-workflows:specify`'s `### Next step`. Three of those four hardcoded "once the pull request above is merged" on runs that reach outcomes opening no pull request; the fourth, `/dev-workflows:update-prd`, named two downstream commands that gate this run's own PRD and stated no wait at all. No offer in this plugin now names a downstream gate its own run feeds and states the wait unconditionally, and an offer added anywhere carries the placeholder.
