...
| PA *(BRD route — grounding)* | [`/brd-ground`](docs/commands/brd-ground.md) | Ground each carved slice's requirement claims against the mounted code and design repos — mandatory, PM-initiated, PA/Dev-executed. A root is never ground; its `/brd-split` gates on these findings. |
