...
(`$SPECS_PATH/specifications/<jira_key>-*/`), then glob it for `specification.md` and `design.md`. No match there → apply `${CLAUDE_PLUGIN_ROOT}/references/addressing.md` §7's one-level-deep fallback before concluding none exists; it is reached only on a flat miss, so a flat key resolves exactly as it did before.
