...
**When it points somewhere unreadable.** For **grounding**, every miss — unset, missing, unreadable, or no markdown file found — is a **silent, non-blocking skip**, never an error, and the grounding path never writes into `$DOCS_PATH` under any circumstance. As a **write target** nothing is silent: every ladder reports which rung answered, `/docs-init` never passes over a `$DOCS_PATH` that carries a docs signal without reporting it and pointing at `/docs-profile`, and a directory resolved there is written to as any other docs repository would be. The `/document` discovery hint is a write-target *candidate* only in that same sense.
