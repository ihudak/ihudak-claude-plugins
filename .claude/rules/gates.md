...
Adding a command, agent, reference file, hook, skill, or environment variable without its docs entry fails the build.
...
No page under `docs/` names the marketplace this plugin ships from or the repository that contains it, with the token set derived from the repo-root README's own install block so a fork re-derives its own. **Identity quarantine:** no page under `docs/` may name a marketplace or a container repo — `getting-started.md` is the single sanctioned exception, pinned by check 7, which is why it carries the install commands inline instead of linking out. Check 10 enforces it. The check covers the marketplace and the containing repository and nothing else — a dev-environment or container-image repository a page might name is outside it, because nothing in the tree marks which third-party slug that is. Check 10 matches its tokens on word boundaries, not as substrings. ([why](../../docs/maintainers/rationale.md#check-10))
...
**Check 13 gates vendor neutrality**: no **text file** under any plugin in `PLUGIN_RELS` — `dev-workflows`, `docs-workflows`, `product-workflows`, `workflows-core` and `guideline-reviewers` today, plus `CLAUDE.md`, `.claude/rules/*.md` and `docs/maintainers/*.md`, which are scanned alongside each of them (CHANGELOG.md excepted, as history; binaries never reach it) — may write a tracker's name unless the line — or the fence opening the block it sits in — carries `<!-- vendor-token-ok: <why> -->`. The sanctioned markers are audited by kind in the check's own comment — **15 marked lines across 8 files** as this is written — and re-derived with `grep -rn --exclude=CHANGELOG.md 'vendor-token-ok:' plugins CLAUDE.md .claude/rules docs/maintainers`, never adjusted as a number. It excludes the changelog by *filename*, which is not cosmetic — the older `| grep -v '/CHANGELOG.md:'` form filtered on line content and so silently dropped from its own audit any line quoting the filter.
...
**What check 13 cannot see:** it matches names, so a tracker-shaped *concept* under a neutral name — a `team:` field on an Epic, an import ladder — is invisible to it; that half is review's. Its fence rule is proven by a **pair** of selftest cases, marked and unmarked: an implementation that simply skipped fenced code would pass the marked one for the wrong reason, and templates and handoff blocks are exactly where a tracker-shaped field would hide. **Widen the scope, not the count, when the corpus moves.**
