...
Every one of the 33 agents above is dispatched by at least one command — none has an empty **Used by** cell. Two of them, `docs-grounder` and `vault-prior-art-finder`, are dispatched indirectly: their calling commands invoke a named procedure (`dispatch-docs-grounder`, `dispatch-prior-art-finder`) defined in the agent's own governing reference file rather than writing `subagent_type: "dev-workflows:<name>"` inline, but that procedure resolves to exactly the `subagent_type` named above, so the derivation counts it as a real dispatch.
