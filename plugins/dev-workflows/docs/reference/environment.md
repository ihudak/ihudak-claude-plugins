...
**When unset.** `/design` and `/ready` — the two commands here whose whole job is to read or write a PRD/Epic artifact — stop immediately, name `SPECS_PATH` explicitly, and offer `choices: ["Set SPECS_PATH (enter the path)", "Cancel"]`. `/implement` does the same on a `<KEY>` address, checking the variable before it resolves the key; an `@<path>` address and a direct prompt need no specs tree and run on (below). The same gate applies to `docs-workflows`' `/document` in keyed mode and `/release-notes`, each on a `<KEY>` address, to the companion `product-workflows` plugin's `/create-prd`, `/update-prd`, `/create-ard`, `/specify`, `/prd-ground`, `/prd-proposal`, `/idea`, and every `/brd-*` command (`/brd-proposal` included — it stops on `SPECS_PATH` the way the route's five do), and to `workflows-core`'s `/frames`; `grep -lE 'Set SPECS_PATH|Required path environment variable unset' plugins/*/commands/*.md`, run from the repository root, re-derives the set. `product-workflows`' `/epics` carries no such stop and turns on its address form instead: a key resolves against the specs tree, finds no folder and stops with `EPICS_NOT_FOUND`, while an `@<path>` address still resolves and the ARD and nested-folder steps that need `$SPECS_PATH` skip. On an `@<path>` address `/implement` still resolves the folder and writes its `implementation.md` record into the folder it names; its `specs-preflight` and in-scope spec/design gate self-gate on `$SPECS_PATH` instead of stopping the whole run, and a direct-prompt run resolves no specs tree at all and is unaffected either way; `/vuln` and `/upgrade` likewise run `specs-preflight` and `commit-artifacts` silently when `$SPECS_PATH` is unset, simply skipping the session-bookkeeping writes those steps would otherwise make. No command silently substitutes the current working directory or any other path.
...
**Resolution.** `${REPOS_PATH:-/workspace}`, read fresh by each command that scans code — there is no persisted override once a run ends. It may be a single directory or a colon-separated list, and every repo-scanning command that offers a choice presents the resolved default first: `choices: ["Use $REPOS_PATH (default /workspace) (Recommended)", "Use a different path (you'll be prompted)", "Cancel"]`. Where a command resolves a repo it was handed the slug of, the match is by `git remote get-url origin` slug, **never by directory name**, so a clone renamed on disk is still found as long as its `origin` remote is intact. Every such command builds the same slug→clone map, this plugin's `/design` and `/ready` among them; `grep -l 'slug→clone map' plugins/*/commands/*.md`, run from the repository root, is the authority on the set, and no list is kept here because the one that was went stale naming three of the seven that do it. Five commands instead discover repos to offer you — `/product-workflows:create-ard` and `/product-workflows:idea`, and `docs-workflows`' `/docs-init`, `/docs-brand` and `/docs-audit` (`/docs-brand` only where neither its `--from` nor the docs profile's recorded `source_repos[]` settles the repository, `/docs-audit` only where `source_repos[]` does not settle the set): each lists the top-level directories under `$REPOS_PATH` (the two `product-workflows` commands attaching the slug as identity) and proposes a candidate set you confirm — no directory name is matched on that path, so a rename changes the name a clone is offered under, never whether it is offered. `/implement`, `/upgrade`, and `/vuln` work directly in the code repository the run is invoked against, so neither discovery mode applies to them.
