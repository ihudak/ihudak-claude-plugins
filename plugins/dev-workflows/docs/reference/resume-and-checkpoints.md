...
- **Suggested session name:** <PRD-ID>-<slug>-<role>   (omit this line when no PRD-Key exists yet — never the case for this plugin's own five commands, each of which either resolves a key or skips `resume.md` entirely; the companion `product-workflows` plugin's `/idea` is the family's own example)
