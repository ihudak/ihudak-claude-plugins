...
**The rule was found by the defect, twice, not derived and then applied.** `/brd-split`'s design gate and `/brd-package`'s round gate both carried that sentence and both then stopped on a file they believed was merged and had not checked. Both now execute `require-on-main` on what their stop actually depends on, and both appear in §3.4.
