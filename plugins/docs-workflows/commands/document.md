...
- **`status: OK` or `PARTIAL` with `gaps` entries** — for each gap, act on its `recommended_action`:
  - `"ask user"` → prompt inline **before** showing the checklist-approval choice. Free-text prompt scoped to the gap; feed the answer back to the planner via a single re-invocation (pass the user's answer as an additional `gap_resolution` field in the brief — `[{gap, answer}]`, one entry per answered gap, the shape `doc-planner`'s input contract gives it). If the user declines, fall back to `"mark TODO in draft"`, and keep that fallback: a declined gap has no entry in `gap_resolution`, so the re-invoked planner returns it again as `"ask user"`, and it is not asked about again — it stays `"mark TODO in draft"`.
