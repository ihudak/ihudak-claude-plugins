...
`dev-workflows` bundles 105 files under `references/` — 43 top-level markdown files, `cost-prices.yaml`, one file under `model-routing/`, and six bundled subtrees. This page enumerates every file a command or agent actually cites by name (45 of the 105 — the 43 top-level files plus `cost-prices.yaml` plus `model-routing/classification.md`), grouped by concern below, then counts the six subtrees rather than listing each file inside them. The arithmetic: 43 + 1 + 1 = 45 named individually, plus 24 + 11 + 10 + 5 + 3 + 2 = 55 markdown pages counted (not enumerated) across the six subtrees — 45 + 55 = 100 accounted for, against 105 files on disk. The remaining five are non-markdown data or templates inside those same subtrees, deliberately not listed as reference pages: `api-guidelines/template/openapi-template.yaml`, `api-guidelines/spectral/ruleset.yaml`, `docs-profiles/docs-profile.default.yml`, `docs-profiles/default-owners.txt`, and `guidelines/check_guidelines.py` — a template, a Spectral ruleset, a defaults file, an owners list, and a lint script, none of them prose a reader would open. The 55-file subtree figures below are markdown-page counts specifically; the five files above already sit inside those same subtrees and are not part of that count, so nobody should later "correct" a subtree figure by adding them back in.
