...
1. **Vale via `.vale.ini`** — if a `.vale.ini` exists in one of those directories — from `site_root` (where passed) up to `repo_root`, the nearest first — run `(cd "<the directory holding the first one found>" && vale --output=JSON <files>)`. Vale looks for its `.vale.ini` in the directory it runs in and then in each directory above it, uses the first it finds, and never looks beside the files. Run from the directory holding the site's `.vale.ini`, it reads that one. Run from a directory outside that one's tree — the session's, say — it reads the first `.vale.ini` at or above that directory instead, which may be another repository's; where there is none, it uses the user's global configuration, or, without one, stops with `E100 [.vale.ini not found]`. Parse the JSON into finding records. Set `primary_linter: vale`. **On non-zero exit / missing binary → record the attempt in `primary_attempts` and continue to step 2.**
