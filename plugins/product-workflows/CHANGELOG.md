...
`/brd-package` part 9 quotes every self-review finding the operator disposes `accepted-risk` in the reviewer's own words, and the plugin-free scan stops the run on any `§`, command or agent name, or working filename in the finished prompt. `brd-package-reviewer`'s findings routinely cited a plugin reference's section, a working filename or a command — in one measured run, 9 of 12 findings carried such a token — and nothing may reword an attack, so an accepted finding carrying one stopped the run with no remedy the text allowed. **Population: every `/brd-package` run on 3.6.x or 3.7.0 whose operator accepted a finding carrying such a token.** The reviewer now writes each finding's target, attack and what would settle it in words a customer can read without this plugin — the test it applies stated in words, a package document named by what it is, a passage of the customer's cited with its captured path — so an accepted finding passes the scan as quoted, and the command still never changes an attack's words.
...
**Re-running `/product-workflows:brd-intake <BRD-KEY> @<brd-file>` over the same folder captures them.** The copy is additive and the inventory's reconciliation keeps every existing id, so a row drawn from a file captured this time takes the next id after the highest in use; every defect already logged keeps its id too (below). **Ids are kept; wording may change**: a row the reconciliation matches keeps its id and takes the wording this read returned, so its `text` can differ from the earlier intake's even over an unchanged document. The re-run is not free where the BRD has already been split: Phase 0 step 7 names every disposition the ledger holds and asks first, because the re-run rewrites every ledger row to `unallocated` and `/brd-split`'s walk has to be taken again.
...
- **Files from outside the folder land in `brd/source-external/`**, at their basename under `idea-format.md`'s collision rule, immutable exactly as `brd/source/` is; `brd/brd-link-log.md` gains a second table mapping every captured link that does not resolve as written — a wikilink, an outside file, an absolute path — to its copy, and never rewrites the customer's link.
...
- **A correct `<KEY> @<file>` invocation read as a path that resolves to nothing.** Phase 1 classified the argument with only the flags removed, so the Phase 0 key token and the `@` were still in it, and a literal reading lands on Case B's *Re-enter the path* on a run with nothing wrong. Phase 1 now removes the key token and every flag with its value, and a leading `@` marks a file whose path is the text after it — the path rule 1 tests and Phase 2 hands `idea-reader`, whose input now says so. **Population: every `/idea` run over a markdown source.**
