...
2. **Validate the resolved repo is a writeable git work tree.** `git -C <repo> rev-parse --is-inside-work-tree` must print `true`; if it errors or prints anything else, stop: `DOCS_BRAND_NOT_A_GIT_WORKTREE: <repo> is not inside a git work tree.` `test -w <repo>` must succeed; if not, stop: `DOCS_BRAND_REPO_NOT_WRITEABLE: <repo> is not writeable.` Resolve and record the git root as `<repo-root>` (`git -C <repo> rev-parse --show-toplevel`); all later reads and writes in this run are relative to it — the profile's among them, since `<repo-root>/.dev-workflows/docs-profile.yml` is its one home (`${CLAUDE_PLUGIN_ROOT}/references/docs-profiles/docs-profile-schema.md`, **Where the profile lives**).

3. **Validate the repo has a Material theme to brand.** For every MkDocs config that exists among `mkdocs.yml` and `mkdocs.internal.yml`, read its **effective** `theme.name`: where the config carries `INHERIT: <parent>` and sets no `theme.name` of its own, it takes its parent's value, followed up the `INHERIT` chain to the config that inherits nothing — MkDocs' own merge, in which a child's keys override its parent's and an absent key is inherited. Each effective value must read `material`. **Read the literal key and this command refuses the repository `/docs-workflows:docs-init` just scaffolded**: that command's `mkdocs.internal.yml` is `INHERIT: mkdocs.yml` with no `theme:` block of its own, by design (`${CLAUDE_PLUGIN_ROOT}/references/docs-workflow/scaffold-tree.md` §6), and its Phase 5 runs this command `--inline` on exactly that tree. This is a config-file test, not a branch on the profile's `generator` field — see the D9 note below. Neither config present, or a present config whose effective `theme.name` reads anything but `material` → stop: `DOCS_BRAND_NOT_MKDOCS: <repo>'s MkDocs config does not resolve to theme: name: material (checked mkdocs.yml, mkdocs.internal.yml, following INHERIT). /docs-workflows:docs-brand writes Material-only palette options and --md-* CSS variable names — scaffold a Material site with /docs-workflows:docs-init first, or point this run at one that already has one.`
...
2. **Else the resolved docs-profile's recorded source-repo set** (`<repo-root>/.dev-workflows/docs-profile.yml`, Phase 0 step 2, when it exists). No shipped profile field records this today, so this rung is currently always a pass-through — it is named here so a later profile field is honoured without a second edit to this command.
