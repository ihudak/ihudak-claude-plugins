...
| `/brd-package` | each `interview/round-<N>.md` | **stops** — `BRD_PACKAGE_ROUND_UNSETTLED` on a round holding a deferred, needs-grounding or untagged question; `BRD_PACKAGE_NO_ROUNDS` on row F where `decisions.md` records decisions and no round record is on a ref. Never optional: the package is assembled from questions the rounds settled, and step 7 refuses a round that settled none. It gates rather than inheriting step 6's single-commit implication, per §4.0's stopping-reader rule |
...
| `brd/brd-inventory.md`, `brd-link.md`, `grounding/baselines.md`, `brd/source/*`, `brd/brd-defect-log.md`, `interview/customer-questions.md`, `self-review-<YYYYMMDD>.md`, `customer-delivery-note-<YYYYMMDD>.md`, `bundle-<YYYYMMDD>/`, `reconciliation-<YYYYMMDD>.md`, `prd-seed.md`, `ard-seed.md`, `spec-seed.md`, `/idea`'s vendored `attachments/*` and `design/idea-sources/*` | advisory | each is read by a named consumer and gated by none: the inventory by `/brd-ground` and by three row-F message splits; `brd-link.md` by BRD-route detection in `/create-prd`, `/create-ard` and `/specify`, by `/epics` step 1a and by `/brd-split` step 5; `baselines.md` by `/brd-ground` Phase 3 on a re-run; the intake and package artifacts by `/brd-package` and `/brd-reconcile`; the three seeds by their one altitude consumer each; `/idea`'s vendored sources by whatever later reads the folder |
| `slices.md`, `/specify`'s `_session.md` / `_glossary.md` / rendered `.html`, `/design`'s `_design-session.md` / `_design-glossary.md`, `/update-prd`'s archived revision | unread | handed off, and no reader found for any of them. **Listed rather than omitted**: an artifact absent from this table is unclassified, and a producer that reads absence as "unread" is making the mistake the frame-set index row records |
...
**A reader that stops on an artifact gates that artifact.** A stop is only as reliable as the ref it reads: a command that refuses a state it found in the *worktree* can be satisfied by a file nobody else can see, so the refusal protects the operator who ran the producer and no one else. This is not a general principle someone inferred — it is the same defect found twice in two increments, in `/brd-split`'s design gate and in `/brd-package`'s round gate, both of which inherited a sibling's `require-on-main` through a "one handoff, one commit" implication that `/brd-ground --no-code` had already falsified. Both now gate what they stop on, and both appear in §3.4. **The consequence for this table**: a stopping reader puts its artifact in **gated**, never in **advisory** — the advisory parenthetical says "no command stops on this", and an artifact something stops on cannot honestly carry it.
...
**unread** — nothing gates it and nothing reads it. **No artifact in the register above is currently in this class.** The frame-set index was its only member until §4.0's verification found `design-grounder` reading it. The class is not empty in the tree — `slices.md` is handed off and no reader was found for it, and so are `/specify`'s `_session.md` and `_glossary.md` — it is empty *in this table*, because those artifacts have not been classified here at all. A producer reaching for this array is making the claim the frame-set index failed: that it looked for a reader and there is none. Look, before selecting it:
