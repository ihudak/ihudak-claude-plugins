...
- **Somewhere to scaffold into** — resolved by `resolve-scaffold-target` (`docs-workflow/repo-resolution.md` §2), which is the **inverted** form of the ladder `/docs-profile`, `/docs-brand`, `/docs-serve` and `/docs-audit` use (`/document` has a ladder of its own): the given path, else `$DOCS_PATH` *when it is absent or carries no documentation signal*, else the working directory. Every other command in this family accepts a directory because it already looks like a documentation repository; this one accepts it because it does not. The rung that answered is always reported.
