...
1. **Create feature branch**
   ...
   - **Where the branch starts.** Resolve `<base>`, the default branch's name, by `${CLAUDE_PLUGIN_ROOT}/references/code-handoff.md` §2.8 — the ladder steps 6.5 and 7.5 resolve it with, so the branch cut here is the base step 7.5's pull request targets. Never judge by a list of names: a local `develop` in a repository whose default is `main` is a branch like any other.
     ...
     - **HEAD is not on `<base>`** (it prints another name, or nothing on a detached HEAD) → list the commits HEAD carries that the base does not: `git log origin/<base>..HEAD --oneline`, or `git log <base>..HEAD --oneline` where `origin/<base>` does not exist (`git rev-parse --verify --quiet origin/<base> >/dev/null` fails). Non-empty output → ask, naming `<base>` and listing those commits, whether to branch from current position, branch from `<base>`, or cancel. Empty output with exit 0 → nothing to ask. A non-zero exit is a failed read, never "no ahead commits": keep its error rather than redirecting it away, show it, and ask the same question.
