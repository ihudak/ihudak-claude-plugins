...
Phase 5 step 3's handoff presents `workflows-core:phase-handoff` §4.3's **advisory** array — correct, and unchanged — but justified it with *"The gated array would promise a downstream stop that cannot happen here."* Since §4.1's clause list split the gated class in two, that phrase identifies two arrays, and the **gated — falling back** one promises no stop at all: its parenthetical is *"the next phase does not stop on this — it reports the artifact as un-landed and proceeds from the resolved folder."* So the stated reason is false of half of what it denotes, and the real reason is structural — §3.4 names no row for `_readiness.md`, so **neither** gated array is available. The sentence now takes the wording the same delta already gave `/frames`, an artifact in the identical advisory, no-§3.4-row position, which is the repo's own standard for this: if the `/frames` rewrite was necessary, the untouched twin was a defect. `phase-handoff.md` §5 rule 4 states the ambiguity as a fact of the tree — *"'the gated variant' identifies two"* — and two sibling sites in `product-workflows` that named the class loosely are aligned in the same change. Nothing misfires at run time; only the stated reason was wrong. **Ungated** — no check reads a command's class justification — verified by mutation.
