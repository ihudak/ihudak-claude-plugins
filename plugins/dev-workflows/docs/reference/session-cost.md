...
Twenty-two commands emit a cost entry: `/idea`, `/create-prd`, `/update-prd`, `/create-ard`, `/specify`, `/epics`, `/design`, `/implement`, `/ready`, `/document` (both its keyed and direct modes), `/release-notes`, `/frames`, `/brd-intake`, `/brd-ground`, `/brd-split`, `/brd-interview`, `/brd-package`, `/brd-reconcile`, `/prompt`, `/feedback`, `/prompt-brainstorm`, and `/prompt-grill-me`. Sixteen of those pass a fixed `phase`/`role` pair baked into the command itself; six infer it. `/frames` infers from the resolved folder's own `kind` — a BRD folder's frame set is `brd-to-prd`/`pm` spend, a PRD or Epic folder's is `prd-creation`/`pm`. The last two do not write their own entry — they cede the session before they could, so the entry is written for them by the next cost-emitting run, as **Spend a command cannot measure itself** below describes. `/release-notes` passes `phase: inferred, role: inferred` and lets the cost phase resolve the real values by checking whether a `specification.md` or a `design.md` already exists under the PRD's specs directory — neither present means the PRD itself was only just created, so the entry is attributed to the PM's early run (`prd-creation` / `pm`); either one present means engineering work is already underway, so the entry is attributed to the dev's later documenting run (`documenting` / `dev`). Epic presence is deliberately excluded from that check, since a PRD can have drafted Epics while still entirely in product hands.

**What does not emit one, and why.** `/vuln`, `/upgrade`, and `/docs-profile` emit no cost entry. The number measures AI investment in a product increment, and none of those advances a PRD- or BRD-scoped artifact — a CVE bump or a library upgrade is noise against a PRD, and a metric that averages the two answers a question nobody asked. `/statusline` sets a configuration value rather than running a task. **Session *feedback* is a different question**: `/vuln` and `/upgrade` do emit that, because it records a gap in the plugin itself rather than the cost of the work.

`/prompt`, `/feedback`, `/prompt-brainstorm` and `/prompt-grill-me` infer differently: each resolves the **target command** whose output it is correcting or remarking on, and inherits that command's own fixed labels. A `/prompt` correcting a `/specify` output is priced as `specification`/`pe`, one correcting a `/design` output as `planning`/`dev` — the cost of fixing a phase's output belongs to that phase, so "what did specifying cost" includes the cost of making the spec right. Where no target resolves, where the target has no attribution row of its own, or where the target is itself a feedback command, the entry becomes [`plugin-feedback`](../roles-and-phases.md#plugin-feedback)/`n/a`. `n/a` is the absence of a role recorded rather than guessed; aggregation should treat it as unattributed rather than folding it into `dev`.

**Why the other four do not.** `/vuln` and `/upgrade` have no Product Requirements Document to attribute spend to and sit outside the pipeline entirely; `/docs-profile` and `/statusline` are one-shot utilities with no PRD either.
