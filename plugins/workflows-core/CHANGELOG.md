...
§4 said `/release-notes` reads *"only blocks appended since its last section was written"*, the file's last-written date the boundary. `release-notes.md` is one per PRD, and `docs-workflows` 1.2.2 lets an Epic address draft a note, so a note for one Epic moved every other Epic's boundary. §4 now takes the boundary per record: the blocks dated after the latest note covering that record, a note for the PRD covering every record and a note for an Epic that Epic's alone, each note's scope and date read off the scope line `docs-workflows`' `release-note-types.md` §1 now defines; a block dated the day of the note is read, since a date cannot order the two and a block read twice shows in the report where one skipped would not. And §4 said `/document` *"reads every block under the PRD"*, which an Epic-level run, reading its Epic's record alone, does not; it now reads every block under its scope. **Population: every `/release-notes` run with diff grounding on over a PRD carrying a note drafted at another scope, and every reader of §4.**
...
*The cap* says a command that adds its own option counts it against the four, and named `/specify` alone; `/implement` adds *"Implement one broad PRD-level slice instead"* too, and its array carried three Epic rows beside it and the remainder option. It now names both, each array carrying at most two Epic rows, and the sentence saying which run overflows at four names both. The *Exactly one* branch said the command auto-proceeds; `dev-workflows` 4.1.2's `/implement` asks between the Epic and a broad PRD-level slice where the PRD folder also holds a flat `specification.md`, and the branch now names that exception. **Population: every reader of the two sections, and every `/implement` run given a PRD address of four or more Epics, through `dev-workflows` 4.1.2.**
