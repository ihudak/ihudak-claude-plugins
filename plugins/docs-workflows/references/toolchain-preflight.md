...
1. **The resolved profile.** Take the **tool** of every `commands.*` value (including every
   ...
   `docs-profiles/render-verification.md` §1, `/document` Phase 6.5 Step 1 and `/docs-serve` Phase 4
   cite both. Where the profile records any `dev_servers.servers[]` entry, add `bash`, `curl` and
