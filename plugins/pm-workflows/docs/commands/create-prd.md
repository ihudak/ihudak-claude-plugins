...
- **`<KEY>`** — mandatory; absent or malformed stops the run with `CREATE_PRD_NEEDS_KEY`, naming the required `/pm-workflows:create-prd <KEY> @<idea.md>` form.
...
- **An existing PRD for `<KEY>`**, checked by a frontmatter glob in the feature folder. `/create-prd` is greenfield-only: if one is found, the run redirects to [`/pm-workflows:update-prd <KEY>`](update-prd.md) (or, with `--from-prd`, offers to update the existing PRD instead of seeding a fresh one).
...
- **Phase 3.5 — Prose style check** (`prose-style:prose-style-checker`, unconditional — `prose-style` is a declared dependency of `pm-workflows`, so there is no absent case). A quality enhancement, not a gate: MAJOR findings are fixed inline and the checker re-runs once; remaining MINOR/NIT findings are only reported.
...
```
/pm-workflows:create-prd PRODUCT-1234 @idea.md --hybrid
```
