...
| V8 | `grep -A3 'Read-only mount — ref stale or diverged' references/escalation-rules.md` | Entry present with 3 `choices:` — "Scan released code at `<ref>` (Recommended — cites shipped behavior)", "Cancel — refresh on the host …", "Other… (describe)" | PASS | Last option is `"Other… (describe)"`; Recommended option is reason-annotated. **Corrected 2026-08-13: original recorded 4 (including a now-unreachable "Scan the working tree …" option); re-derived at `7142976` = 3, that option removed — superseded same-day by 7142976.** |
