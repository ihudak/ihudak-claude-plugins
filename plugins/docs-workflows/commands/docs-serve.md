...
`--port <n>`, when given, overrides the selected entry's configured `port` for this run's own reachability checks (Phases 2 and 3 test `<n>`, not the profile's recorded port, and Phase 2 runs no pre-scan from it); it does not rewrite the entry's `command` — a command that hardcodes its own port (a baked-in `-a 0.0.0.0:8000`, say) is reported as such rather than silently overridden.
...
- **Another checkout, or another of this repo's servers** — the listener's checkout is established as another one, or the evidence names a server other than the selected one. Leave it running and record nothing, and name what holds the port in the Phase 6 report — another checkout by its top level. At the port being served it is a collision, Phase 3's case; in the pre-scan and in Phase 3's walk, pass over it.
- **Nothing names a server** — no listener is named, or its command line names none, and the page names none or does not apply:
  ...
  - where it holds **more than one**, report the port as answering as this docs site with its server **unconfirmed**, together with the listener's pid where the socket table named one; start nothing and record nothing — a guessed identity is exactly the entry a later `--stop` would act on — and name the two ways forward: stop that process by hand (its pid, where known), or re-run with `--port <n>` to serve the selected server on another port.
...
2. No living entry returned (a different session started it, the state was lost, or nothing has ever run) — probe the port directly, and judge it as above:
   - **It does not answer** — nothing is listening **there**, which is not yet proof the server is down: an earlier run's Phase 3 may have moved it further up and its record may be gone. So, on every profile, pre-scan the ports Phase 3 would walk (`port + 1` … `port + 20`), judging each one that answers exactly as above. Where the evidence finds the selected server, re-adopt it (the lowest such port); failing that, where a port answers as this docs site and nothing names its server, that outcome governs at the lowest such port — re-adopted on a one-server profile, unconfirmed on a multi-server one; pass over every other port. Only where the pre-scan does neither, proceed to Phase 4 at the configured port. A multi-server profile's common case — another of its servers on its own configured port, its record lost — is identified as that server and passed over, so only a site nothing can name still stops the run.
   - **It answers** — a living recorded entry of another server bound to it, an answer that is not this docs site, or evidence naming another checkout or another of this repo's servers, is Phase 3's case; otherwise the evidence decides, as above.

   **An explicit `--port <n>` runs no pre-scan.** The pre-scan infers a history — that an earlier collision moved a copy off this port — and an operator who names the port has overridden that inference. It is also what makes the unconfirmed report's `--port <n>` remedy work: a pre-scan from `<n>` would meet the same site nothing can name, and stop again. `--port` changes nothing else — this step and Phase 3 judge `<n>` and walk from it exactly as they judge a configured port.

## Phase 3 — Port collision

The selected port answers, but not as the selected server — it is not this docs site, or another of this repo's servers or another checkout holds it (Phase 2 step 2). Never fail on this and never reuse the port silently: walk forward from it (`port + 1`, `port + 2`, …, capped at 20 attempts) with the same probe Phase 2 used, judging each candidate the way Phase 2 judges the configured port:
...
- **Phase 2's evidence finds the selected server there, or names no server** — the selected server may already be running here, moved by an earlier run's Phase 3 whose record is gone. **Never walk past it to start a second copy**: what the evidence decides governs here exactly as at the configured port. Re-adopted, the report says it sits off the configured port, and its entry is keyed at this port with `requested_port` the port Phase 1 resolved; where nothing names a server on a profile with more than one, it is unconfirmed, and nothing is started or recorded.
...
Poll the port this run resolved (Phase 1, as shifted by Phase 3) up to `dev_servers.readiness_timeout_seconds` (default 120 when the profile omits it), on a short interval, using the same reachability probe Phase 2 used. Stop polling the moment it answers, read the pid of its listener from the socket table as Phase 2 names one, and proceed to Phase 6. **That pid is the one Phase 7 records**, because the process holding the port is the one `--stop` must end — **never a wrapper's `$!` or the Bash tool's background-task handle**. `run_in_background` returns a task handle, not a pid, and a wrapped command (the schema's own `pnpm cloud:start`, or an npm script) leaves the port held by a child that outlives its wrapper, so stopping the wrapper would leave the site serving. Where no listener is named, record the entry without a pid and say so, as Phase 2 does for a re-adopted server. A timeout is reported, not treated as failure — the started command and the elapsed wait are both in the report, so the operator can inspect a slow or wedged boot themselves rather than have a possibly-good server killed on a guess. No process holds the port yet, so there is no pid to read: record the entry without one and say so, in the same words. Once the server answers, a re-run of `/docs-serve` re-adopts it by Phase 2's evidence, with its listener's pid where the socket table names one.
...
- NEVER infer a server's identity from the port it answers on — only from evidence that names exactly one server: a living recorded entry, the listener's checkout and the command line it descends from, or, where the builds' site names differ, the home page's whole site name (Phase 2). Re-adopt only a server identified as the selected one, or the sole server of a one-server profile, and never a listener established as another checkout's — that one is left running and never recorded. Report a match as unconfirmed, starting nothing, only where nothing names a server on a profile with more than one (Phase 2 step 2, Phase 3)
- ALWAYS record the listener's pid, read from the socket table — never a wrapper's `$!` or a background-task handle — and NEVER report a server stopped while its port still answers as this docs site (Phase 5, `--stop`)
