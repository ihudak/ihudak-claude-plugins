...
- `DOCS_BRAND_NOT_MKDOCS` — the resolved repo carries neither `mkdocs.yml` nor `mkdocs.internal.yml`, or a config it does carry resolves to a theme other than Material (the effective `theme.name`, `INHERIT` followed, must be `material` in every config that exists). Scaffold one with `/docs-workflows:docs-init` first, or point this run at a repo that already runs Material.
