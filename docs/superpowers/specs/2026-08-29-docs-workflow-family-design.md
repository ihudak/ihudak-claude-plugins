...
6. **Poll for readiness** up to `dev_servers.readiness_timeout_seconds` (default 120), then print the URL. As shipped, what answers is judged by step 3's evidence and recorded only where step 3 would re-adopt it as the selected server — its listener in this checkout, named as that server — so a race lost to another checkout, to another of the repo's servers or to a foreign process records nothing and names what won the port; a timeout records its entry without a pid, the one entry that ever lacks one (`/docs-serve` Phase 5).
