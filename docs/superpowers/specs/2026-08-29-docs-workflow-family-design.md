...
5. Start the command with Bash `run_in_background`. As shipped, with its `{port}` token replaced by the port it serves on, and nothing else in it rewritten (`/docs-serve` Phase 4).
