...
**The comment records what the run read, never what it merely found.** The blocks and the scan are resolved in Phase 3, before any repository is a clone; the diffs are read in Phase 5, and the read set is settled there. So a repository you answer *Skip and continue without its refs* for, one no clone under `$REPOS_PATH` matched, and a ref `diff-summarizer` could not resolve all contribute nothing to the comment — the run reports them under **Not read**, and the next grounded run reads them again rather than skipping them forever on the strength of a note that never opened them.
