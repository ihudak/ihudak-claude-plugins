...
**Why the other ten do not.** `/vuln` and `/upgrade` have no Value Increment to attribute spend to and sit outside the pipeline entirely; `/docs-profile`, `/statusline`, and the two guideline reviewers are one-shot utilities with no VI either. The four feedback commands — `/feedback`, `/prompt`, `/prompt-brainstorm`, `/prompt-grill-me` — are the interesting case, and their reasons differ:
...
- **`/feedback` and `/prompt` have no such obstacle.** Both end with an ordinary terminal report phase, `/prompt` already resolves the `jira_key` it would need, and a keyless run is a solved problem — §9's pending file plus opportunistic reconciliation is exactly what `/idea` uses. `/feedback` is cheap enough that the omission barely matters; `/prompt` does real work in its Phase 2 and is a genuine gap rather than a decision. Nothing in the plugin documents a reason for either.
