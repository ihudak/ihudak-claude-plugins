...
- **The `docs grounding:` line was shown before anything had resolved it.** Phase 1 showed it "as resolved", while `resolve-docs-grounding` runs in Phase 2.5. The line is now shown in Phase 2.5, as that procedure returns, and the Flags paragraph points `--no-docs` there. **Population: every `/idea` run** — only under `--no-docs` was the line's value knowable at Phase 1.
