...
Every cost-emitting command passes a `phase` and a `role` label to the cost phase at the point it calls the shared entry point — this is what lets the maintainer later group spend by where in the lifecycle it was spent and who was doing the spending. [Roles and phases](../roles-and-phases.md) is the place that vocabulary actually lives — the eleven cost-attribution phases and the four roles, and which command emits each — so this page does not restate it. What belongs here is the mechanics of the assignment itself, not what the labels mean once assigned.

Fifteen commands emit a cost entry here: `/idea`, `/create-prd`, `/update-prd`, `/create-ard`, `/specify`, `/epics`, `/design`, `/implement`, `/ready`, `/brd-intake`, `/brd-ground`, `/brd-split`, `/brd-interview`, `/brd-package`, and `/brd-reconcile`. Every one of them passes a fixed `phase`/`role` pair baked into the command itself; none infers. The two commands that used to make this list, `/document` and `/release-notes`, ship in the companion `docs-workflows` plugin now and charge to phases this plugin owns — `/document` with a fixed pair, `/release-notes` by inference — so their spend still lands against the same PRD; that plugin's own session-cost page describes them.

**What does not emit one, and why.** `/vuln` and `/upgrade` emit no cost entry. The number measures AI investment in a product increment, and neither advances a PRD- or BRD-scoped artifact — a CVE bump or a library upgrade is noise against a PRD, and a metric that averages the two answers a question nobody asked. **Session *feedback* is a different question**: `/vuln` and `/upgrade` do emit that, because it records a gap in the plugin itself rather than the cost of the work.

**Five more emitters ship in `workflows-core`, and the total spend they represent is real.** `/feedback`, `/frames` and `/prompt` write their own entry; `/prompt-brainstorm` and `/prompt-grill-me` cede the session and defer theirs to a later run, as **Spend a command cannot measure itself** below describes. `/frames` infers from the resolved folder's own `kind` — a BRD folder's frame set is `brd-to-prd`/`pm` spend, a PRD or Epic folder's is `prd-creation`/`pm`. The other four resolve the **target command** whose output they are correcting or remarking on and inherit that command's own fixed labels, which is why they are described here at all: the phase they charge to is one of this plugin's. Their own page is `workflows-core`'s.
...
Two details in that sentence are doing real work. The ceded run is found **by name**, not by counting invocations — a window often contains commands that emit no cost entry at all (`/vuln`, `/upgrade`, `/workflows-core:statusline`, `/docs-workflows:docs-profile`), and an interrupted run leaves a mark too, so counting positions would shift every attribution by one and file a security run's spend under a product phase. And a command invocation is recognised by its whole envelope rather than by one tag, because Claude Code writes built-in and plugin commands in two different tag orders; matching only the built-in shape finds no plugin command at all. If a recorded intent matches no invocation, nothing is written for it — its spend stays with the replaying run, and the run says so rather than attaching it to a neighbour.

This matters because the error it removes was large, not small. A grill is expensive, and before the deferral existed its spend was not lost but **misattributed** — it rolled into the next command's bucket, so a long grill followed by `/implement` was priced as `implementation`/`dev`. What still goes unmeasured is a session where no cost-emitting command ever follows the grill; there the spend stays in the ordinary post-last-command tail, unattributed, exactly as it would for any other command.

**`/vuln` and `/upgrade` emit no cost attribution at all.** Neither has a Product Requirements Document to attribute spend to, and neither calls the cost entry point; `workflows-core:cost-emission` gives neither an attribution row, naming them only among the commands a correction has nothing to inherit from. Both commands do pass a field also named `phase` to their own fixer/executor subagents (`full`, `verify-resume`, `regression-resume`), but that is a completely different vocabulary belonging to the model-routing resume protocol, saying how much of a single command's own work must be re-executed after a review or a failed test — not where a run sits in the product lifecycle. The two vocabularies share nothing but the field name; see the closing note on [Roles and phases](../roles-and-phases.md#cost-attribution-phases) for the fuller distinction.
...
The dollar figure attached to a phase depends on which price table priced it — override the bundled default with `$DEV_WORKFLOWS_COST_PRICES`, a path to your own `models:`-keyed price file. That variable is read by the `workflows-core` plugin, which ships the price table and the emitter, and its environment page carries the exact resolution order and file shape. Setting it is never required — the shipped table covers every model the routing policy can reach.
...
Walking it field by field: `id` is stable and timestamp-suffixed, so a re-run never double-logs against it. `date` is when this entry's window ended — except on a deferred entry, which is dated when its run ceded the session, a moment near the *start* of the window it measures. And `command`/`phase`/`role`/`prd`/`epic` are the attribution fields from the section above. `plugin_version` pins which release of the plugin produced this entry. `duration_s` is wall-clock seconds, not token-processing time — it is the gap between when this command's window started (the previous dev-workflows command's checkpoint in this session, or the session's own start if this was the first) and when it ended; a large `duration_s` on a small `cost_computed_usd` usually just means a long-idle session, not an expensive one.
