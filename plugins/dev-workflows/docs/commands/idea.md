...
- **Links to other pages, two levels deep.** The source's own links, and the links on *those* pages. Depth 3 is never reached. Both syntaxes count — the `[[wikilink]]` form and the ordinary markdown link form — because a note written outside a vault uses the second, and a page reached only that way would otherwise be read by nothing and reported by nothing.
