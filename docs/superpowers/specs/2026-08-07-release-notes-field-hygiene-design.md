...
Three Jira dropdown values leak into the plugin as questions the user must answer twice:
...
All three are dropdowns the PM sets in Jira regardless. Answering them in the plugin does not remove
that work — it duplicates it. And on the re-import, all three come back in the VI frontmatter
anyway, which is where the plugin should have been reading them from all along.
...
> **Sourced or omitted — never asked, never guessed.**

If the Jira import carries a field, use it. If it does not, omit what depends on it and say so. Do not
ask the user for a value they will set in a Jira dropdown anyway, and do not invent one.
...
- **`Change type:` line** — removed from `combined_rendered`. With it go the `change_type_hint` and
  `authored_vi_fields` inputs, the `field: change_type` and `field: change_type_divergence` gaps, and
  Phase 6's four-way confirm prompt (`commands/release-notes.md:184-189`).
...
| `references/handoff/release-notes-writer.md` | Drop `context_label_hint`, `change_type_hint`, `authored_vi_fields`, `release_versions` multiplication, the `change_type` / `change_type_divergence` / `context_label` / `release_version` gaps; make `context_label` optional; add the conditional `fixes` shape. |
| `commands/create-vi.md` | Phase 3 step 1 (`:118`) — drop the three asks. |
| `commands/release-notes.md` | Phase 2 gate (`:100`); Phase 3 parse + `authored_vi_fields` resolution (`:130-140`); Phase 6 dispatch inputs (`:173-178`) and gap prompts (`:184-189`); Phase 8 report lines (`:239-241`, `:261`); Invariants (`:362-363`). |
...
4. `commands/release-notes.md` contains no `choices:` block naming a Change Type value.
5. An end-to-end `/release-notes` run on a VI whose import carries `release_notes_category` produces a
   draft with a correct `{{#context}}` line, no `Change type:` line, exactly one Summary block, and
   zero field questions.
6. An end-to-end run on a VI whose import lacks the category produces a draft with **no**
   `{{#context}}` line and still zero field questions.
