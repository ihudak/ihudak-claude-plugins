...
The report also lists every dev-server command the schema's field rules call a profile defect — one that binds only the loopback; one that detaches its server, `astro dev` without `ASTRO_DEV_BACKGROUND` set and `astro preview` without `ASTRO_PREVIEW_BACKGROUND` among them, since Astro 7 detaches its dev server wherever it detects an agent, and Astro 7.2 its preview server; and one that moves off a busy port instead of failing, as VitePress does without `--strictPort` and Astro does without `vite.server.strictPort` in its configuration, its command line having no such flag — each with the rule it breaks and how to fix it. It fixes none of them itself: which flag or setting a tool takes is the tool's business, so the command stays as detected and you edit it, or the tool's configuration, by hand.
