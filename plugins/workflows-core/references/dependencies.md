...
**`docs-workflows` hard-requires two: `workflows-core` and `prose-style`.** The first is the same structural requirement for the same reason — its six commands and eight agents read the shared corpus through the loader skill and dispatch core's agents natively. The second it declares exactly as `product-workflows` does: `prose-style` is a *marketplace sibling*, and declaring it converts a runtime probe into an install-time guarantee, which is why neither of the two carries an absent case for its style gate at all. `dev-workflows` has no tie to `prose-style` of either kind since `/epics`, `/create-prd` and `/update-prd` moved out.
