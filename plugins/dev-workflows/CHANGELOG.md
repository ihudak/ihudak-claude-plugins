...
**Update `workflows-core` with this plugin.** Every change below follows `workflows-core` 1.7.1: the first quotes an array its `phase-handoff.md` §4.3 changed, and a command presents that array verbatim, so the two must match; the other three rest on what its `addressing.md` §4 now reads — the order a folder's carrier is found in, which makes a BRD-route slice's kind `brd`.
...
Phase 0 step 1a validated `--claimed` against `workflow-states.md`'s ladder "for the resolved kind", and Phase 3(0) derived the phase from the same ladder. `workflows-core:addressing` §4 reads a slice's kind off its `brd-link.md` — `brd` — and `workflow-states.md` has only a PRD ladder and an Epic ladder, so on a BRD-route slice neither step had a ladder to use. Phase 0 step 1 now places the folder by its prefix — a `PRD-` folder on the PRD ladder, an `EPIC-` folder on the Epic ladder — and a folder with no kind prefix — a legacy one, or an unprefixed folder named by `@<path>` — by positive evidence: a resolved `kind: epic` is an Epic folder, a resolved `kind: prd` or a `brd-link.md` naming a `parent:` a PRD folder; step 1a and Phase 3(0) use what it placed. A `BRD-` container — or an unprefixed folder holding `coverage-ledger.md` or `brd/brd-inventory.md` and no `brd-link.md` naming a `parent:` — is on neither ladder, and now stops the run with `READY_BRD_NOT_SLICED`, naming a slice to check; before, nothing decided its altitude. The command page says both. **Population: every `/ready` run on a BRD-route slice, and every one addressed to a BRD container.**
