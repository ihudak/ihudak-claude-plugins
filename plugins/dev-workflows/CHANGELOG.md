...
- **`check-docs.sh --selftest` grew from 11 cases to 28** — the skills sub-check's second direction (a documented skill that does not exist) had no case, and the new check 8 gets three: an unattributed `emit-cost` call, a drifted role in §7, and a §7 row for a command that emits no fixed pair.
