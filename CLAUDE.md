...
`/update-prd`, `/create-ard`, `/specify`, `/design`, `/ready`) must load and follow this file at the
...
- Only `^(idea|prd|ard|spec|design|ready)/` branches are the plugin's to switch away from or delete; any other **named** branch is left alone and the artifacts are committed on it
