...
| Plugin | Description |
|--------|-------------|
| [dev-workflows](plugins/dev-workflows/) | `/impl` (dispatcher), `/impl:code`, `/impl:docs`, `/impl:jira:docs`, `/impl:jira:epics`, `/vuln`, `/upgrade`, `/api-guideline-reviewer`, `/guideline-reviewer` — Opus-backed planning, code review, product-docs review, Epic review, vulnerability remediation, dependency upgrades, API and UI guideline compliance — `/impl:code` accepts multi-source input (spec files, Jira ticket folders, multiple repos) and fans out a parallel scan for large briefs |
