...
**Update `workflows-core` with this plugin.** Every change below follows `workflows-core` 1.7.1: the first quotes an array its `phase-handoff.md` §4.3 changed, and a command presents that array verbatim, so the two must match; the others rest on what its `addressing.md` §4 now reads — the order a folder's carrier is found in, which makes a BRD-route slice's kind `brd` — and on §4.1, which now owns how a folder is placed at a level.
