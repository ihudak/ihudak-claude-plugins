...
- NEVER push the **code repo** directly to `main` / `master` — always use the dedicated fix branch (`agents/vuln-fixer.md`). This binds the code repo only: the specs-repo steps below push `$SPECS_PATH`'s bounded artifact paths to the specs repo's own branch (`${CLAUDE_PLUGIN_ROOT}/references/specs-repo-git.md` §3.4 / §4), and never force-push (§1 rule 4)
