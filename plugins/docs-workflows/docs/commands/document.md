...
- **Keyed mode (Mode A)** — the first token is a single positional address: a `<KEY>` matching `workflows-core:addressing` §1's grammar, or an `@<path>` naming a folder in the specs tree. The resolved folder supplies the specs this run documents from, and the PRD comes from the PRD folder — the resolved folder itself, or, where the address names an Epic folder, the folder above it, the Epic then scoping the repositories resolved, the diff summaries and the documentation plan to itself. A folder that resolves and holds no PRD takes a named stop, never the re-enter/cancel escalation an unresolved key gets, since re-entering a key that resolved cannot help. In Phase 0, `DOCUMENT_BRD_NOT_SLICED` stops on a BRD container, whose PRDs sit in its slices: it lists each slice, and offers `Enter a slice key` or `Cancel`, taking a typed key only where it is one of the slices listed and re-asking otherwise; a container with no slice yet is a plain stop naming `/product-workflows:brd-split`. `DOCUMENT_FOLDER_NOT_PLACED` stops on a folder the run cannot place at any level, naming what the folder carries and the remedy: give it a carrier, or, where it holds an `idea.md` and no `prd.md`, run `/product-workflows:create-prd`. In Phase 3, `DOCUMENT_NO_PRD` stops on a PRD folder with no `prd.md` and names `/product-workflows:create-prd`, or, for an Epic with no PRD folder above it, moving the Epic folder into one. A `prd.md` whose frontmatter asserts no `kind: prd` is still read as the PRD.
- **Direct mode (Mode B)** — everything else: a leading `@file` token, free-text prose, or a directory outside the specs tree. This is the **one-shot doc-edit** pipeline: apply a described change to existing pages with no address, and with no repo or ref resolution at all.
...
Eight subagents are dispatched, none shared by both modes except `docs-style-checker`, `workflows-core:doc-fixer`, and `workflows-core:impl-maintenance`. Keyed-mode-only: `diff-summarizer` (Phase 5, up to 4 concurrent per batch), `doc-location-finder` (Phase 5.5), `doc-planner` (Phase 5.7, pinned to the Opus `planning_model` chain), `doc-writer` (Phase 6.3, also Opus-pinned — the sole author of the written pages; the orchestrator prepares its handoff and commits its output, but never writes pages itself), and `doc-reviewer` (Phase 7, Opus-pinned by its own frontmatter). Shared by both modes: `docs-style-checker` (Phase 6.4 in keyed mode, Phase 3.5 in direct mode), `doc-fixer` (invoked from the style-check phase in either mode, plus Phase 7 in keyed mode), and `impl-maintenance` (Phase 8 in keyed mode, Phase 4 in direct mode, alongside three general-purpose maintenance agents in a single dispatch). Direct mode additionally dispatches a read-only general-purpose exploration agent at Phase 2A before drafting its plan — so even the lighter pipeline dispatches well over the two-subagent floor.

## What it needs

**Direct mode** needs the doc-edit description itself — an `@file` or free text — and expects the content to already be in the user's head or the file, not scattered across PRD sections and the diffs behind them; that's the boundary with keyed mode. It works on the repository it edits — the one holding the directory or `@file` it is given, or `cwd`'s where the description names no path — and its toolchain preflight, its checklist and its style check all read that repository, whichever one `cwd` sits in. Where the target sits in a site below that repository's top level — a monorepo's `website/` — the preflight and the style check look for the site's own `.vale.ini`, lockfile and lint configuration there first and then in each directory above it up to the top level, as keyed mode does. When the target is not a git repository, Phase 0 records all three of direct mode's gates and skips the checklist extraction, rather than stopping the run. Its toolchain preflight is sourced only from the repo's own config signals and documented `Prerequisites` — there is no profile in direct mode, since a profile is a keyed-mode/docs-repo concept.

**keyed mode** needs:
- **A resolved address** — a key or an `@<path>` naming a folder in the specs tree. A key needs `$SPECS_PATH` set, since the key is found by searching that tree: with the variable unset the run stops in Mode detection, naming it and offering to set it or cancel, before it resolves anything; an `@<path>` address names its folder directly and runs without the variable. Keyed mode never gates this PRD on being merged anywhere: nothing in the command calls `require-on-main`, so it reads the PRD folder directly regardless of what specs artifacts exist or where they live.
- **A writable docs repository**, resolved cwd-preferred: the git root of `cwd` when it carries a docs signal (a `package.json` script matching `*:start`, `*:build`, `*:lint` or `docs:*`; a `.docstack/` directory, `mkdocs.yml`, `docusaurus.config.js`, `antora.yml`, a Vale configuration file under any of the five names Vale reads (`.vale`, `_vale`, `vale.ini`, `.vale.ini`, `_vale.ini`) or `DOCUMENTATION-GUIDELINES.md`; or a `_snippets/` directory at any level), else the `$DOCS_PATH` hint (default `/workspace/docs`) when it carries a docs signal or an in-repo profile, else a docs repo discovered under `$REPOS_PATH` by signal (an in-repo profile, or a docs signal — never by repository name), else an explicit ask. Whichever answers, the run then works at that directory's git top level — a `$DOCS_PATH` or a typed path naming a site below it, such as a monorepo's `website/`, is taken up to it — because that is where the profile lives and where its paths and commands are rooted. A resolved path that fails `test -w` stops the run with `REPO_NOT_WRITEABLE`.
- **A docs-profile** — loaded in-repo from that top level, from the built-in default (which applies only to a repo shaped like the bundled worked example), or generated on demand by an inline `/docs-profile` run for any other repo that has none; a custom repo whose profiling run is cancelled stops with `PROFILE_REQUIRED`, and one whose profiling stops on its own error — a bootstrap branch an earlier run left behind — stops with that error. The inline run hands back the branch it cut and the commit it made, and the finish renames that branch and squashes onto that commit — it never looks either up. **A run that stops after that commit and before Phase 6.2 renames the branch** — a missing tool you cancelled on, a Cancel at a clarification, classification or plan prompt, or any later stop or Cancel before Phase 6.2 renames the branch — leaves the docs repository on that branch with the profile commit on it and nowhere else, so it ends by naming the branch, the commit and the base branch to switch back to, with the commands to keep or drop the profile: a leftover branch of that name stops the next run that profiles the repository inline.
...
- **Mounted repos under `$REPOS_PATH`**, matched by their `git remote get-url origin` slug to each in-scope `repo` slug the implementation record and the commit scan named. A slug with zero matches is never silently dropped — it is escalated once, in a single consolidated gate covering every missing repo in the run, not one prompt per slug.

## What it produces

**Direct mode** produces edits to the target file(s) — in the repository the argument resolves to, which is not necessarily `cwd` — left **uncommitted** — the user manages git manually for a doc edit, and this mode creates no branch and no commit, ever. It always runs its four Phase 4 maintenance agents (documentation, knowledge base, instructions, `impl-maintenance`) and, on any accepted proposal, applies it uncommitted alongside the doc edit. It ends with a Phase 5 report; there is no next-step recommendation printed, since a one-shot edit has no PRD to hand off.

**keyed mode** produces one or more product-doc pages, written by `doc-writer` into the resolved docs repo at the confirmed write targets. When the write context is `docs_repo` (or a `non_docs_repo` the user confirmed), Phase 6.2 creates a feature branch before writing, the orchestrator commits what `doc-writer` wrote into the docs repo, and Phase 8.5 squashes the run into clean history and offers a push plus pull request; for `obsidian` or `plain_dir` contexts nothing is ever branched or committed there. When Phase 5.8 records a `document-as-spec`, `skip-and-report`, or qualifying `document-as-code` decision, a `<KEY>-implementation-gaps.md` bug-report draft is also written to the resolved PRD folder. A new screenshot the pages need goes into the PRD folder's `Doc screenshots/` subfolder — the default staging directory for an image uploaded by hand, too — and never into its `attachments/`, which holds only the sources a run vendored; an image already under `attachments/` is still offered as a Phase 5.6 candidate. The run ends with a Phase 9 Final Report (PRD folder summary, repos analysed, refs in scope, output files, branch, the verification-gates table, render verification, review verdict and triage, the four maintenance summaries, screenshots staged for manual upload, implementation gaps, deferred items) followed by Phase 10 follow-ups and Phase 11 session cost.
...
`build_check` runs every build the profile records in `builds[]` — the profile `/docs-init` writes records each of its builds there — and otherwise the build command of each space the run wrote into. Each build is reported on its own, so a failure names the build that failed, and `doc-fixer` is handed that build's output. Where a build will not run for an environmental reason — its tool missing, say, or a missing `.docstack` shim — the gate's registered fallback still runs when its own tools are present: the smoke check's boot of that build's own servers, the ones that publish what it compiles (for a `builds[]` entry the server of the same `visibility`, for a space's build that space's servers, for the flat build every server the check boots). `build_check` is recorded `DEGRADED` once one of those servers comes up; another build's server is no proof for it. You are asked whether to install the tool, proceed without the check, or cancel only where that fallback cannot run or brings none of those servers up, and declining the smoke check then declines the build check with it. `render_smoke_check` reports a 404 on an affected page with its URL and leaves the page for you to check by hand, never sending it to `doc-fixer`: the page's URL is derived, so a 404 cannot tell a wrong URL from a missing page. A server error (5xx) is a render defect, handled like a build's content failure. The smoke check starts and stops its servers under `bash`, probes ports with `curl` and reads process groups — from `/proc` on Linux, which needs nothing installed, and with `ps` elsewhere — so the toolchain preflight requires `bash` and `curl`, and off Linux `ps`, wherever the profile records a dev server; where one cannot run, the check does not start rather than reading a missing tool as a port that answers: every affected page goes to the manual pages-to-visit table, the gate's fallback, and `render_smoke_check` is recorded `DEGRADED` with the tool named. A server whose own command's tool is missing — a `pnpm`, `npm` or `yarn` whose dependencies are not installed counting as missing — is never booted either: its pages go to the same table and the gate is recorded `DEGRADED`, rather than the check waiting out a readiness timeout for a server that could never start.
...
Keyed mode also runs a Phase 5.8 **discrepancy analysis** whenever the PRD narrative, the (optional) spec, and the code disagree — an analysis table is presented and the user decides per claim or in one batch which phrasing to document; nothing is ever auto-resolved. It makes zero direct API calls: a keyed run's `refs[]` are resolved by pure local `git` against clones under `$REPOS_PATH`, matched by `git remote get-url origin` slug, and nothing here calls a REST API over HTTPS — the one forge command in the run is the `gh pr create` Phase 8.5 offers for the run's **own** pull request, and `gh` wraps the API rather than calling it directly. And its branch policy classifies the **resolved `docs_repo_path`**, not `cwd` — Phase 6.2 branches only when that classification lands on `docs_repo` or a user-confirmed `non_docs_repo`.
...
The run resolves the docs repo and profile, asks for output path / refresh policy / screenshot intent, classifies (typically `SIGNIFICANT`), reads the PRD folder, resolves the repos the implementation record names, determines the applicable space(s), summarises the diffs in parallel, locates write targets, reviews images, plans the documentation, resolves any PRD/spec/code discrepancy, writes the pages via `doc-writer`, runs the style check and render verification, gates on `doc-reviewer`, and closes with the four maintenance agents plus the Final Report — recommending `/docs-workflows:release-notes <PRD>` next, once every Epic is documented.

A same-session typo fix on an unrelated page runs direct mode instead — `/docs-workflows:document @note.md` or a free-text description — which skips address and repo resolution entirely, explores the target file and its neighbours, plans, edits, runs the mandatory style check, and ends with a report; no branch, no commit, and no reviewer gate.

## See also

- Roles and phases — what the `dev` role owns, including why both of this command's modes emit the same fixed `documenting`/`dev` cost attribution. `workflows-core:cost-emission` §7 is the authority that assigns this command its fixed pair; `dev-workflows`' `docs/roles-and-phases.md` describes the `dev` role, and `documenting` has no roles page — [Session cost](../reference/session-cost.md) says which commands emit it; `workflows-core`'s `docs/roles-and-phases.md` covers only the three phases its own commands reach.
