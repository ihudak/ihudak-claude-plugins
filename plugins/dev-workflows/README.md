...
| PM *(BRD route)* | [`/brd-intake`](docs/commands/brd-intake.md), [`/brd-split`](docs/commands/brd-split.md), [`/brd-interview`](docs/commands/brd-interview.md) | Intake a customer BRD verbatim, extract its requirement inventory, split it once every row is allocated, then decide its open questions round by round. |
