...
**What you also need, and it is not a plugin.** Nothing — the pipeline reads and writes one markdown tree. If you keep your work in a tracker as well, syncing the two is yours to arrange; no command here learns whether one exists.
...
`product-workflows` reads three environment variables. One is required for the pipeline to have anywhere to write (`SPECS_PATH`); the other two are optional and each degrades to a documented default or a silent skip.
