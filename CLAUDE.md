...
/brd-ground          → [require-on-main: coverage-ledger.md] → resolve repos + baseline-integrity gate → [docs-grounder (lead-only)] → [code-grounder×N + design-grounder (parallel)] → [grounding-verifier@Opus: independent re-derivation, no outcome ⇒ not evidence] → write findings → [handoff-to-main: grounding findings] → impl-maintenance → commit-artifacts
/brd-split           → [require-on-main: grounding/code-grounding.md] → [gate: every finding carries a verifier outcome] → propose + key slices (`full` mode only) → walk every `unallocated` ledger row to a recorded fate → write slices.md → [handoff-to-main: ledger + slices + each child's seed files] → impl-maintenance → commit-artifacts   (on a slice: `allocate-only` — no child, four resolutions instead of five)
/brd-interview       → [require-on-main: grounding/code-grounding.md] → [gates: every finding verified; no ledger row `unallocated`] → resolve/open the round → tag every question → answer each `[G]` from the findings → decide each `[V]` with the operator → hold each `[C]` for the customer → write decisions.md + interview/round-N.md → [handoff-to-main: register + round record] → impl-maintenance → commit-artifacts
...
--from-brd (tail)    → /create-prd <BRD-KEY> --from-brd [no idea ladder; reads prd-seed.md + decisions.md from the resolved BRD folder; --full by default; grill restricted to gaps, a [VD#n]/[CD#n] never reopened; two Phase 0 refusals — a claimed ledger row still `unallocated`, or no `covered-here` row at all] → [prd-reviewer@Opus] → /create-ard <BRD-KEY> --from-brd [ard-seed.md; no PRD gate] → /specify <BRD-KEY> --from-brd [spec-seed.md + its derivation matrix] → then /epics, /design and /ready exactly as above. Each of the three writes `consumed_by: PRD|ARD|specification` on what it drew on and reports, by id, what at its own altitude is still `consumed_by: none`. A BRD key addresses a folder, never a tracker record; the tracker key `/epics` and `/release-notes` need is minted by the Jira round-trip and lands in `jira_key`. The PRD's `brd_key`, `brd_parent` and `depends_on` record committed BRD provenance and **no command consumes them yet** — `/epics` and `/ready` read none of the three — so do not justify them by naming a consumer; they exist because provenance captured at authoring time is the precondition for any future one.
