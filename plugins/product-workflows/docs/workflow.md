...
| `/create-ard` | `<SLICE-KEY>` | `--no-docs` | On a slice, with no further condition — gates and reads `prd.md` as on the idea route, proceeding when there is none, and never authors from the ledger |
