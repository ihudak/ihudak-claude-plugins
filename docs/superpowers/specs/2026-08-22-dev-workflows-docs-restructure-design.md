...
- **`REPOS_PATH`** — where your code clones live, one directory or a colon-separated list, defaulting to `/workspace`. Repos are matched by their `git remote get-url origin` slug, never by directory name, which is the part that surprises people.
...
The filtered and unfiltered counts differ for most commands, but the **set** of thirteen is identical either way: no command sits close enough to the threshold for the counting rule to move it. That is worth stating because it is the reason to trust the trigger rather than the numbers. A linear sequence with no fan-out is a numbered list; drawing it as a flowchart is decoration.
