...
- **`vault-prior-art-finder` agent and `references/vault-prior-art.md`.** `/idea` and `/create-vi` now ground on vault prior art to surface tracked initiatives under `Projects/Products/` and `Projects/ideas/` that may extend, parallel, or supersede the work the user is authoring. The agent retrieves prior-art matches ranked by confidence and extracts the relation type — so the user can make an informed choice at the Phase 4 gate.
- **`resolve-export-for-key` entry point in `jira-input-resolution`.** The `idea-reader` and `vault-prior-art-finder` agents use it to handle Jira product keys that exist nested in the export (e.g. `jira-products/PRODUCT-14640/PRODUCT-14796/PRODUCT-14796.md`), not just at the top level. When a key resolves to multiple copies, the entry point returns the most recently modified one, fixing a lookup failure on the 431 keys stored in subdirectories.
- **`## Prior art` section in `idea-format.md`.** Documents the wikilink-plus-key bullet format for transcribing prior-art matches from the digest into `idea.md` — why the Jira key is the durable identifier when present, and that every entry is transcribed from the finder's output rather than invented.
...
- **The Phase 4 gate's `(Recommended)` derivation could name row 1 ("Rewrite <KEY>") when row 1 was absent from the display.** When a supplied VI has no vault work document, the finder classifies the case as `supersedes_self` but yields `item_dir: null`, so row 1 (rewrite) is not rendered. The gate's selection rule now falls back to row 3 (write to container default) in every case where the selected row is not present, eliminating a recommendation that points at nothing.
