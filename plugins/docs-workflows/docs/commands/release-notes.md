...
**The run has no worthiness gate, and there is no content state in which it refuses to draft.** One existed and was retired: it read a `relevant_for_release_notes` flag off the PRD and stopped on an explicit `false` or `no`. Every PRD is relevant for release notes, so the flag asked a question with one answer and the only value that changed anything was one nobody should write; the field is retired in `workflows-core:prd-format` and a value left in an existing PRD is read by nothing. The run refuses only on its address and on what resolving it needs: Phase 0 stops naming `$SPECS_PATH` where a `<KEY>` arrives with that variable unset, stops with `RELEASE_NOTES_NEEDS_KEY` where the prompt carries no positional address at all, stops naming every match where one resolves ambiguously, and raises the shared re-enter/cancel escalation where one resolves to nothing. A folder that resolves and holds no PRD takes a named stop instead, since re-entering a key that resolved cannot help. In Phase 0, `RELEASE_NOTES_BRD_NOT_SLICED` stops on a BRD container, whose PRDs sit in its slices: it lists each slice, and offers `Enter a slice key` or `Cancel`, taking a typed key only where it is one of the slices listed and re-asking otherwise; a container with no slice yet is a plain stop naming `/product-workflows:brd-split`. `RELEASE_NOTES_FOLDER_NOT_PLACED` stops on a folder it cannot place at any level, naming what the folder carries and the remedy: give it a carrier, or, where it holds an `idea.md` and no `prd.md`, run `/product-workflows:create-prd`. In Phase 3, `RELEASE_NOTES_NO_PRD` stops on a PRD folder with no `prd.md` and names `/product-workflows:create-prd`, or, for an Epic with no PRD folder above it, moving the Epic folder into one. A `prd.md` whose frontmatter asserts no `kind: prd` is still read as the PRD. Whether a note is worth drafting is the decision of whoever runs the command.
