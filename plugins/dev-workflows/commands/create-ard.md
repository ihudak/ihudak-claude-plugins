...
- **PRD-level ARD:** if the PRD has 0 Epics → `choices: ["Hand to a Product Engineer — /dev-workflows:epics <PRD> (then create them in Jira + re-import) (PE) (Recommended)", "Author a PRD-level spec — /dev-workflows:specify <PRD> (PE)", "Stop here", "Other… (describe)"]`; else offer `/dev-workflows:specify <PRD>` (PE). *(No `/design` — no Epics yet.)* Either way, `/dev-workflows:specify` won't treat this ARD as available until the pull request above is merged — until then it architects without it, same as if none existed.
- **Epic-level ARD:** `choices: ["Author the spec — /dev-workflows:specify <PRD> <Epic> (PE) (Recommended)", "Hand to Dev — /dev-workflows:design <PRD> <Epic> (Dev)", "Stop here", "Other… (describe)"]`. **Epic fan-out** — repeat this ARD for a sibling Epic: `/dev-workflows:create-ard <PRD> <another-Epic>`. Both `/dev-workflows:specify` and `/dev-workflows:design` wait the same way — this ARD is invisible to them until its pull request above is merged.
