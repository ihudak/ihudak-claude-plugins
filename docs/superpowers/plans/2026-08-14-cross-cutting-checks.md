...
Recorded proof tables rather than assurances, for Tasks 6–13's phase-handoff-gates commands. Baseline for every discrimination proof: `bca4191`. This file is the evidence Task 20's verification record cites for R20, R30, Risk 2, and Risk 8 (`2026-08-14-phase-handoff-gates.md` Step 3 table).
...
**Dispatch-vs-prose control** (why the dispatch form matters, not the bare word — matches the brief's measured baseline for the prose mention alone): bare `code-scanner` mention vs. real dispatch, current tree —
...
A check keyed on the bare word would have passed every row on coincidence (the prose mention always precedes the real dispatch); the dispatch-form pattern above is what the R30 table actually measures.

**Can-fail proof** — same script, gate pattern replaced with `Final report` (a string that appears only near the end of a file):
...
All seven flip to `CHECK` — the check discriminates. R30 met on the real pattern — no fix required for R30 itself; see Step 2a for the three ordering defects the *audit* (a distinct check) found in `/epics`, `/design`, and `/implement`.
...
Post-fix re-verification of R30 (line numbers shift only for the three edited files' gate line — re-run in Step 2 above already reflects the post-fix state: all seven still `YES`).
...
All seven cells filled. **Critical check: does an absent input stop any command except `/design`?** No — every row above shows the absent case reaching non-stopping, pre-existing behaviour (fall-through, fallback, skip, inactive, missing-in-roll-up); only `/design`'s row F stops, which is `/design`'s own pre-existing, unchanged behaviour per §3.4's own text ("stops — but that stop already exists; this reference only makes its test correct"). No Critical found.

**Observed but out of scope for this task:** `/specify` also gates a VI (`require-on-main` at `commands/specify.md:90`, behaviourally identical to `/create-ard`'s VI gate — falls back to the Jira export, reported, never stops), but `phase-handoff.md` §3.4's table has no row for it — only `/create-ard`'s VI gate is listed. This is a completeness gap in §3.4's table, not a behavioural defect (the actual `/specify` behaviour is already correct, matching `/create-ard`'s pattern). Not fixed here — flagged for the maintainer, since Step 4's mandate was to prove the seven existing §3.4 rows, not to extend the table.
