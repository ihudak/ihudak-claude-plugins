...
- **It gates nothing on the build ladder, and nothing on that ladder reads what it writes.** It does gate its own input — the `proposal.md` on each included slice, under *Gates* above — and nothing beyond it. No command of the build ladder reads a proposal; the one command that reads another folder's proposal is this one, and it reads a **slice's**, never an umbrella's. Every read of the umbrella it writes is an own-folder one — a later run of this command, which anchors its re-estimate on the prior figures, and `proposal-reviewer` inside the run that wrote it, under *Gates* above — so its handoff presents the consent array for an artifact no command *stops* on, whose reader reads your working copy rather than the default branch.
