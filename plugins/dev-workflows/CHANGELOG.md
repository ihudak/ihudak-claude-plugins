...
The step now resolves `<base>` by `code-handoff.md` §2.8, the ladder Phase 4.6 uses, so the branch a run cuts from is the base its pull request targets. It tests "HEAD is not on `<base>`" in place of the list, reads `git log origin/<base>..HEAD --oneline` — §2.8 yields a `<base>` only where `origin/<base>` exists (next entry) — and treats a non-zero exit as a failed read, never as "no ahead commits": git's error is shown and the question is asked. An exhausted ladder asks nothing and prints a `Base branch unresolved` line that the Phase 5 report's `### Branch` section carries. *"Branch from default branch"* runs `git switch <base>`, with no fetch and no pull, as `/vuln`'s switch onto the same base does; a switch git refuses stops the run before any branch exists, rather than cutting one from the HEAD the user declined.

### Fixed — the base-branch ladder no longer trusts a dangling `origin/HEAD`

`code-handoff.md` §2.8's first rung, `git symbolic-ref --quiet --short refs/remotes/origin/HEAD`, still prints `origin/master` and exits 0 after the remote renames its default branch to `main` and the clone fetches with `--prune` — `origin/HEAD` is left naming the branch the remote deleted. The ladder stopped there, so `/implement`, `/upgrade` and `/vuln` offered to branch from `master`, read their ahead commits against an `origin/master` that is gone (`git log` exits 128), and handed `gh pr create` a `--base` the remote does not have. Rung 1 now counts only where `git -C "<repo>" rev-parse --verify --quiet origin/<name>` also succeeds; otherwise the ladder goes on to rung 2, which finds `origin/main`. Since §2.8 now yields a base only where `origin/<base>` exists, the `<base>..HEAD` alternative `/implement` step 4 and `/upgrade`'s prep read "where `origin/<base>` does not exist" — which served only the dangling state — could no longer be reached, and is gone. `workflows-core`'s `read-only-repos.md` §3 and `specs-repo-git.md` §3.2 carry the same fix.
