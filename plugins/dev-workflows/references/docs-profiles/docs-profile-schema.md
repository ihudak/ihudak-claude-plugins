...
cross_space_override:
  manifest: self-hosted/docstack.jsonc
  mechanism: "the self-hosted manifest pulls an allowlist of ../cloud/_content/... pages; last-write-wins by path silently shadows a self-hosted/_content override"
  rule: "to make a self-hosted/_content override win, add the shared cloud path to the allowlist block's `ignore`"
shared_registries:
  - files: [schema-ids.yml, schema-mappings.yml]
    when: "renaming/retitling/creating a settings-schema page under cloud/_content/api/settings/schemas/"
...
tokens:
  ...
  project_conditionals: "{{#if project='cloud'}}…{{/if}} / project='self-hosted' / project='classic'"
internal_links:
