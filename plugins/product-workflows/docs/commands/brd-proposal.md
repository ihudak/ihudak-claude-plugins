...
- **It gates nothing downstream, and nothing reads what it writes.** No command of the build ladder reads a proposal; the one command that reads another folder's proposal is this one, and it reads a **slice's**, never an umbrella's. That is why its handoff presents the consent array for an artifact nothing downstream reads.
