...
3. **Resolve `idea.md` (ladder — stop at first hit). Skipped entirely on the BRD route**, where
   ...
   4. *(retired — the discover rung searched a personal vault for a stray `idea.md`. `/idea` writes into the resolved folder now, so rung 1 finds it.)* — out-of-contract, as rung 2;
