...
naming it would send the operator to a run that moves nothing. Only a row still `unallocated` — which
a slice this route interviewed never holds — is named with `/product-workflows:brd-split <BRD-KEY>`.
