...
| Agent | Model | Tools | What it does | Used by |
|---|---|---|---|---|
| `diff-summarizer` | per routing | Read, Glob, Grep, Bash, Skill | Reads one repository's recorded refs and returns a documentation-focused summary; pure local git, calling no forge and no CLI that wraps one. | `/document`, `/release-notes` |
