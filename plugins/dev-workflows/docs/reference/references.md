...
`dev-workflows` bundles 102 files under `references/` — 40 top-level markdown files, `cost-prices.yaml`, one file under `model-routing/`, and six bundled subtrees. This page enumerates every file a command or agent actually cites by name (42 of the 102 — the 40 top-level files plus `cost-prices.yaml` plus `model-routing/classification.md`), grouped by concern below, then counts the six subtrees rather than listing each file inside them. The arithmetic: 40 + 1 + 1 = 42 named individually, plus 24 + 11 + 10 + 5 + 3 + 2 = 55 markdown pages counted (not enumerated) across the six subtrees — 42 + 55 = 97 accounted for, against 102 files on disk. The remaining five are non-markdown data or templates inside those same subtrees, deliberately not listed as reference pages: `api-guidelines/template/openapi-template.yaml`, `api-guidelines/spectral/ruleset.yaml`, `docs-profiles/docs-profile.default.yml`, `docs-profiles/default-owners.txt`, and `guidelines/check_guidelines.py` — a template, a Spectral ruleset, a defaults file, an owners list, and a lint script, none of them prose a reader would open. The 55-file subtree figures below are markdown-page counts specifically; the five files above already sit inside those same subtrees and are not part of that count, so nobody should later "correct" a subtree figure by adding them back in.
...
- `brd-addressing.md` — the key grammar and one-level-deep folder-resolution rule for a BRD (business requirements document) parent or slice under `specifications/`; not yet cited by any command, but fixed ahead of the `/brd-*` family that will.
- `brd-format.md` — what a BRD is (customer-supplied, immutable once intaken, markdown only), the `[BR#n]` inventory row shape and the split-on-multiple-obligations rule, the six `[DEF#n]` defect classes, and defect-resolution states; not yet cited by any command, but fixed ahead of the `/brd-*` family that will.
- `coverage-ledger-format.md` — the coverage ledger's row shape, its six dispositions (only `unallocated` blocks), the allocation gate `/brd-split` enforces, and the PRD-eligibility rule read from it; not yet cited by any command, but fixed ahead of the `/brd-*` family that will.
- `grounding-format.md` — the grounding finding contract: the `[CG#n]`/`[DG#n]` record shape, the six verdicts, the `baseline-integrity` procedure, the `current`/`will-change` horizons, the four design-grounding reconciliation classes, the optional derivation matrix, and the four verification outcomes; not yet cited by any command, but fixed ahead of the `code-grounder`, `design-grounder`, `grounding-verifier`, and `/brd-ground` family that will.
