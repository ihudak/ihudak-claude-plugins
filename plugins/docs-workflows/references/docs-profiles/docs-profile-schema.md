...
- `dev_servers.readiness_timeout_seconds` is optional (default 120) — how many seconds a consumer polls a server it started for readiness: `/document`'s Phase 6.5 before falling back to the manual table, and `/docs-serve`'s Phase 5 before reporting a timeout, which leaves the server running and records its entry without a pid.
