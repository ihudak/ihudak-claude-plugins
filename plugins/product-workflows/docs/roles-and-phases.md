...
Emitted by `/prd-proposal` and `/brd-proposal`, role `pm`. Being in this phase means a requirement set is being **priced** rather than advanced — work packages, hours by role, and a range with its evidence — for a document a vendor sends a customer. It is the one phase here that no other phase waits on and that gates nothing outside itself — `/brd-proposal`'s gate on a slice's `proposal.md` is this same phase gating its own earlier run: a proposal is optional at every readiness tier, and no command of the build ladder reads one. The one command that reads **another folder's** proposal is `/brd-proposal`, which gates on each included slice's `proposal.md` in order to roll it into the programme umbrella — a second proposal rather than a phase of the build. Every other read is a command opening the proposal in its own folder: a later run of `/brd-proposal` anchors its re-estimate on the umbrella it wrote last time, and a later run of `/prd-proposal` does the same with the slice proposal it wrote — a read, never a gate. It is its own phase rather than part of `brd-to-prd` because `/prd-proposal` runs on the idea route as readily as on the BRD route.
