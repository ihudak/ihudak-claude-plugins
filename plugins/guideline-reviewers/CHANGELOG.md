...
Both agents ran their linter bare — `npx --no-install eslint`, or the repository's lint script, and `spectral` / `npx --no-install @stoplight/spectral-cli` — and a subagent's Bash tool starts every call in the session's directory, which need not be the reviewed repository. `npx --no-install` resolves its tool from the directory it runs in, so from anywhere else it printed *"npx canceled due to missing packages"* and the lint was skipped as a missing binary — a silent `a11y_check: none` or `lint_source: none` — or it ran under another repository's configuration. A monorepo whose packages keep their own linter was skipped even with the session at its top level. Each agent now partitions the files it reviews by the nearest directory at or above each one, up to its repository's git top level, that holds the linter's config or a `package.json` declaring it — a file with no such ancestor falling to its top level's partition — and runs each partition's probes and its one lint in that directory, each as one `(cd "<dir>" && …)` subshell, merging the findings, each keyed by its file. That is the rule `docs-workflows`' `docs-style-checker` and `prose-style`'s `/prose-review-pr` and `/prose-review-docs` already follow for Vale, applied to each reviewed file rather than once for the whole set: files reviewed together from two packages that each keep their own linter configuration are each linted under their own, where one directory for the whole set would have been their top level, which holds neither. The report carries one `a11y_check` or `lint_source` line per partition where the files span more than one, each naming its directory. The Spectral ruleset follows the same rule: the nearest `.spectral.*` up to the top level, no longer only one at the repository's root. A repository that keeps its linter configuration only at its top level, reviewed with the session standing in it, gets the same findings as before, from the same directory; Spectral now lints its specs in one run rather than one run each.
