...
- `toolchain-preflight.md` — the environment check `/document` runs at Phase 0, deriving the required tool set from the resolved profile, the repo's config signals and the repo's own documented prerequisites; the tool→gate map; and the prompt that fires only when a required tool is missing. `/docs-init` runs the same check at its Phase 2 but derives nothing — there is no profile yet — and hands it a fixed set of its own. The file says which test each tool takes, by the shell that will run it: a tool the plugin runs through `bash -c` or as `command <name>` — `bash`, `curl`, `lsof`, `ps`, `vale` and every dev server's tool — is tested as a binary alone, since neither runs an alias of yours, nor a shell function of yours that you have not exported, while a build or lint command's tool, which runs in the Bash tool's own shell, is tested as that shell resolves it: a binary, a shell function, or an alias whose first word it finds. `/docs-serve` runs no preflight, but tests its own tools with the first of those tests — not a bare `command -v`, which an alias or a shell function named after the tool passes; `/document`'s render check uses it too — and takes the file's definition of a command's tool, with that test, before it starts a server, and its set-aside of a leading `cd <dir> &&` and `VAR=value` words when it matches a recorded command to a process's command line. The file also defines, once, the five names Vale reads its configuration from and how the plugin runs Vale over one — a lint and `vale sync` alike, on the repository's configuration with no global Vale configuration and no `VALE_CONFIG_PATH`, as a clean CI runner has neither, and with the limit it states for a configuration that sets no `StylesPath`, which keeps the machine's shared default styles directory — which `docs-style-checker`, `docs-scaffold-reviewer` and `/docs-init` cite.
