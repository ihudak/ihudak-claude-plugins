...
| `idea-reader` | per routing | Read, Glob, Grep | Ingests one idea source into a source digest for `/idea`: wikilinks two levels deep, linked images read and described as context, every other linked file enumerated but never opened. | `/idea` |
