...
**A third-route orphan row (§2) is counted through the parent's current disposition for that `[BR#n]`, never as it reads.** It holds the slice's own earlier decision, which the parent's re-allocation overruled, so counting it as written would report a requirement this slice no longer holds as covered or deferred here — and, summed across siblings, twice. An orphan row is third-route wherever it differs from what §3's orphan table maps the parent's current row for that `[BR#n]` to; the first two routes write exactly that mapping, so for them the rule changes nothing. Read the parent's row, take the table's mapping — `covered-by: <SIBLING-KEY>`, `covered-by: <PARENT-KEY>`, or `rejected`/`superseded-by` verbatim — and count this row as that mapping, a `covered-by` one resolved one hop by the table above and counted among the delegated rows (§6.4). Where the parent's row is `unallocated` — after a `/brd-intake` re-run, before any walk re-allocates it — count it `unallocated`. Reading the parent's row fixes what this row is; it is not a hop, and the one hop the mapping may then take is the only one.
...
`covered-by` orphan rows (§2), and every third-route orphan row the rule above counts as a
`covered-by` one. That is deliberate and is the audit trail §1 exists for: a requirement a slice
...
`<delegated>` counts the `covered-by` rows, and the third-route orphan rows §6.1 counts as
...
is **not** a property of being a slice. A slice holding orphan rows (§2) reports its `covered-by` ones as
delegated, resolved one hop through the sibling or parent each names, exactly as §6.1 resolves any
other `covered-by` row, and a third-route one through the parent's current disposition for that
`[BR#n]` (§6.1).
