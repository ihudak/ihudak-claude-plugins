...
- **The gated readers are six**, and `ard-resolution.md` must name all six in both its header and its Consumers section. Before J it listed four in the header (`:4-5`) and five in the body (`:59-63`) — a stale caller list contradicted by its own text. J adds `/create-ard`, which reads the inherited VI-level ARD on an Epic-level run, so the final set is `/create-ard`, `/design`, `/implement`, `/specify`, `/epics`, `/ready`. **Adding a consumer without adding it to that list re-creates the defect J is fixing**, which is exactly what happened once during implementation.
...
- **R51** `ard-resolution.md`'s header caller list and its Consumers section both name the same **six** callers — `/create-ard`, `/design`, `/implement`, `/specify`, `/epics`, `/ready`. The header previously named four and the body five; J adds `/create-ard` as a reader of the inherited VI-level ARD.
