...
0. **The route is a property of the resolved folder, not of a flag.**
   ...
   **Where the folder resolved through `workflows-core:addressing` §5's legacy
   ...
   refusal does not fire on it, and refusing it would offer `/pm-workflows:brd-split` on a folder
   ...
   **The remedy is the same two branches `/pm-workflows:create-prd`'s own container refusal takes,
   ...
   - **One or more slices** — the ordinary shape, since a split always confirms at least one. Name
     every slice and offer `/pm-workflows:specify <SLICE-KEY>` once per slice. Do **not** name
     `/pm-workflows:brd-split <BRD-KEY>`: the slices exist, and on a parent whose ledger is fully
   ...
   - **No slice at all** — `/pm-workflows:brd-split <BRD-KEY>` is the run that carves one, walking
     ...
     stops naming `/pm-workflows:brd-ground <BRD-KEY>` when they do not; and **where this BRD's
...
3. **Resolve the feature folder.** Derive provisional kebab-case slugs from the relevant
   ...
   - **Resolve the feature folder itself**, by case. **There are two cases, not three**: an Epic
     always has a PRD above it, because `/pm-workflows:epics` is the only command that creates an
     `EPIC-` folder and it writes every one of them under a PRD folder (D6). A top-level `EPIC-`
     folder with no PRD above it — the third case this step used to carry — is retired: `/pm-workflows:epics`
     ...
     - `focus_key` set (an Epic nested under a PRD) →
       ...
       minted by `/pm-workflows:epics` and by nothing else, so one that does not exist is a stop,
       not a directory to make:
       `SPECIFY_EPIC_NOT_FOUND: no Epic folder for <EPIC> under <the resolved PRD dir> — an EPIC- folder is created by /pm-workflows:epics and by no other command. Run '/pm-workflows:epics <PRD>' to draft this PRD's Epics, then re-run '/pm-workflows:specify <EPIC>'.`
       That remedy runs in the state this stop reports: `/pm-workflows:epics` takes the PRD address
   ...
   **The write location then follows `focus_key`, by the same two cases as above and not by a rule of its
   ...
   BRD route: where `/pm-workflows:epics` has minted `EPIC-` folders under the slice, Phase 2 Step A's
   ...
   Epic directly — `/pm-workflows:specify <EPIC-KEY>` — resolves an `EPIC-` folder, which carries no
   ...
   **Deciding it on the resolved folder alone was a silent contract loss, and the plugin recommends the
   address that triggered it.** `/epics` ends by offering `/pm-workflows:specify <EPIC>`; taken on a
   ...
   `/docs-workflows:release-notes`, `/pm-workflows:epics` and `/pm-workflows:create-ard` each name
   in their own `absent` stops — and step 0 refuses the `BRD-` container a `/pm-workflows:brd-intake` run
   would leave behind:
   `SPECIFY_BRD_NOT_FOUND: no folder found for <ADDRESS> under $SPECS_PATH/specifications/ (every level addressing.md §3 bounds, plus §5's legacy fallback) — check the address. /specify specifies an existing PRD or Epic folder and creates neither. A PRD- folder is created by /pm-workflows:idea <KEY> or /pm-workflows:create-prd <KEY> on the idea route, and by /pm-workflows:brd-split on its parent BRD on the BRD route — /specify writes into that folder, never into the BRD- container above it, and a parent BRD is created by /pm-workflows:brd-intake <BRD-KEY> @<brd-file> and then grounded and split before any slice exists. An EPIC- folder is created by /pm-workflows:epics <PRD-ADDRESS> and by no other command.`
...
— `/pm-workflows:create-prd` is still not a prerequisite for this command, on either route. What the gate
...
`/pm-workflows:create-prd` opens `prd/<SLICE-KEY>-<slug>` there and its handoff can be declined — and
...
artifact. It says nothing about the PRD, which is authored by `/pm-workflows:create-prd` under the `prd/`
...
folder — a PRD folder, which can hold a `prd.md` and can hold `EPIC-` folders that `/pm-workflows:epics`
...
the ordinary state on the BRD route, where `/pm-workflows:create-prd` is not a prerequisite for this
...
the case itself (Phase 0 step 3): `/pm-workflows:epics` writes every `EPIC-` folder under a PRD folder, and
...
- **PRD with 0 Epics** → this PRD hasn't been split yet. Offer the without-Epics choices:
  `choices: ["Split into Epics first with /pm-workflows:epics (Recommended)", "Author one broad PRD-level spec now", "Cancel"]`
  `/specify` does not write Epic folders itself — on "Split…", stop and point at
  `/pm-workflows:epics`, which writes them into this PRD folder where this command will see them. On
  "Author one broad PRD-level spec now", leave `focus_key` = null and proceed to Step B.
  - **`/pm-workflows:epics` is only offered where the resolved folder holds an authored `prd.md`.**
    ...
    `"Author the PRD first — /pm-workflows:create-prd <ADDRESS> (Recommended)"`, which is the run that
    ...
    **Passing that command's container refusal is not enough to name it**, and it is the only one of
    ...
    `/pm-workflows:create-prd` option where the gate set clears both data tests, the corresponding
    `/pm-workflows:brd-split` where one fails and a command exists, and **no first choice at all**
    where none does, leaving the two below it. A folder carrying no `brd-link.md` has no gate set and
    takes `/pm-workflows:create-prd <ADDRESS>` on the container test alone. The second and third
...
- **`spec-seed.md`** — implementation-altitude content, when the folder holds any. **Where to look, and it is two places.** The resolved slice first. Then, when the slice holds none, the **parent BRD folder** named by `brd-link.md`'s `parent:` — that is where `--sort-existing` actually writes all three seeds, because slices do not exist when intake runs. A seed found there is BRD-wide content, not slice-scoped: read it as context for this slice, say which folder it came from, and never treat it as though it were written for this slice alone. Finding neither remains the ordinary case. **No `/brd-*` command writes this file on the normal route** — the one writer is
  `/pm-workflows:brd-intake --sort-existing`, a one-time migration path for a package authored
  ...
  **`prd-seed.md` and `ard-seed.md` are not read**, at all: they are the product and
  architecture altitudes of the same router, belonging to `/pm-workflows:create-prd` and
  `/pm-workflows:create-ard`. Reading the first would let product requirements be restated as spec
...
- **`grounding/code-grounding.md`** and **`grounding/design-grounding.md`** — the `[CG#n]` and
  `[DG#n]` finding records, per `workflows-core:grounding-format` §2 — **and, in
  the first of the two, the derivation matrix.** The matrix is not a file of its own and is not inside
  `spec-seed.md`: `/pm-workflows:brd-ground` appends its rows to the slice folder's `grounding/code-grounding.md`
  (that command's Phase 8), classed per `workflows-core:grounding-format` §7
  ...
  an absent matrix is ordinary, since `/pm-workflows:brd-ground` runs it only on a reporting- or
...
- **`brd-link.md`** — for `parent:` and `depends-on:` only. This run reads no `claims:` list and no
  ...
  here is a decision, not an omission — `/pm-workflows:create-prd` on the BRD route is where that gate
  ...
  **Two places do open both, and neither gates anything this run does**: Phase 2 Step A's
  ...
  `/pm-workflows:create-prd <SLICE-KEY>` can be *named* at all, since that command refuses three
...
`/pm-workflows:create-prd` and `/pm-workflows:create-ard`, both of which read this same register and
...
1. **Auto-derive candidate repos.** From the Phase 2 capability themes and any repositories the
   ...
   every repo `/pm-workflows:brd-ground` read, plus the Phase 2 themes. That is a stronger starting
...
2. **This run cannot satisfy either cause that would license a reopening.**
   ...
   except through `/pm-workflows:brd-reconcile`.
...
`/pm-workflows:brd-ground <BRD-KEY> --rebaseline` mints and `/pm-workflows:brd-interview <BRD-KEY>`
then re-decides against; a `[CD#n]` needs the customer, through
`/pm-workflows:brd-package <BRD-KEY>` and then
`/pm-workflows:brd-reconcile <BRD-KEY> @<review-file>`. A contradiction with an `AD#N` is a different
...
(`workflows-core:grounding-format` §8). These are the **only** writes this command
...
span suggestion (PRD-level→`/pm-workflows:epics` `/compact`; Epic-level→`/dev-workflows:design` `/clear`) +
...
`/pm-workflows:create-prd` is not a prerequisite at all — so the folder it just wrote a specification
into may legitimately hold no PRD. **Test the resolved PRD folder for an authored `prd.md`** before
rendering the recommendation: where there is one, name `/pm-workflows:epics <ADDRESS>`; where there
is not, name **`/pm-workflows:create-prd <ADDRESS>`** instead — **but only where that command can
...
carries a `brd-link.md`; an idea-route PRD folder has no gate set, so `/pm-workflows:create-prd
...
|---|---|
| No row `unallocated`, and at least one `covered-here` | `/pm-workflows:create-prd <SLICE-KEY>` — all three refusals cleared |
| A row still `unallocated` | `/pm-workflows:brd-split <SLICE-KEY>` instead: `/create-prd` would raise `CREATE_PRD_BRD_UNALLOCATED`, and that walk is what moves those rows (allocate-only on a slice). Its own Phase 0 gates on this slice's grounding findings carrying a verifier verdict, so say so beside the offer |
| No row `covered-here`, gate set **empty** | `/pm-workflows:brd-split <PARENT-KEY>` instead — the keep-or-remove run for a standing empty child, and not a no-op there |
...
same two data tests before offering `/pm-workflows:create-prd <SLICE-KEY>` and drops the option on
...
`/dev-workflows:design` and `/pm-workflows:epics` both resolve a folder in the specs tree with
...
- **`/pm-workflows:epics <ADDRESS>`** — it reads the PRD folder and gates nothing this run
