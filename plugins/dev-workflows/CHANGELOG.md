...
**A Cargo workspace with two or more members has the same limit, and it needs no `Makefile` at all.** `cargo test` at the workspace root runs every member and prints each test as `<module path>::<name>`, with nothing in the line naming the crate, so two members that name a test alike mint one identifier and a loss in one is masked by the other's copy. Measured on Rust 1.98.1 / cargo 1.98.1: a two-member workspace whose members both print `tests::works` recorded **3** identifiers for **4** passing tests, and after the second member lost that test the run returned `OK` with nothing in *Missing from run* and its own `Passing` down 4 to 3. This is easier to meet than the `make` shape, because the collision arrives from the toolchain rather than from anything you wrote — measured again on two crates `cargo new --lib` had just written and nothing had edited, both printing `tests::it_works`. The counts, the status and the row are correct; what is masked is which member a name belongs to. The run says so: `### Notes` names every member the workspace holds, states that its identifiers are not attributed to them, and carries the `CAVEAT: ` mark, so it reaches you on every status including `OK`. A workspace with exactly one member collapses nothing and is not marked.
