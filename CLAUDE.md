...
/epics               → [resolve-docs-grounding — a deliberate exception to §5 rule 2's ordering] → [require-on-main: VI-level specification.md] → jira-reader → [docs-grounder] → [code-scanner×N (parallel, optional)] → writing → [dt-style-checker] → [doc-fixer] → [epic-reviewer@Opus] → [doc-fixer] → impl-maintenance → commit-artifacts
