...
| `grounding-verifier` | opus | Read, Glob, Grep, Bash | Independently re-derives a `[CG#n]`/`[DG#n]` finding from the repository, without reading its `evidence` first, and returns agree / extend / contradict / unprovable with its own evidence. | `/brd-ground` |
