...
`${CLAUDE_PLUGIN_ROOT}/references/phase-handoff.md` §4.3's consent choice verbatim — the **unread** variant (§4.0), whose second option says nothing downstream reads this rather than promising a stop. Nothing gates a frame-set index and nothing reads one, so both other variants would be false here and would contradict the §4.1 clause this phase prints on a decline. On the first
