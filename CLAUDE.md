...
- Every git invocation is `git -C "$SPECS_PATH"` — the working directory is NEVER changed; twelve of the twenty callers resolve or run inside a repository that is not the specs repo (all twenty minus the eight that touch no other repository at all: `/brd-intake`, `/brd-split`, `/create-prd`, `/update-prd`, `/feedback`, `/prompt`, `/prompt-brainstorm`, `/prompt-grill-me`)
