...
- **`cdn_upload_resolutions`** — how Phase 6.1 resolved each screenshot the checklist placed on an `image_policy: cdn_upload_required` target, one `{target, image, resolution, cdn_url}` entry per screenshot: `uploaded`, where the user pasted its CDN URL at Phase 6.1 and `cdn_url` carries it, or `deferred`, where it was staged for manual upload. `[]` when no screenshot has that policy. Supplies the upload-form evidence for the 'Screenshots' dimension.
