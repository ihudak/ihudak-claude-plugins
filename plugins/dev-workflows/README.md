...
| `/design <VI-KEY \| Epic-KEY \| jira-export-dir> [focus-Epic-KEY] [--design-twice]` | Jira-driven engineering design authoring (Dev phase). Takes over a merged `specification.md` from the specs repo's main branch and authors a reviewed engineering `design.md` through a relentless one-question-at-a-time grill that **challenges** the spec (recording an `## Engineering review` section + open questions back onto it) and **designs** the implementation, grounded strictly in the fully-mounted code (hard repo gate — unmounted repos stop the run). Accepts the shared Jira-input grammar (`<VI> <Epic>` / `<dir> <Epic>`); for a multi-Epic VI it renders the progress-aware Epic picker (done-predicate `design.md` exists). A tiered model gate hard-stops SIGNIFICANT/HIGH-RISK work not on Opus. Durable/resumable via `_design-session.md` + `_design-glossary.md`; on a contested interface (a `design-format.md` `## Seams` signal) Phase 5 offers a three-take `interface-designer` fan-out, which `--design-twice` forces outright with no offer, feeding the unconditional `### Alternatives considered` section; gated by Opus `design-reviewer`; `design.md` open questions hard-block handoff; offers a branch+PR handoff (`design/<EPIC>-<eslug>` / `design/<VI>-<vslug>`) to the specs repo's main for `/implement`. Does not read Jira content — the spec is the source of truth. |
...
Thirty-four reusable subagents (invoked internally by the commands). The nine Opus-backed reviewers/planners are pinned; the rest have no fixed pin — their tier is assigned per the model-routing policy (mechanical → Sonnet, synthesis/review → Opus).
...
| `review-fixer` | per routing | Applies BLOCKER / MAJOR findings from a `code-review` report. Receives **survivors only** — the orchestrator has already triaged the findings (`references/finding-triage.md`), so every finding that arrives is one whose claimed consequence was verified; it does not re-triage and its dispositions stay Applied / Deferred. Applies that reference's **patch gate**: the fix must add no public surface and guard no state the finding did not demonstrate — a survivor whose smallest correct fix would do either is deferred as `DEFERRED — needs human decision` rather than patched speculatively. Returns a structured fix report with a `Stop condition flag` so callers know whether to re-review. Used by `/implement`, `/vuln`, `/upgrade`. |
...
| `doc-fixer` | per routing | Applies BLOCKER / MAJOR findings from a `doc-reviewer`, `epic-reviewer`, or `docs-style-checker` report. Shared between `/document` and `/epics`. From a **reviewer** the list arrives already triaged (`references/finding-triage.md`) — **survivors only**, never re-triaged; from a **style checker** no triage ran, because a linter violation is not a claim about consequence. Applies that reference's **patch gate**: no section, heading, page, cross-reference, or content beyond what the finding names — anything more is deferred as `DEFERRED — needs human decision`. Returns the same `Stop condition flag` contract as `review-fixer`. |
...
| `interface-designer` | per routing | Produces ONE interface proposal for ONE contested interface under ONE named constraint (minimise the interface / maximise flexibility / optimise for the most common caller), for `/design` Phase 5's three-take fan-out — dispatched three times in parallel, each blind to the others, so the takes diverge. Read-only; proposes an interface, never writes one. Used by `/design` only. |
