...
```
/design <VI-KEY | dir> [<Epic-KEY>] [--design-twice]
```

`/design` is **jira-driven only** — a plain prompt with no Jira input stops with `DESIGN_NEEDS_JIRA`; it uses the shared front-end only to parse the grammar and classify the key, never to read Jira content, because the requirements source of truth is the merged `specification.md` in the specs repo. The front-end resolves `jira_key` (the top-level VI, or a stand-alone item's own key) and `focus_key` (the Epic to design within it, or `null` for a bare VI / stand-alone item / directory).

**The Epic is the unit of work — one `design.md` per invocation, no fan-out.** When `focus_key` is already set, that Epic is the scope. When it is `null`, `/design` inspects the resolved VI directory in the specs repo itself (not Jira): a flat `specification.md` there (a stand-alone Epic, or a broad VI-level spec) means one design at that level; Epic subfolders are enumerated by which ones already carry a `specification.md` merged to main, and rendered as a progress-aware picker — ○ not started (a specification exists but no design and no in-progress session), ◐ in progress (a design session exists but no `design.md` yet, selectable as a resume), ● done (`design.md` exists, shown greyed, offering *revise* instead of a fresh run). Exactly one spec'd Epic auto-selects with no picker. `--design-twice` forces the Phase 5 interface fan-out on the run's load-bearing interface even when no contested-interface signal fired, and skips the offer described below entirely.

## How it runs

`/design` has 13 `## Phase` headings. The diagram below collapses the two gates that decide whether the run continues at all into explicit decision nodes, since they are the two facts most worth knowing before running this command.
...
- **A Jira VI or Epic**, via the shared front-end — a direct prompt is rejected outright (`DESIGN_NEEDS_JIRA`); `/design` has no non-Jira behavior.
...
Behind a consent choice, Phase 7 hands the feature folder off via `handoff-to-main`: a branch (`design/<EPIC>-<eslug>` for a per-Epic or stand-alone-Epic design, `design/<VI>-<vslug>` for a broad VI-level design), commit, push, and an opened pull request to main. Merged-to-main is what `/implement` gates on next. For a per-Epic design selected from a multi-Epic VI's picker, Phase 7 additionally offers "Next Epic" once the write/commit completes — re-opening the picker minus the just-completed Epic (now showing done) and looping back through Phases 2–7 for the next selection; this does not apply to a stand-alone Epic, a single-Epic VI, or a broad VI-level design.

## Gates

Phase 6 dispatches `design-reviewer` (Opus, frontmatter-pinned) against `design.md`, `specification.md`, the classification, and any ARD invariants. **Any unresolved open question in `design.md` is a BLOCKER by policy** — this is the gate a `/design` run actually hits, since the interview is required to resolve every open question to zero before Phase 6, pushing a genuinely undecidable one onto the spec instead. On `BLOCK`, the orchestrator/grill fixes the BLOCKER findings inline (no delegated writer) and re-reviews once; `PASS WITH RECOMMENDATIONS` / `MINOR` / `NIT` findings are deferred to the final report. Cap: one fix cycle plus one re-review — an unresolved BLOCKER after that is escalated individually rather than looped on. See [Model routing](../reference/model-routing.md) for the Opus fallback chain `design-reviewer` resolves against.
