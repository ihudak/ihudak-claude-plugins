...
`--inline` is stripped from `$ARGUMENTS` before the remaining token is read as the optional docs-repo path (Phase 0). `--inline` is the flag `/document` (keyed mode) passes when it invokes this flow itself, from its own Phase 0 — it skips the branch-name prompt in favour of a deterministic branch name, and hands the PR draft back to `/document` instead of reporting one itself, together with the fixed-port and dev-server command defect lists its own report would carry (below), which `/document` shows you. Where a branch of that name is already there, left by an earlier inline run that stopped after profiling, it stops with `DOCS_PROFILE_BOOTSTRAP_BRANCH_EXISTS`, naming the branch, its base and where it forked, rather than switch onto it — at once, before it scans the repository or asks anything; delete or merge that branch, then re-run.
...
The report also lists the dev-server commands that break the schema's field rules in the ways it checks — one that binds only the loopback; one that detaches its server, `astro dev` without `ASTRO_DEV_BACKGROUND` set and `astro preview` without `ASTRO_PREVIEW_BACKGROUND` among them, since Astro 7 detaches its dev server wherever it detects an agent, and Astro 7.2 its preview server; and a VitePress or Astro command that moves off a busy port instead of failing, as VitePress does without `--strictPort` and Astro does without `vite.server.strictPort` in its configuration, its command line having no such flag — each with the rule it breaks and how to fix it. VitePress and Astro are the two tools whose pinning it checks: whether a command running another tool moves off a busy port, and what would pin it, is that tool's own behaviour, which the command has no verified form to judge, so the report neither lists nor clears such a command. It fixes none of them itself: which flag or setting a tool takes is the tool's business, so the command stays as detected and you edit it, or the tool's configuration, by hand.
