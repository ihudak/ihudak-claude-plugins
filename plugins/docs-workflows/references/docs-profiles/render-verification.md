...
the same list, maintained at several sites at once, went stale at each of them in turn. **Those ending
names are positional**: a citing site names an ending by its number in this list, so a reordering
