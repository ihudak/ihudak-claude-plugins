...
4. Run `vale ls-config` (the one sanctioned `Bash` use besides read-only inspection) to prove `.vale.ini` parses and every named style resolves; `Read` `accept.txt` for its presence (dimension 4). Run it as `(cd "<the directory holding the written .vale.ini>" && vale ls-config)`, inside one Bash call: your Bash tool starts every call in the session's directory — where the dispatching command stands, which need not be the scaffolded repository — and a `cd` does not persist between calls, while Vale reads the first configuration file it finds in the directory it runs in or in one above it, so a bare `vale ls-config` proves some other repository's configuration, the user's global one, or none. **First `Glob` that directory for the other names Vale reads its configuration from** (`${CLAUDE_PLUGIN_ROOT}/references/toolchain-preflight.md` §2, source 2): Vale takes `.vale`, `_vale` and `vale.ini` before `.vale.ini`, so where one of them sits beside the written file, Vale — `vale ls-config`, Phase 7's `vale docs/` and CI's alike — reads that one instead, and the written `.vale.ini` governs nothing. That is a dimension 4 finding, named with the file that shadows it; a `_vale.ini` beside it is not, since Vale takes `.vale.ini` first.
...
| 4 | Vale configuration | `.vale.ini` is the configuration Vale reads — no `.vale`, `_vale` or `vale.ini` beside it — parses, and every style it names resolves (`vale ls-config`); `accept.txt` exists beside it, carrying `scaffold-tree.md` §7's seed on a `/docs-init` run. |
