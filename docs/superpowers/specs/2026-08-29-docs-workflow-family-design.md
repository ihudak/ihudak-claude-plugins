...
5. Start the command with Bash `run_in_background`. As shipped, with its `{port}` token replaced by the port it serves on, and nothing else in it rewritten, from the repository's git top level, where the profile lives (§8.5) (`/docs-serve` Phase 4). A server command binds the port it is given or fails: one that moves itself off a busy port — as Vite does without `--strictPort` — is a profile defect, like a loopback-only bind (`docs-workflows:docs-profiles/docs-profile-schema`'s field rules).
