...
- **A docs-profile** — loaded in-repo from that top level, from the built-in default (which applies only to a repo shaped like the bundled worked example), or generated on demand by an inline `/docs-profile` run for any other repo that has none; a custom repo whose profiling run is cancelled stops with `PROFILE_REQUIRED`. The inline run hands back the branch it cut and the commit it made, and the finish renames that branch and squashes onto that commit — it never looks either up.
