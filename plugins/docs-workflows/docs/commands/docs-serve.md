...
- **A written `dev_servers` block** — `.dev-workflows/docs-profile.yml` at the resolved repo's git top level (not in the plugin, and not in a site directory below that top level), specifically its `dev_servers.servers[]` list, whose commands run from that top level. A repo that has never been profiled, or whose profile predates `dev_servers`, stops with `DOCS_SERVE_NO_DEV_SERVER` and points at `/docs-workflows:docs-profile` to write one.
