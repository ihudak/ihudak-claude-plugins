...
- **`REPOS_PATH`** — where code clones live; a single directory or a colon-separated list. Defaults to `/workspace`. The match is by `git remote get-url origin` slug where a command was handed the slug, and by directory basename where a command lists candidates to offer you (`/create-ard`, `/idea --ground-code`, `/docs-init`, `/docs-brand`).
