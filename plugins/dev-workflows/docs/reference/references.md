...
- `customer-review-schema.md` — the twelve sections the customer's reviewer fills and sends back, the rule that puts the review's evidence limitations ahead of every claim resting on them, and the D13 rule that the review is exactly one new file with nothing in the package modified; unlike every other reference here its body is rendered verbatim into the customer's prompt, so it names no path, command, agent or skill a reader with nothing installed could not resolve. `/brd-package` inlines its body — from section 2 onward, never the whole file — into the prompt the customer pastes; `/brd-reconcile` will parse a returned review against the same body, once it exists.
- `decision-register-format.md` — the decision register's record shape for a `[VD#n]` (delivery-team) and `[CD#n]` (customer) decision, its five statuses (including the first-class `withdrawn`), the mandatory `argumentation` field, the two causes that may reopen a decision, what `conditional_on` records and makes findable, the D19 rule against resting solely on a `will-change` finding, and the `[AS#n]` assumption every open one of which reaches the customer; `/brd-interview` writes `[VD#n]` and `[AS#n]` records against it and enforces its will-change rule, `brd-package-reviewer` reads them, and `/brd-package` surfaces every open assumption and every `conditional_on` position to the customer; `/brd-reconcile` will consume it once it exists.
