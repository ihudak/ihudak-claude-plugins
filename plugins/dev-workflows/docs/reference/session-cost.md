...
**Why the other six do not.** `/vuln` and `/upgrade` have no Product Requirements Document to attribute spend to and sit outside the pipeline entirely; `/docs-profile`, `/statusline`, and the two guideline reviewers are one-shot utilities with no PRD either.
...
Two details in that sentence are doing real work. The ceded run is found **by name**, not by counting invocations — a window often contains commands that emit no cost entry at all (`/vuln`, `/upgrade`, `/statusline`, `/docs-profile`, the two guideline reviewers), and an interrupted run leaves a mark too, so counting positions would shift every attribution by one and file a security run's spend under a product phase. And a command invocation is recognised by its whole envelope rather than by one tag, because Claude Code writes built-in and plugin commands in two different tag orders; matching only the built-in shape finds no plugin command at all. If a recorded intent matches no invocation, nothing is written for it — its spend stays with the replaying run, and the run says so rather than attaching it to a neighbour.
