...
**When unset.** Every command that gates on it — `/create-prd`, `/update-prd`, `/create-ard`, `/specify`, `/design`, `/ready`, and each `/brd-*` command (`/brd-intake`, `/brd-ground`, `/brd-split`, `/brd-interview`) — stops immediately, names `SPECS_PATH` explicitly, and offers `choices: ["Set SPECS_PATH (enter the path)", "Cancel"]`. `/epics` and `/release-notes` instead degrade: `/epics` skips the artifact steps that need it, and `/release-notes` falls back to `run_phase: pm`. No command silently substitutes the vault, the current working directory, or any other path.
