...
| `/impl:jira:docs <VI-KEY [saas\|managed]>` | Jira-driven feature documentation. Phase 0 preflight-discovers the docs repo + profile (in-repo → built-in dynatrace-docs default → on-demand `/impl:docs:profile`) and the VI's specs dir under `/workspace`. Phase 4.5 determines/confirms the applicable space(s); optional `saas\|managed` constraint scopes the run to one space. Reads the pre-exported Jira hierarchy from the vault, resolves PR URLs to local repos, runs parallel PR-diff summaries, synthesises docs, runs `docs-style-checker` + Opus `doc-reviewer` gates, writes into the docs repo. Jira-vs-source discrepancies are escalated to the user (Phase 5.8) with a bug-report draft. |
