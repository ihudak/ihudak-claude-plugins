...
- **`vi_disposition: rewrite`, `status: refined`** — the key is already known from the `vi` source. Relocate `idea.md` to `$SPECS_PATH/specifications/<KEY>-<slug>/idea.md` (resolve the folder by key-number, tolerating a human-adjusted slug), then execute `handoff-to-main` (`${CLAUDE_PLUGIN_ROOT}/references/phase-handoff.md` §2) with `prefix: idea`, `deliverable_paths` = the relocated file, `title: <KEY> Refine idea for <summary>`. **No human round trip** — the key exists. Then report the §4.1 outcome line.
