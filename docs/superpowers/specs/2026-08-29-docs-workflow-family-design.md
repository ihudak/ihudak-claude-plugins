...
7. Record the pid and port — and the space and visibility of the server started — under `.dev-workflows/` so `--stop` and `--status` work across sessions. As shipped, that is the `.dev-workflows/` at the repository's git top level, beside the profile (§8.5); the pid is the one the socket table names for the port, never a wrapper's, and where `lsof` or `ss` is present it counts as living only while it still holds that port — where neither is, `kill -0` and the port's answering as the docs site stand in, which a reused pid beside a restarted server can pass; `--stop` reports a server stopped only once its port no longer answers as the docs site and no process there is identified as one of its servers (`/docs-serve` Phase 5 and `--stop`). Where no rule ignores the state file, the command appends its one path to the repository's local `.git/info/exclude`, never committed, so a clean-tree gate does not meet it as an untracked file (`/docs-serve` Phase 7). A bare `--port <n>` with `--status` or `--stop` names the server bound to that port, never one a collision moved off it. And where the profile sets `dev_servers.concurrent: false`, nothing starts while another of its servers is known to be running — a living recorded entry, or one this run's own port checks identified, which on such a profile take in every other server's configured port (`/docs-serve` Phase 2 step 3 and Phase 4).
