...
- **`scripts/check-docs.sh` (new CI gate).** Seven checks — internal links and anchors resolve; every docs page is reachable from `docs/README.md`; the commands/agents/reference-files/hooks inventories match the shipped plugin tree; every plugin-read environment variable is documented, and every documented one is actually read; no table cell exceeds 200 characters; `getting-started.md`'s install commands match the repo-root README — plus a `--selftest` mode that mutates a copy of the passing fixture once per check and asserts the gate's exit code against each mutation, so a check that stops being able to fail shows up as a red build instead of a green one. Wired into `.github/workflows/validate-catalog.yml` alongside the pre-existing `check-id-grammar.sh` and `validate-catalog.py` steps.

### Changed
- **`plugins/dev-workflows/README.md` rewritten from 379 lines / 74,620 bytes to 46 lines / 4,410 bytes**, its longest table cell cut from 2,066 to 194 characters. The full command reference, workflow map, and per-command detail moved into the new `docs/` tree; the README itself is now a role-indexed pointer table with a `## Documentation` section linking into `docs/`.
...
- **D2 — `references/cost-emission.md` §7 omitted `/update-vi`.** The table introduced itself as the complete "Fixed per-command labels" authority but listed ten commands; `commands/update-vi.md` calls `emit-cost` with `phase: vi-update, role: pm`, a phase value the table never enumerated. One row inserted immediately after `/create-vi`, preserving lifecycle-stage row order; the table now has eleven rows. Verified complete by a one-time bidirectional check (every command with a fixed phase/role has a row; every row names a real command) but not yet defended by a standing gate — a future command added with a fixed phase/role and no §7 row would still go uncaught.
...
- **D3 — `/ready`'s role is `team` in `references/cost-emission.md` §7, but was `QA` in the old (now-retired) README.** `CLAUDE.md` has zero occurrences of "QA", making `team` the surviving authority, but whether the two labels were ever meant to denote the same thing is a product decision outside a documentation restructure's authority. The one concrete carrier inside this restructure's own scope — `docs/workflow.md`'s Mermaid `QA["QA — verification & gates"]` subgraph label, copied verbatim from the old README — is fixed (now reads `Team — verification & gates`); the broader `/ready`-role-naming question is left for Ivan's decision.
- **D6 — three references still assert the retired four-role vocabulary.** `references/next-phase-offer.md`, `references/session-hygiene.md`, and `references/workflow-states.md` each describe the plugin's roles as a four-way split — "PM / PA / PE / Team" — where `references/cost-emission.md` §7, the authority for every command's emitted role, actually uses five (`pm`/`pa`/`pe`/`dev`/`team`, splitting the old "Team" into `dev` and `team`). This is not just a vocabulary shorthand: `workflow-states.md`'s VI status ladder genuinely performs the collapse, labeling `/implement`'s role `Team` where §7 gives it `dev`. All three files sit outside this restructure's scope (none is under `docs/` or the README) and are left for Ivan to decide whether they should adopt the five-role vocabulary or keep the four-role split as a deliberate simplification.
