...
6. **Poll for readiness** up to `dev_servers.readiness_timeout_seconds` (default 120), then print the URL. As shipped, what answers is judged by step 3's evidence and recorded only where step 3 would re-adopt it as the selected server — its listener in this checkout, identified as that server or as a one-server profile's only one — so a race lost to another checkout, to another of the repo's servers or to a foreign process records nothing and names what won the port; a timeout records its entry without a pid, the one entry that ever lacks one (`/docs-serve` Phase 5).
7. Record the pid and port — and the space and visibility of the server started — under `.dev-workflows/` so `--stop` and `--status` work across sessions. As shipped, that is the `.dev-workflows/` at the repository's git top level, beside the profile (§8.5); the pid is the one the socket table names for the port, never a wrapper's, and it counts as living only while it still holds that port — step 6's timeout being the one entry recorded without one; `--stop` reports a server stopped only once its port no longer answers as the docs site and no process there is identified as one of its servers (`/docs-serve` Phase 5 and `--stop`). Where no rule ignores the state file, the command appends its one path to the repository's local `.git/info/exclude`, never committed, so a clean-tree gate does not meet it as an untracked file (`/docs-serve` Phase 7). A bare `--port <n>` with `--status` or `--stop` names the server bound to that port, never one a collision moved off it. And where the profile sets `dev_servers.concurrent: false`, nothing starts while another of its servers is known to be running — a living recorded entry, or one this run's own port checks identified, which on such a profile take in every other server's configured port (`/docs-serve` Phase 2 step 3 and Phase 4).
