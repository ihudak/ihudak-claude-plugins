...
by calling `resolve-address <jira_key>` (`${CLAUDE_PLUGIN_ROOT}/references/addressing.md` §3), then glob it for `specification.md` and `design.md`. That entry point searches every level §3 bounds and carries §5's legacy fallback — see §7's one-level-deep fallback before concluding none exists; it is reached only on a flat miss, so a flat key resolves exactly as it did before.
