...
2. No living entry returned (a different session started it, the state was lost, or nothing has ever run) — probe the port directly. An unreachable port means nothing is listening **there**. On a profile whose `dev_servers.servers[]` holds one server, that is not yet proof the server is down: an earlier run's Phase 3 may have moved it further up and its record may be gone. So before Phase 4, probe the ports Phase 3 would walk (`port + 1` … `port + 20`), and where one answers as this docs site, re-adopt it by Phase 3's rule for that case; only where none does, proceed to Phase 4. On a profile with more than one server, proceed to Phase 4 at once: a port above answering as this docs site there may be another of its servers on its own configured port, and nothing recorded says which. A port that answers needs identification, not just presence:
...
- **A living recorded entry of another server is bound to it** — whatever its page says, as in Phase 2 step 2 — **or it answers but not as this docs site** — walk on.
- **It answers as this docs site** — the selected server may already be running here, moved by an earlier run's Phase 3 whose record is gone. **Never walk past it to start a second copy.** Where `dev_servers.servers[]` holds one server, re-adopt it exactly as Phase 2 step 2 would have at the configured port: report its URL, say that it sits off the configured port, and record an entry keyed at this port, its `requested_port` the port Phase 1 resolved and its `pid` read from the socket table as Phase 2 step 2 reads it — then stop. Where the profile holds more than one server, Phase 2 step 2's unconfirmed rule governs: report the URL as this docs site with its server unconfirmed, start nothing, and record nothing.
...
- NEVER start a second server on a port that already answers as this docs site — detect and report the existing one instead (Phase 2) — and NEVER walk past such a port to start a second copy elsewhere (Phase 3)
...
- NEVER re-adopt an unrecorded server by guessing which one it is — a profile with one server re-adopts and records it, at its configured port or wherever the fall-forward walk finds it; a profile with more than one reports the match as unconfirmed and starts nothing (Phase 2 step 2, Phase 3)
