...
- **The discrimination proof can itself be broken — check that it moves.** A two-direction proof only means something if the pre-change and post-change runs actually differ. An `awk` range or `grep -A` window that is wider than intended can sweep in unrelated text and return the *same* count on both sides, which reads as "verified" while proving nothing. When a discrimination check shows no difference, suspect the extraction before concluding the content is wrong; when it shows one, confirm the difference is the text you changed.
- **When a fix changes text an assertion greps for, re-derive the assertion in the same breath.** A fix instruction and its covering assertion can contradict each other: rewording a sentence to insert a clause destroys the contiguous substring an earlier check matched on, so following the instruction fails the check and passing the check means ignoring the instruction. Both were written by the same person minutes apart. Whenever a fix touches asserted text, quote the new expected string alongside the new text.
- **Any assertion whose pass condition is empty output must first be shown producing non-empty output.** This is one defect class with three known faces here: `ugrep` returns 0 for a `<(…)` path; `ugrep` warns rather than errors on a missing file; and `git diff` silently returns nothing when a pathspec matches nothing (a stale `cd` doubling a relative path is enough). In every case the tool reports success while having examined nothing. Before trusting an empty result, prove the same command is non-empty against a known-positive input — the pre-change file, a probe string, or the un-narrowed pathspec.
...
- `finish-and-handoff.md:74` — "The plugin never opens the PR itself." becomes *"For a Bitbucket-hosted docs repo the plugin cannot open the pull request — there is no CLI for it — so it writes the draft and the user opens it. This is a host capability limit, not a policy: the plugin does open its own pull request in the **GitHub-hosted specs repo**, which is a separate flow against `$SPECS_PATH` (`phase-handoff.md` §2.6) — never in this docs repo."*
- `document.md:1080` — replace the trailing "The plugin never opens the PR itself." with the same capability sentence, shortened — but it **must** keep the "GitHub-hosted specs repo, a separate flow" qualifier.

**Both of these sit immediately beside an unchanged bullet stating that GitHub gets a `gh pr create` command *the user* may run.** Without the specs-repo qualifier, the new sentence reads locally as "a GitHub-hosted docs repo gets an auto-opened PR", which is false and contradicts the adjacent bullet. `document.md` is the worse case: nothing else in that file mentions the specs repo, so a reader there has no way to infer that §2.6 governs a different repository. Copy the qualifier phrasing from `finish-and-handoff.md:6` and `README.md:217`, which get it right.
