...
A direct-mode run takes no address, so it ordinarily resolves no PRD folder, and its cost entry went to the pending file and its feedback unfiled to the specs-repo root — both reconciling against nothing, forever, which is the problem D19 names. It now takes the same rung `/docs-init` and a standalone `/docs-brand` take: both land under `$SPECS_PATH/documentation/<docs-repo-slug>/dev-workflows/`, the slug being `workflows-core:specs-repo-git` §2.1's name for the write target the run resolved — its git root, or the target directory where that is not a git work tree. Every direct-mode run resolves a write target, so none parks an entry in pending any more; a run whose key does resolve a PRD folder still files under it. Keyed mode is unchanged. Pending and unfiled entries left by earlier direct-mode runs stay where they are.
