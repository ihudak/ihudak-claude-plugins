...
| V18 | surface vii (D-4) — vii-a/vii-b return no unclassified printed invocation site | 0 | vii-a: 1 line (`implement.md:55`, documented LEAVE — self-referential); vii-b: 28 lines, all documented LEAVE/split-verdict-bare-halves (`implement.md:387`, `:445` — 2; five `*_NEEDS_JIRA` messages — 5; 16 subagent-handoff `Command run: /X` templates (9 blockquote-style `> - Command run: /X` + 7 prose-style `ALWAYS pass \`Command run: /X\``) — 16; 3 split-verdict self-referential halves (`update-vi.md:20`, `create-vi.md:21`, `document.md:79`) — 3; 2 comparison/self-referential prose lines (`design.md:193`, `ready.md:373`) — 2). No unclassified site. **Corrected 2026-08-13: the original breakdown (2 + 5 + 14 + 3 + 3 = 27) did not sum to the stated 28 lines. Recount at `f02eee3` (`grep -nHE "([A-Z][A-Z0-9_]{3,}:\|re-run\|\brun\b)[^.]{0,170}(^\|[^:a-z-])/($CMDS)\b" *.md` in `plugins/dev-workflows/commands/`) confirms 28 total lines. The subagent-handoff bucket was undercounted (14 → 16: `vuln.md:179` and `upgrade.md:157` were omitted) and the comparison-prose bucket overcounted (3 → 2, only `design.md:193` and `ready.md:373` qualify); 2 + 5 + 16 + 3 + 2 = 28.** | PASS |
