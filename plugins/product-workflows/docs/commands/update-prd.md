...
- **The PRD**, at the resolved folder's `prd.md` — the only current copy there is (earlier revisions are archived under `revisions/` and never read as the base), and therefore the base without a freshness test. A `prd.md` that exists but cannot be read stops the run with `UPDATE_PRD_BASE_UNREADABLE` before anything is written.
