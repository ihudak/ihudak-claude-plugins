...
- **`proposal-reviewer`** (Phase 9, Opus-pinned by frontmatter, no override) — the review gate, dispatched with both artifact paths, the profile, the resolved tier and the anchor revision where one exists. Its findings are triaged by the orchestrator before anything is edited: each finding is verified at the location it names, every dismissal is recorded with a reason that disposes of that finding's own claim, and only survivors are fixed. Cap: one fix cycle plus one re-review. See [Agents](../reference/agents.md) for the plugin's subagent inventory.
