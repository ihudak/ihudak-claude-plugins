...
**`/implement` is untouched, and that is checked rather than assumed.** It cannot reach an uncovering baseline at verify: every arm of its Pre-Phase 3.5 prompt either re-captures with a hint or records `test_decision: skip`, which drops steps 4–6 and so runs no verify at all. Its own enumeration of verify's `RUN_FAILED` causes therefore stays complete for what it can reach, and a third cause added there would have been a case its own structure excludes.
