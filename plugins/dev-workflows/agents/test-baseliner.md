...
1. **Detect every suite** — Scan the working directory for **every** marker below and **never stop at the first match** — first-match-wins is what made a Rails app carrying an asset-build `package.json` report a JavaScript baseline for a Ruby suite, a wrong number rather than a missing one. A marker becomes a **candidate** only where its own qualifier holds, and a **row** is one candidate however many of its markers are present — three Python markers in one repository are one pytest suite, not three. Every candidate is a suite except where the wrapper rule below folds one into another. Detection runs whether or not a `command_hint` was supplied: the hint decides what is *run*, never what is *detected*, so the return always names the whole set a caller could narrow to. Where a tool ships a pinned entry point, that is the command: `./mvnw`, `./gradlew`, and Ruby's `bin/` binstubs ahead of `bundle exec`.
   ...
   **It is not inert for the runners that already run once, and it is kept anyway.** Jest reads it as `--ci`, which is *"on by default in most popular CI environments"* and under which a snapshot with no committed file fails instead of being written; Mocha from 12 reads it as `--forbid-only`; Playwright's built-in reporter default becomes `dot` rather than `list` (`defaultReporter = process.env.CI ? 'dot' : 'list'`, and both print the `X passed` line step 3's table reads), and the config `npm init playwright` writes takes `forbidOnly`, `retries` and `workers` from it too. Every one of those is **symmetric**, because capture and verify both run under the prefix: a committed `.only` or a stale snapshot fails at both ends and sits in the baseline's own `### Pre-existing failures`, so it is never read as a regression. The asymmetry is what the run itself writes **between** the two calls — a snapshot with no committed file, an `.only` left in — which fails at verify alone, as a **New failure**, a list the caller reads separately from the `Status` (verify step 6). Dropping the prefix costs a hang, which loses a whole suite; a per-runner flag costs the guess this prefix exists to avoid.

   **Karma invoked directly reads no `CI`**: where the script this command actually runs — the root `scripts.test`, or, under a `workspaces` field, each workspace's own, since `--workspaces` runs those and not the root's — runs `karma start` without `--single-run`, that script's suite is commanded `npx karma start <the script's own arguments> --single-run` from the directory that script belongs to, and any other workspaces run through `CI=true npm test --if-present` with a `--workspace <name>` for each, so no workspace runs twice.

   No other row's command watches — `./mvnw test`, `./gradlew test`, `bin/rspec`, `bin/rails test`, `swift test`, `xcodebuild test`, `go test`, `cargo test` and `pytest` each run once and exit. **The wrapper rule below settles *parsing*, not watching**, so `make test` does not escape this paragraph: where that rule folds the `package.json` row into `make test`, the prefix goes with it — `CI=true make test` — since a recipe inherits the environment it was invoked with and hands it to every process it starts, however many levels down the runner sits. Where the Karma carve-out above replaced that row's command, the carve-out's command is the suite's and `make test` is not its entry point.

   Two rules resolve the candidate set, and neither drops a suite:
   - **`Make` is a wrapper, not a rival.** A `Make` candidate whose `test` recipe invokes another candidate's runner is not a suite of its own: that candidate runs through `make test`, the project's own pinned entry point — in the form the paragraph above gives it, `CI=true make test` for the `package.json` row — and is parsed with its own framework's row. Where the recipe invokes more than one candidate's runner, those candidates are **one** suite — `make test` runs once and its output is parsed with each of their rows. **Where the recipe names no runner directly, follow it one level** — the script it executes, or the `$(MAKE) -C <dir> <target>` it delegates to — and apply the same test to what you find; an indirection left unresolved is the case where the same tests run twice and are summed twice, which is the wrong number this step exists to prevent. Where one level does not settle it, both run and `### Notes` names the pair as possibly covering the same tests. Where the recipe's own flags suppress test names, the passing list may be empty and the verify diff falls back to counts.
...
6. **Compute Status** — before returning, set the first that applies:
   ...
   - `RUN_FAILED` — no suite produced counts in this run at all, so nothing was verified. Reached only where the baseline held **no test that could go missing**: any suite it recorded `OK` would have put that suite's tests into **Missing from run** above, so what is left here is a baseline of rows reading `RUN_FAILED`, `not run` or `NO_TESTS`, and suites that had no row at all
