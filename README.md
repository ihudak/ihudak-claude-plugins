...
- **`curl`** *(required by `/docs-serve`)* — it judges a dev server's port with it. Off Linux it also needs `lsof` and `ps`, which macOS ships; on Linux it reads the socket and process tables from `/proc`, so a slim container, the recommended one below included, needs nothing more. `/document`'s opt-in render check uses `curl` too, and without it falls back to a table of pages to check by hand.
