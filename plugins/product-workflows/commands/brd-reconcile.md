...
The presence relation this needs is already in the digest and was recorded and never consumed — the twelve sections, each `present`, `stated-none` or `absent`, and the reason those are **two facts that are never merged** is exactly this test. Three cases, exhaustive and non-overlapping, decided in this order:

1. **Any of the three sets is non-empty** → ordinary run. Nothing below applies.
2. **All three empty, and every one of the twelve sections is `stated-none`** → the customer positively said there is nothing, in every section, which is a real answer and an ordinary outcome. Proceed, and say so in the reconciliation record: a review that changes nothing is a review, and refusing it would make "we accept it as it stands" unrecordable.
3. **All three empty, and at least one section is `absent` or `present`** → stop. `absent` is a section the review does not have; `present` asserts the section exists **with content**, so a `present` section that contributed nothing to any of the three sets is content the reader could not turn into anything. Either way this is a read that failed, not a customer who agreed:
   `BRD_RECONCILE_EMPTY_DIGEST: customer-review-reader returned no decisions, no required changes and no challenges, while <N> sections are absent and <M> are present with content that yielded none of the three. This is a read that failed, not a customer who agreed — a customer who agrees says so, and every section comes back stated-none. The canonicalised review is at <path>; open it, and re-run '/product-workflows:brd-reconcile <KEY> @<review-file>' — telling the run the file is prose, so the reader takes free-text mode, where the schema pass could not see it.`

**The discriminator is `stated-none` versus the other two, and nothing else** — no judgement about which sections are "substantive", which would be a word this file defines nowhere and every reader would resolve differently. The three values already carry the whole distinction: `stated-none` is the customer speaking, `present` is content, `absent` is silence.

Note which way this errs. A customer who genuinely accepts everything reaches case 2 and is recorded; a review the reader could not parse reaches case 3 and is reported. The failure this replaces reported case 3 as though it were case 2.
...
`/product-workflows:brd-split <BRD-KEY>` as the fix. And **no row ever returns to `unallocated`**: that
...
1. **Declared dependents — a scan.** Any BRD whose `brd-link.md` declares `depends-on:` carrying
   ...
   scans `specifications/` and the levels below it that `resolve-address` searches (three, per `workflows-core:addressing` §3), the same bound `resolve-address` uses and for
   the same reason (`workflows-core:addressing` §3, §6). This is the reverse of key resolution: a dependency
...
- **`/product-workflows:create-prd <SLICE-KEY>` is offered only where this run stands on a slice and
  ...
  **The rows are this slice's ledger rows, narrowed by its `brd-link.md` `claims:`.** This
  ...
  Phase 0.
- **`/product-workflows:create-ard <SLICE-KEY>` and `/product-workflows:specify <SLICE-KEY>` are offered on
...
Where this run can go next:
  • Author this slice's PRD           — /product-workflows:create-prd <SLICE-KEY>   (PM)
  • Author this slice's architecture  — /product-workflows:create-ard <SLICE-KEY>   (PA, optional)
  • Author this slice's specification — /product-workflows:specify <SLICE-KEY>      (PE)
...
re-enters at `/product-workflows:brd-ground <SLICE-KEY>`, runs `/product-workflows:brd-split <SLICE-KEY>`
...
fact and offered as nothing, because `/product-workflows:brd-ground` stops on it at
...
wrote — because `/product-workflows:brd-ground`'s own Phase 0 gates `coverage-ledger.md` on
`origin/<default>`, and this run wrote to a coverage ledger.

```
Where this run can go next:
  • Ground a slice                  — /product-workflows:brd-ground <SLICE-KEY> <merge-clause>  (PA), once per non-empty slice
  • Reconcile another BRD or slice  — /product-workflows:brd-reconcile <KEY> @<review-file>
```

```
choices: ["Stop here — the decisions are frozen and both sweeps are recorded", "Ground a slice — /product-workflows:brd-ground <SLICE-KEY> <merge-clause> (PA), once per non-empty slice", "Reconcile a slice — /product-workflows:brd-reconcile <SLICE-KEY> @<review-file> (PM)"]
```
