...
`### REPOS_PATH` still read *"`code-scanner` resolves repositories under it"* — the same sentence, word for word, that this release corrected on `docs/reference/environment.md`, the page this very section links to two paragraphs above. `origin/main` carried it on both pages; only one moved, because the sweep that followed the correction looked for the wording it had just written (*"no agent reads `$REPOS_PATH`"*) and that phrase shares nothing with the sentence still standing. It now states what the reference page states: the command resolves the repository and hands `code-scanner` an absolute `repo_path`. The recipe is unchanged — `grep -l REPOS_PATH plugins/*/agents/*.md` returns nothing.
