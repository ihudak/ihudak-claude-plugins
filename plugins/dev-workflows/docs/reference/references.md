...
- `brd-addressing.md` — the key grammar and the depth-bounded folder-resolution rule for a BRD (business requirements document) under `specifications/`, searching that level and exactly one below it because nesting is capped there; every `/brd-*` command cites it for key validation and BRD-folder resolution, and `/brd-reconcile` uses the same two-level bound in reverse to find every BRD depending on the one it is reconciling.
