...
**When it points somewhere unreadable.** Every miss — unset, missing, unreadable, or no markdown file found — is a **silent, non-blocking skip**, never an error and never a gate or reviewer BLOCKER. `pm-workflows` never writes into `$DOCS_PATH` under any circumstance.
