...
4. **`priority_comparisons[]` is produced by `ia-planner` and applied by nobody inside the pair — this command applies it.** On `--refresh`, a cell that is already occupied is not re-minted; instead `ia-planner` re-derives that unit's `priority` and `priority_reason` and returns both beside the stored pair, keyed by the existing unit's id. `backlog-format.md` §1's mechanical test is then **this command's** to run, because only it holds the file: where the stored reason is exactly the one this run derived, nothing was hand-edited and both fields take the new values; where it differs, both stored values stand and the run reports the pair it would have written. **Skipping this makes `--refresh` unable to update a priority after the code moved, which is most of what a refresh is for** — the defect was found in Task 4's review and fixed on the agent side only.
