...
`/design` is **jira-driven only** — a plain prompt with no Jira input stops with `DESIGN_NEEDS_JIRA`; it uses the shared front-end only to parse the grammar and classify the key, never to read Jira content, because the requirements source of truth is the merged `specification.md` in the specs repo. The front-end resolves `jira_key` (the top-level PRD, or a stand-alone item's own key) and `focus_key` (the Epic to design within it, or `null` for a bare PRD / stand-alone item / directory).

**The Epic is the unit of work — one `design.md` per invocation, no fan-out.** When `focus_key` is already set, that Epic is the scope. When it is `null`, `/design` inspects the resolved PRD directory in the specs repo itself (not Jira): a flat `specification.md` there (a stand-alone Epic, or a broad PRD-level spec) means one design at that level; Epic subfolders are enumerated by which ones already carry a `specification.md` merged to main, and rendered as a progress-aware picker — ○ not started (a specification exists but no design and no in-progress session), ◐ in progress (a design session exists but no `design.md` yet, selectable as a resume), ● done (`design.md` exists, shown greyed, offering *revise* instead of a fresh run). Exactly one spec'd Epic auto-selects with no picker. `--design-twice` forces the Phase 5 interface fan-out even when no contested-interface signal fired — the run says which interface it forced it on, and skips the offer described below entirely.
...
- **A Jira PRD or Epic**, via the shared front-end — a direct prompt is rejected outright (`DESIGN_NEEDS_JIRA`); `/design` has no non-Jira behavior.
