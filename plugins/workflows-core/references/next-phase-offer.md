...
- `/dev-workflows:brd-split <BRD-KEY>` — **depth** → `/dev-workflows:brd-interview <BRD-KEY>` (PM);
  **breadth** → `/dev-workflows:brd-ground <CHILD-KEY>` (PA) once per **non-empty** child the run
  ...
  it holding only orphan rows — is a standing empty child (`/dev-workflows:brd-split` Phase 7), and
...
- `/dev-workflows:brd-interview <BRD-KEY>` → `/dev-workflows:brd-package <BRD-KEY>` (PM), offered
  ...
  (`/dev-workflows:brd-package` Phase 0 owns that test); otherwise → another
...
- `/dev-workflows:epics <PRD>` → `/dev-workflows:specify <EPIC>` (per Epic); optional PA → `/dev-workflows:create-ard <EPIC>`.
  **Every `/dev-workflows:epics` option above and below is conditional on the folder it names
  ...
  under one and refuses everything else (`/dev-workflows:epics` Phase 0 step 1b). Where the caller's
