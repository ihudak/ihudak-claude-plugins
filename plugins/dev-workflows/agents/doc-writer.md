...
Multi-space mechanics are governed by `${CLAUDE_PLUGIN_ROOT}/references/docs-profiles/multi-space-writing.md` and discrepancy application by `${CLAUDE_PLUGIN_ROOT}/references/source-truth.md` §7.4–§7.6 — read them; this agent carries the data, those carry the logic.
...
Apply the no-hard-wrap prose convention in `${CLAUDE_PLUGIN_ROOT}/references/prose-formatting.md` to every prose block you write. Multi-space safety is governed by `${CLAUDE_PLUGIN_ROOT}/references/docs-profiles/multi-space-writing.md`. Before writing, resolve **per-space routing** for each target:
...
- A target whose home space is **not** in `target_spaces` is a routing error — **return `status: BLOCKED`** naming the target (per Entry validation) (it should not occur once Phase 4.5/5.5 honored `target_spaces`) — **unless** it is one of the two legitimate writes outside `target_spaces` (both are step 0 below):
  ...
  - a **`conditional`** edit of a shared source page whose `write_strategy.target_space` IS in `target_spaces`. Editing a shared file outside `target_spaces` is correct and expected — `${CLAUDE_PLUGIN_ROOT}/references/docs-profiles/multi-space-writing.md` §2: *render-unchanged ≠ file-untouched*. The `{{#if project='<target_space>'}}` wrapper leaves the home space's render unchanged. This is the path a `profile.announcement_pages` target takes: all three example-docs announcement pages are cloud-home, so a `[self-hosted]` run reaches them only this way, and blocking them here would make `doc-location-finder`'s announcement exemption inert.
...
0. **Apply the approved write strategy** (per `write_strategies[<target_path>]` and `${CLAUDE_PLUGIN_ROOT}/references/docs-profiles/multi-space-writing.md`):
